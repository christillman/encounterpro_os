...
select distinct r.[matching KE or US rxcui] , r.[comments: Ciru]
...
and [matching KE or US rxcui] != 'KEB3456' -- incorrect brand
and u.[NDA_MAL_HDP] != '2101' -- differing brand/strength

-- [brand_form_rxcui] identified
UPDATE u
SET brand_form_RXCUI = f.form_rxcui, u.SBD_Version = f.form_descr
-- select r.[NDA_MAL_HDP], u.brand_form_RXCUI, r.[brand_form_rxcui], u.SBD_Version, f.form_descr, u.[ug_NAME OF DRUG], [ug_STRENGTH OF DRUG], r.[correction SBD_Version]
from [Uganda_Drugs] u
join [dbo].[05_02_2022_UgandaDrugs_Massaged_Review 1-Revised] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
...
AND r.[correction SBD_Version] IS NULL
and r.[brand_form_rxcui] != 'KEB8640' -- wrong mapping not corrected

UPDATE u
SET brand_form_RXCUI = f.form_rxcui, u.SBD_Version = f.form_descr
-- select r.[NDA_MAL_HDP], u.brand_form_RXCUI, r.[brand_form_rxcui], u.SBD_Version, f.form_descr, u.[ug_NAME OF DRUG], [ug_STRENGTH OF DRUG], r.[matching KE or US rxcui], r.[Comments: Ciru]
from [Uganda_Drugs] u
join [dbo].[05_02_2022_UgandaDrugs_Massaged_Review 1-Revised] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
join c_Drug_Formulation f ON f.form_rxcui = r.[matching KE or US rxcui]
where r.[matching KE or US rxcui] != u.[brand_form_rxcui]
and r.[matching KE or US rxcui] not like 'No match%'


UPDATE u
SET brand_form_RXCUI = r.[matching KE or US rxcui], u.SBD_Version = [correction SBD_Version]
-- select r.[NDA_MAL_HDP], u.brand_form_RXCUI, r.[brand_form_rxcui], u.SBD_Version, u.[ug_NAME OF DRUG], [ug_STRENGTH OF DRUG], r.[matching KE or US rxcui], r.[Comments: Ciru]
from [Uganda_Drugs] u
join [dbo].[05_02_2022_UgandaDrugs_Massaged_Review 1-Revised] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
where u.[brand_form_rxcui] is null and r.[matching KE or US rxcui] is not null
and r.[matching KE or US rxcui] not like 'N%'
and r.[matching KE or US rxcui] not like 'Obsol%'
...
and r.[Matching corrected US or KE generic_rxcui] not like 'Obsol%'
and r.[Matching corrected US or KE generic_rxcui] not like 'N%'
...
UPDATE u
SET brand_name_RXCUI = b.brand_name_RXCUI, brand_name = b.brand_name
-- select u.NDA_MAL_HDP, u.brand_name_RXCUI, r.brand_name, u.[ug_NAME OF DRUG]
from [Uganda_Drugs] u
join [dbo].[20220114ImportUgandaDrugs] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
join c_Drug_Brand b ON b.brand_name = r.brand_name
where u.brand_name is null and r.brand_name is not null
...
update [dbo].[Uganda_Drugs] set generic_form_RXCUI = '197450'
where generic_form_RXCUI = 'KEG5786'

select * from [dbo].[Uganda_Drugs]
where generic_form_RXCUI in ('861004','860999')

select * from c_Drug_Formulation
where form_RXCUI in ('197407','861004','860999')

select * from [dbo].[UgandaJun2022]
where [NDA REGISTRATION NUMBER]='1366' 
...
join [dbo].[05_02_2022_UgandaDrugs_Massaged_Review 1-Revised] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
where u.reviewed = 1
and u.brand_name != r.[Corrected brand_name]
order by u.SBD_Version

-- insert [Corrected brand_name] column
select u.NDA_MAL_HDP, u.brand_name_rxcui, u.brand_name, r.[Corrected brand_name_rxcui], r.[Corrected brand_name]
from [Uganda_Drugs] u
join [dbo].[05_02_2022_UgandaDrugs_Massaged_Review 1-Revised] r on r.[NDA_MAL_HDP] = u.NDA_MAL_HDP
where u.reviewed = 1
and u.brand_name is null and r.[Corrected brand_name] is not null
order by u.SBD_Version

-- Add proposed formulation names
ALTER TABLE Uganda_Drugs_2
ADD proposed_brand_form_descr varchar(max), proposed_generic_form_descr varchar(max)
ALTER TABLE Uganda_Drugs_2
ADD proposed_ingredients varchar(max)

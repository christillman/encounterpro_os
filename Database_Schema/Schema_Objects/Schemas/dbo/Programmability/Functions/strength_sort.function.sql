...
Print 'Drop Function [dbo].[strength_sort]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[strength_sort]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION dbo.strength_sort
go
Print 'Create Function [dbo].[strength_sort]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
CREATE FUNCTION dbo.strength_sort (@drug_description varchar(1000))
...
go
/* test
select descr, dbo.strength_sort(descr) as sort_expression
from c_Drug_Formulation
where descr like 'actonel%' or descr like 'bleph%' 
order by dbo.strength_sort(descr)
*/
...
-- Drop Function [dbo].[fn_document_default_recipient]
Print 'Drop Function [dbo].[fn_document_default_recipient]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_document_default_recipient]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_document_default_recipient]
GO

-- Create Function [dbo].[fn_document_default_recipient]
Print 'Create Function [dbo].[fn_document_default_recipient]'
GO
SET ANSI_NULLS ON

...
-- Drop Trigger [dbo].[tr_p_patient_encounter_progress_insert]
Print 'Drop Trigger [dbo].[tr_p_patient_encounter_progress_insert]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[tr_p_patient_encounter_progress_insert]') AND [type]='TR'))
DROP TRIGGER [dbo].[tr_p_patient_encounter_progress_insert]
GO

-- Create Trigger [dbo].[tr_p_patient_encounter_progress_insert]
Print 'Create Trigger [dbo].[tr_p_patient_encounter_progress_insert]'
GO
SET ANSI_NULLS ON
...
BEGIN
	...
	FROM inserted
	JOIN p_Patient_Encounter ON inserted.cpr_id = p_Patient_Encounter.cpr_id
		AND inserted.encounter_id = p_Patient_Encounter.encounter_id
	WHERE inserted.progress_type = 'UNCancelled'
...
BEGIN
	...
	JOIN p_Patient_Encounter ON inserted.cpr_id = p_Patient_Encounter.cpr_id
		AND inserted.encounter_id = p_Patient_Encounter.encounter_id
	WHERE inserted.progress_type = 'ReOpen'
END
...
-- warning, updating triggered table
UPDATE p_Patient_Encounter_Progress
SET progress = p_Attachment.attachment_text
FROM inserted
JOIN p_Patient_Encounter_Progress ON inserted.cpr_id = p_Patient_Encounter_Progress.cpr_id
	AND inserted.encounter_id = p_Patient_Encounter_Progress.encounter_id

...
-- Drop Procedure [dbo].[jmj_role_task_list]
Print 'Drop Procedure [dbo].[jmj_role_task_list]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmj_role_task_list]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmj_role_task_list]
GO

-- Create Procedure [dbo].[jmj_role_task_list]
Print 'Create Procedure [dbo].[jmj_role_task_list]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE PROCEDURE jmj_role_task_list
(
...
AND a.owned_by = @ps_role_id
AND (a.owned_by = '!Exception' OR a.ordered_service <> 'MESSAGE')
AND (a.office_id IS NULL OR a.office_id IN (SELECT office_id FROM @offices))

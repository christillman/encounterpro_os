...
-- Drop Procedure [dbo].[sp_get_posting_failure_reasons]
Print 'Drop Procedure [dbo].[sp_get_posting_failure_reasons]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_posting_failure_reasons]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_posting_failure_reasons]
GO

-- Create Procedure [dbo].[sp_get_posting_failure_reasons]
Print 'Create Procedure [dbo].[sp_get_posting_failure_reasons]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_posting_failure_reasons]
	TO [cprsystem]
GO

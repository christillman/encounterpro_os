DROP PROCEDURE IF EXISTS [jmjrpt_selectdate_bills]
...
GO
CREATE PROCEDURE [jmjrpt_selectdate_bills] @ps_encounter_date varchar(10)
AS
...
Declare my_daily_curse CURSOR LOCAL FAST_FORWARD for
SELECT	pe.cpr_id,
		pe.encounter_id,
        p.billing_id,
        p.last_name,
...
SELECT @mycount =  (Select count(*)
       ...
       bill_flag = 'Y'
       AND encounter_status = 'CLOSED'
       AND encounter_date BETWEEN @encounterdate AND DATEADD( day, 1, @encounterdate))
If @mycount > 0
 BEGIN
 Open my_daily_curse 
  WHILE @mycount > 0
...
GRANT EXECUTE ON [jmjrpt_selectdate_bills] TO [cprsystem] AS [dbo]

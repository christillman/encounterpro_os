...
-- Drop Function [dbo].[fn_progress_all]
Print 'Drop Function [dbo].[fn_progress_all]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_progress_all]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_progress_all]
GO

-- Create Function [dbo].[fn_progress_all]
Print 'Create Function [dbo].[fn_progress_all]'
GO
SET ANSI_NULLS ON

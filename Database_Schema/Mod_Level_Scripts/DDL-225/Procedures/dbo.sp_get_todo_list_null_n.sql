...
-- Drop Procedure [dbo].[sp_get_todo_list_null_n]
Print 'Drop Procedure [dbo].[sp_get_todo_list_null_n]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_todo_list_null_n]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_todo_list_null_n]
GO

-- Create Procedure [dbo].[sp_get_todo_list_null_n]
Print 'Create Procedure [dbo].[sp_get_todo_list_null_n]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_todo_list_null_n]
	TO [cprsystem]
GO

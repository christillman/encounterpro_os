...
-- Drop Procedure [dbo].[sp_c_display_cmd_att_update]
Print 'Drop Procedure [dbo].[sp_c_display_cmd_att_update]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_c_display_cmd_att_update]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_c_display_cmd_att_update]
GO

-- Create Procedure [dbo].[sp_c_display_cmd_att_update]
Print 'Create Procedure [dbo].[sp_c_display_cmd_att_update]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_c_display_cmd_att_update]
	TO [cprsystem]
GO

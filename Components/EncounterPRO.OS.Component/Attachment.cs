...
namespace EncounterPRO.OS.Component
{
	...
	/// </summary>
	public abstract class Attachment : Common
	{
		private System.Collections.Specialized.StringDictionary attributes = null;
		...
		public StdEproReturn Display (byte[] Attachment, string Extension)
		{
			try
			{
				return display (Attachment, Extension);
			}
			catch(Exception exc)
			{
		...
		public byte[] Edit (byte[] Attachment, string Extension)
		{
			try
			{
				byte[] result = edit (Attachment, Extension);
				if(null == result)
					return new byte[]{}; // Can't pass null through COM-interop to PB
				else
					return result;
			}
			catch(Exception exc)
		...
		public bool Is_Displayable (string Extension)
		{
			try
			{
				return isDisplayable (Extension);
			}
			catch(Exception exc)
			{
				Log(exc.ToString(), System.Diagnostics.EventLogEntryType.Error);
		...
		public byte[] Render (byte[] Attachment, string Extension, int Width, int Height, ref string FileType)
		{
			try
			{
				...
				byte[] result = render (Attachment, Extension, Width, Height, ref FileType);
				if(null == result)
					return new byte[]{}; // Can't pass null through COM-interop to PB
				else
					return result;
		...
		protected virtual StdEproReturn initialize (int AttributeCount, string[] Attributes, string[] Values)
		{ return StdEproReturn.Success; }
		protected virtual StdEproReturn display (byte[] Attachment, string Extension)
		{
			throw new Exception("Method not implemented.");
		}
		protected virtual byte[] edit (byte[] Attachment, string Extension)
		{
			throw new Exception("Method not implemented.");
		}
		protected abstract bool isDisplayable (string Extension);
		protected abstract bool isEditable (string Extension);
		protected virtual byte[] render (byte[] Attachment, string Extension, int Width, int Height, ref string FileType)
		{
			return new byte[] {};
		}

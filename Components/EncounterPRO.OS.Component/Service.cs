...
namespace EncounterPRO.OS.Component
{
	...
	/// </summary>
	public abstract class Service : Common
	{
		public Service():base()
		{
		}
		...
		/// <summary>
		/// Performs a desired service
		/// </summary>
		/// <returns>ServiceReturn value indicating the completion state of the associated
		/// workplan item</returns>
		public string DoService(string XmlDocument)
		{
			try
			{
				string result = doService(XmlDocument);
				if(null==result)
		...
		public string ConfigureService(string XmlDocument)
		{
			try
			{
				string result = configureService(XmlDocument);
				if(null==result)
					return string.Empty; // Can't pass null through COM-Interop to PB
				else
					return result;
		...
		protected abstract string doService(string XmlDocument);
		protected abstract string configureService(string XmlDocument);
	}
}

...
namespace EncounterPRO.OS.Component
{
	...
	/// </summary>
	public abstract class Document : Common
	{
		...
		public string CreateDocument()
		{
			...
			{
                XmlDocument = null;
                string result = createDocument(XmlDocument);
				if(null==result)
		...
		public string CreateDocument(string XmlDocument)
		{
			try
			{
				string result = createDocument(XmlDocument);
		...
		public string ConfigureDocument(string XmlDocument)
		{
			try
			{
				string result = configureDocument(XmlDocument);
				if(null==result)
					return string.Empty; // Can't pass null through COM-interop to PB
				else
					return result;
		...
		public string GetDocumentElements(string XmlDocument)
		{
			try
			{
				string result = getDocumentElements(XmlDocument);
				if(null==result)
					return string.Empty; // Can't pass null through COM-interop to PB
				else
					return result;
		...
		protected abstract string createDocument(string XmlDocument);
		protected abstract string configureDocument(string XmlDocument);
		protected abstract string getDocumentElements(string XmlDocument);
	}
}

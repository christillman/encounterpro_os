...
-- Drop Procedure [dbo].[jmj_list_messages_for_day]
Print 'Drop Procedure [dbo].[jmj_list_messages_for_day]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmj_list_messages_for_day]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmj_list_messages_for_day]
GO

-- Create Procedure [dbo].[jmj_list_messages_for_day]
Print 'Create Procedure [dbo].[jmj_list_messages_for_day]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmj_list_messages_for_day]
	TO [cprsystem]
GO

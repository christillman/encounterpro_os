...
-- Drop Procedure [dbo].[sp_Get_Encounter_Property]
Print 'Drop Procedure [dbo].[sp_Get_Encounter_Property]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_Get_Encounter_Property]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_Get_Encounter_Property]
GO

-- Create Procedure [dbo].[sp_Get_Encounter_Property]
Print 'Create Procedure [dbo].[sp_Get_Encounter_Property]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_Get_Encounter_Property]
	TO [cprsystem]
GO

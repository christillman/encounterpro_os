...
-- Drop Procedure [dbo].[sp_complete_workplan_item]
Print 'Drop Procedure [dbo].[sp_complete_workplan_item]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_complete_workplan_item]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_complete_workplan_item]
GO

-- Create Procedure [dbo].[sp_complete_workplan_item]
Print 'Create Procedure [dbo].[sp_complete_workplan_item]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_complete_workplan_item]
	TO [cprsystem]
GO

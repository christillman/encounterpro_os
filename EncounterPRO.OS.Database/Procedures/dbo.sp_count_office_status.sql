...
-- Drop Procedure [dbo].[sp_count_office_status]
Print 'Drop Procedure [dbo].[sp_count_office_status]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_count_office_status]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_count_office_status]
GO

-- Create Procedure [dbo].[sp_count_office_status]
Print 'Create Procedure [dbo].[sp_count_office_status]'
GO
SET ANSI_NULLS ON
...
FROM	p_Patient_WP w WITH (NOLOCK)
		...
		JOIN o_Rooms r (NOLOCK) ON e.patient_location = r.room_id
		JOIN o_Group_Rooms g (NOLOCK) ON r.room_id = g.room_id

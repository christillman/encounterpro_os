...
-- Drop Procedure [dbo].[sp_get_treatments_to_post]
Print 'Drop Procedure [dbo].[sp_get_treatments_to_post]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_treatments_to_post]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_treatments_to_post]
GO

-- Create Procedure [dbo].[sp_get_treatments_to_post]
Print 'Create Procedure [dbo].[sp_get_treatments_to_post]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_treatments_to_post]
	TO [cprsystem]
GO

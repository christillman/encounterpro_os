DROP PROCEDURE IF EXISTS [jmjrpt_patient_bills]
...
CREATE PROCEDURE [jmjrpt_patient_bills] @ps_cpr_id varchar(12)
AS
...
Declare my_daily_curse CURSOR LOCAL FAST_FORWARD TYPE_WARNING  for
SELECT	pe.encounter_date,
	pe.encounter_id,
        p.billing_id,
        p.last_name,
        p.first_name,
  ...
  WHILE @mycount > 0
   BEGIN
    ...
    FETCH NEXT FROM my_daily_curse
       into @encounterdate,@li_encounter_id,@billing_id,@last_name,@first_name,@dischargedate
...
group by encounter_date,pat_name,icd_9_code,cpt_code,modifier,charge,discharge_date
DROP Table ##c_bill_Tree_Table
DEALLOCATE my_daily_curse



GRANT EXECUTE ON [jmjrpt_patient_bills] TO [cprsystem] AS [dbo]

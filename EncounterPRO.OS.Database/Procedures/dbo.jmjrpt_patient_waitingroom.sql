...
-- Drop Procedure [dbo].[jmjrpt_patient_waitingroom]
Print 'Drop Procedure [dbo].[jmjrpt_patient_waitingroom]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_patient_waitingroom]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_patient_waitingroom]
GO

-- Create Procedure [dbo].[jmjrpt_patient_waitingroom]
Print 'Create Procedure [dbo].[jmjrpt_patient_waitingroom]'
GO
SET ANSI_NULLS ON
...
CREATE PROCEDURE jmjrpt_patient_waitingroom
...
SELECT uof.description As Office
      ...
      ,Avg(DATEDIFF(minute, pp.progress_date_time, wp.begin_date)) AS Avg_Minutes_Wait
...
Group by uof.description,usr.user_short_name
Order by office asc, Provider asc
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_patient_waitingroom]
	TO [cprsystem]
GO

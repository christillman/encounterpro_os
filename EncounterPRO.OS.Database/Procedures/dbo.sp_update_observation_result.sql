...
-- Drop Procedure [dbo].[sp_update_observation_result]
Print 'Drop Procedure [dbo].[sp_update_observation_result]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_update_observation_result]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_update_observation_result]
GO

-- Create Procedure [dbo].[sp_update_observation_result]
Print 'Create Procedure [dbo].[sp_update_observation_result]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_update_observation_result]
	TO [cprsystem]
GO

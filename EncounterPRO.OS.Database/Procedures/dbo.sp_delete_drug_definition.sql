...
-- Drop Procedure [dbo].[sp_delete_drug_definition]
Print 'Drop Procedure [dbo].[sp_delete_drug_definition]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_delete_drug_definition]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_delete_drug_definition]
GO

-- Create Procedure [dbo].[sp_delete_drug_definition]
Print 'Create Procedure [dbo].[sp_delete_drug_definition]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_delete_drug_definition]
	TO [cprsystem]
GO

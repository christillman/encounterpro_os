...
-- Drop Procedure [dbo].[jmjrpt_vaccine_sum_usage]
Print 'Drop Procedure [dbo].[jmjrpt_vaccine_sum_usage]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_vaccine_sum_usage]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_vaccine_sum_usage]
GO

-- Create Procedure [dbo].[jmjrpt_vaccine_sum_usage]
Print 'Create Procedure [dbo].[jmjrpt_vaccine_sum_usage]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_vaccine_sum_usage]
	TO [cprsystem]
GO

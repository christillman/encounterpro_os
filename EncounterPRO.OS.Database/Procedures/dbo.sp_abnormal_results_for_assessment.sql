...
-- Drop Procedure [dbo].[sp_abnormal_results_for_assessment]
Print 'Drop Procedure [dbo].[sp_abnormal_results_for_assessment]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_abnormal_results_for_assessment]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_abnormal_results_for_assessment]
GO

-- Create Procedure [dbo].[sp_abnormal_results_for_assessment]
Print 'Create Procedure [dbo].[sp_abnormal_results_for_assessment]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE PROCEDURE sp_abnormal_results_for_assessment (
...
FROM p_Treatment_Item t
	...
	INNER JOIN c_Observation_Type ot
	ON co.observation_type = ot.observation_type
...
GO
GRANT EXECUTE
	ON [dbo].[sp_abnormal_results_for_assessment]
	TO [cprsystem]
GO

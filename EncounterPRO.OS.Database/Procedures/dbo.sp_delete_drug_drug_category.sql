...
-- Drop Procedure [dbo].[sp_delete_drug_drug_category]
Print 'Drop Procedure [dbo].[sp_delete_drug_drug_category]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_delete_drug_drug_category]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_delete_drug_drug_category]
GO

-- Create Procedure [dbo].[sp_delete_drug_drug_category]
Print 'Create Procedure [dbo].[sp_delete_drug_drug_category]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_delete_drug_drug_category]
	TO [cprsystem]
GO

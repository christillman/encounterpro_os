...
-- Drop Procedure [dbo].[sp_update_family_illness]
Print 'Drop Procedure [dbo].[sp_update_family_illness]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_update_family_illness]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_update_family_illness]
GO

-- Create Procedure [dbo].[sp_update_family_illness]
Print 'Create Procedure [dbo].[sp_update_family_illness]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_update_family_illness]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[jmj_reset_active_services]
Print 'Drop Procedure [dbo].[jmj_reset_active_services]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmj_reset_active_services]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmj_reset_active_services]
GO

-- Create Procedure [dbo].[jmj_reset_active_services]
Print 'Create Procedure [dbo].[jmj_reset_active_services]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmj_reset_active_services]
	TO [cprsystem]
GO

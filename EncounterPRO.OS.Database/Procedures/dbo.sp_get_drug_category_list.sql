...
-- Drop Procedure [dbo].[sp_get_drug_category_list]
Print 'Drop Procedure [dbo].[sp_get_drug_category_list]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_drug_category_list]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_drug_category_list]
GO

-- Create Procedure [dbo].[sp_get_drug_category_list]
Print 'Create Procedure [dbo].[sp_get_drug_category_list]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_drug_category_list]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_get_folder_message_list]
Print 'Drop Procedure [dbo].[sp_get_folder_message_list]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_folder_message_list]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_folder_message_list]
GO

-- Create Procedure [dbo].[sp_get_folder_message_list]
Print 'Create Procedure [dbo].[sp_get_folder_message_list]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE PROCEDURE sp_get_folder_message_list (
...
AND i.folder = @ps_folder
AND i.status = 'COMPLETED'
AND i.ordered_service = 'MESSAGE'

GO
GRANT EXECUTE
	ON [dbo].[sp_get_folder_message_list]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_get_preferred_provider]
Print 'Drop Procedure [dbo].[sp_get_preferred_provider]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_preferred_provider]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_preferred_provider]
GO

-- Create Procedure [dbo].[sp_get_preferred_provider]
Print 'Create Procedure [dbo].[sp_get_preferred_provider]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_preferred_provider]
	TO [cprsystem]
GO

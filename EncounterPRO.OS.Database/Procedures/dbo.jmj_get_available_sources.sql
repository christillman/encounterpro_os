...
-- Drop Procedure [dbo].[jmj_get_available_sources]
Print 'Drop Procedure [dbo].[jmj_get_available_sources]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmj_get_available_sources]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmj_get_available_sources]
GO

-- Create Procedure [dbo].[jmj_get_available_sources]
Print 'Create Procedure [dbo].[jmj_get_available_sources]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmj_get_available_sources]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_new_menu_item]
Print 'Drop Procedure [dbo].[sp_new_menu_item]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_new_menu_item]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_new_menu_item]
GO

-- Create Procedure [dbo].[sp_new_menu_item]
Print 'Create Procedure [dbo].[sp_new_menu_item]'
GO
SET ANSI_NULLS ON

...
-- Drop Procedure [dbo].[jmjrpt_patients_with_ins]
Print 'Drop Procedure [dbo].[jmjrpt_patients_with_ins]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_patients_with_ins]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_patients_with_ins]
GO

-- Create Procedure [dbo].[jmjrpt_patients_with_ins]
Print 'Create Procedure [dbo].[jmjrpt_patients_with_ins]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_patients_with_ins]
	TO [cprsystem]
GO

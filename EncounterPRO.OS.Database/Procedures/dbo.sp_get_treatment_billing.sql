...
-- Drop Procedure [dbo].[sp_get_treatment_billing]
Print 'Drop Procedure [dbo].[sp_get_treatment_billing]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_treatment_billing]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_treatment_billing]
GO

-- Create Procedure [dbo].[sp_get_treatment_billing]
Print 'Create Procedure [dbo].[sp_get_treatment_billing]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_treatment_billing]
	TO [cprsystem]
GO

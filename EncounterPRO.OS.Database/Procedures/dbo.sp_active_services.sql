...
-- Drop Procedure [dbo].[sp_active_services]
Print 'Drop Procedure [dbo].[sp_active_services]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_active_services]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_active_services]
GO

-- Create Procedure [dbo].[sp_active_services]
Print 'Create Procedure [dbo].[sp_active_services]'
GO
SET ANSI_NULLS ON

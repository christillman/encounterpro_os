Print 'Drop Procedure [dbo].[sp_supply_missing_dose_unit]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'sp_supply_missing_dose_unit') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_supply_missing_dose_unit]
GO

Print 'Create Procedure [dbo].[sp_supply_missing_dose_unit]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE sp_supply_missing_dose_unit
AS BEGIN
/*
	Sets values where missing for dose_unit, dosage_form, default_dispense_unit, and administer_method 
	This is critical when selecting drugs for prescription; the packages are not recognized unless 
	dose_unit is filled in
*/
SELECT form_rxcui 
INTO #insulin
...
where form_rxcui IN (SELECT form_rxcui FROM #insulin)
AND ISNULL(generic_form_descr, form_descr) LIKE '%inhalation powder%'
and default_dispense_unit IS NULL
-- (6 row(s) affected)
...
or ISNULL(generic_form_descr, form_descr) LIKE '%dispersible oral tab%'
and dose_unit IS NULL
-- 0

...
-- Drop Procedure [dbo].[sp_set_workplan_status]
Print 'Drop Procedure [dbo].[sp_set_workplan_status]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_set_workplan_status]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_set_workplan_status]
GO

-- Create Procedure [dbo].[sp_set_workplan_status]
Print 'Create Procedure [dbo].[sp_set_workplan_status]'
GO
SET ANSI_NULLS ON
...
IF ((@ps_progress_type = 'COMPLETED' AND @ls_close_flag = 'Y') OR (@ps_progress_type = 'CANCELLED' AND @ls_cancel_flag = 'Y'))

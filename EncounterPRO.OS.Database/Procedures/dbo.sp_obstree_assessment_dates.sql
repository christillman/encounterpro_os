...
-- Drop Procedure [dbo].[sp_obstree_assessment_dates]
Print 'Drop Procedure [dbo].[sp_obstree_assessment_dates]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_obstree_assessment_dates]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_obstree_assessment_dates]
GO

-- Create Procedure [dbo].[sp_obstree_assessment_dates]
Print 'Create Procedure [dbo].[sp_obstree_assessment_dates]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_obstree_assessment_dates]
	TO [cprsystem]
GO

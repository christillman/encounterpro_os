...
-- Drop Procedure [dbo].[sp_treatment_any_results]
Print 'Drop Procedure [dbo].[sp_treatment_any_results]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_treatment_any_results]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_treatment_any_results]
GO

-- Create Procedure [dbo].[sp_treatment_any_results]
Print 'Create Procedure [dbo].[sp_treatment_any_results]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_treatment_any_results]
	TO [cprsystem]
GO

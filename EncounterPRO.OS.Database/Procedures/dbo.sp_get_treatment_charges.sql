...
-- Drop Procedure [dbo].[sp_get_treatment_charges]
Print 'Drop Procedure [dbo].[sp_get_treatment_charges]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_treatment_charges]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_treatment_charges]
GO

-- Create Procedure [dbo].[sp_get_treatment_charges]
Print 'Create Procedure [dbo].[sp_get_treatment_charges]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_treatment_charges]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_get_growth_data]
Print 'Drop Procedure [dbo].[sp_get_growth_data]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_growth_data]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_growth_data]
GO

-- Create Procedure [dbo].[sp_get_growth_data]
Print 'Create Procedure [dbo].[sp_get_growth_data]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE Procedure sp_get_growth_data (@ps_cpr_id varchar(12),
@ps_obs_id VARCHAR(24),
...
SELECT 	@ps_cpr_id,
	...
	result_unit ,
	observation_id 
FROM dbo.fn_patient_results(@ps_cpr_id, @ps_obs_id, -1)
WHERE location = 'NA'

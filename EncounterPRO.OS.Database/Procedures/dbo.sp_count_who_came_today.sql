...
-- Drop Procedure [dbo].[sp_count_who_came_today]
Print 'Drop Procedure [dbo].[sp_count_who_came_today]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_count_who_came_today]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_count_who_came_today]
GO

-- Create Procedure [dbo].[sp_count_who_came_today]
Print 'Create Procedure [dbo].[sp_count_who_came_today]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_count_who_came_today]
	TO [cprsystem]
GO

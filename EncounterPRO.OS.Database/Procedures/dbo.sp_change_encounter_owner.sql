...
-- Drop Procedure [dbo].[sp_change_encounter_owner]
Print 'Drop Procedure [dbo].[sp_change_encounter_owner]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_change_encounter_owner]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_change_encounter_owner]
GO

-- Create Procedure [dbo].[sp_change_encounter_owner]
Print 'Create Procedure [dbo].[sp_change_encounter_owner]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_change_encounter_owner]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_Find_Encounter_Service]
Print 'Drop Procedure [dbo].[sp_Find_Encounter_Service]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_Find_Encounter_Service]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_Find_Encounter_Service]
GO

-- Create Procedure [dbo].[sp_Find_Encounter_Service]
Print 'Create Procedure [dbo].[sp_Find_Encounter_Service]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_Find_Encounter_Service]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[sp_get_todo_list]
Print 'Drop Procedure [dbo].[sp_get_todo_list]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_todo_list]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_todo_list]
GO

-- Create Procedure [dbo].[sp_get_todo_list]
Print 'Create Procedure [dbo].[sp_get_todo_list]'
GO
SET ANSI_NULLS ON

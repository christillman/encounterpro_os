...
-- Drop Procedure [dbo].[jmjrpt_lab_results_pending]
Print 'Drop Procedure [dbo].[jmjrpt_lab_results_pending]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_lab_results_pending]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_lab_results_pending]
GO

-- Create Procedure [dbo].[jmjrpt_lab_results_pending]
Print 'Create Procedure [dbo].[jmjrpt_lab_results_pending]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_lab_results_pending]
	TO [cprsystem]
GO

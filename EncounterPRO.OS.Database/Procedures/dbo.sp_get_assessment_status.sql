...
-- Drop Procedure [dbo].[sp_get_assessment_status]
Print 'Drop Procedure [dbo].[sp_get_assessment_status]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_assessment_status]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_assessment_status]
GO

-- Create Procedure [dbo].[sp_get_assessment_status]
Print 'Create Procedure [dbo].[sp_get_assessment_status]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_assessment_status]
	TO [cprsystem]
GO

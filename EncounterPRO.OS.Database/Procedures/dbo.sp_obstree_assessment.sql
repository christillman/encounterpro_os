...
-- Drop Procedure [dbo].[sp_obstree_assessment]
Print 'Drop Procedure [dbo].[sp_obstree_assessment]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_obstree_assessment]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_obstree_assessment]
GO

-- Create Procedure [dbo].[sp_obstree_assessment]
Print 'Create Procedure [dbo].[sp_obstree_assessment]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE   PROCEDURE sp_obstree_assessment
(	 @ps_cpr_id varchar(12)
	,@ps_observation_id varchar(24)
	,@pl_problem_id int
	,@ps_attached_observations_only char(1) = 'Y'
)
AS
...
/*
	...
	patient, root observation, and assessment.  The top part of the tree is the latest 
...
SET	Observation_sequence = 
		(	SELECT	MAX(observation_sequence)
			...
			AND (	EXISTS (
					...
					AND pat.treatment_id = r.treatment_id
					AND pat.problem_id = @pl_problem_id
							)
				OR (@ps_attached_observations_only = 'N'
					AND r.result_date_time >= ISNULL( @ldt_begin_date, '1-1-1900' )
					AND	r.result_date_time <= ISNULL( @ldt_end_date, '1-1-2100' )
					)
				)
...
ON	x.observation_sequence = r.observation_sequence
...
ON	x.observation_sequence = c.observation_sequence
WHERE 	c.cpr_id = @ps_cpr_id
--AND	c.current_flag = 'Y'
AND (	EXISTS (
		SELECT 1

...
-- Drop Procedure [dbo].[sp_abnormal_results]
Print 'Drop Procedure [dbo].[sp_abnormal_results]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_abnormal_results]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_abnormal_results]
GO

-- Create Procedure [dbo].[sp_abnormal_results]
Print 'Create Procedure [dbo].[sp_abnormal_results]'
GO
SET ANSI_NULLS ON

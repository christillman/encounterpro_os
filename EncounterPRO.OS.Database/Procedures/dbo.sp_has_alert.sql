...
-- Drop Procedure [dbo].[sp_has_alert]
Print 'Drop Procedure [dbo].[sp_has_alert]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_has_alert]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_has_alert]
GO

-- Create Procedure [dbo].[sp_has_alert]
Print 'Create Procedure [dbo].[sp_has_alert]'
GO
SET ANSI_NULLS OFF

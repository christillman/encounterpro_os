...
-- Drop Procedure [dbo].[sp_cancel_encounter_objects]
Print 'Drop Procedure [dbo].[sp_cancel_encounter_objects]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_cancel_encounter_objects]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_cancel_encounter_objects]
GO

-- Create Procedure [dbo].[sp_cancel_encounter_objects]
Print 'Create Procedure [dbo].[sp_cancel_encounter_objects]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_cancel_encounter_objects]
	TO [cprsystem]
GO

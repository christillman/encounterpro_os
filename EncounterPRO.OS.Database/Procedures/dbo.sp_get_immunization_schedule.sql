...
-- Drop Procedure [dbo].[sp_get_immunization_schedule]
Print 'Drop Procedure [dbo].[sp_get_immunization_schedule]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_immunization_schedule]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_immunization_schedule]
GO

-- Create Procedure [dbo].[sp_get_immunization_schedule]
Print 'Create Procedure [dbo].[sp_get_immunization_schedule]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_immunization_schedule]
	TO [cprsystem]
GO

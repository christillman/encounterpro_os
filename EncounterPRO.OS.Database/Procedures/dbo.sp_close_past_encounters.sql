...
-- Drop Procedure [dbo].[sp_close_past_encounters]
Print 'Drop Procedure [dbo].[sp_close_past_encounters]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_close_past_encounters]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_close_past_encounters]
GO

-- Create Procedure [dbo].[sp_close_past_encounters]
Print 'Create Procedure [dbo].[sp_close_past_encounters]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_close_past_encounters]
	TO [cprsystem]
GO

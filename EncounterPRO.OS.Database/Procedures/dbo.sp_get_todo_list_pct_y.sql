...
-- Drop Procedure [dbo].[sp_get_todo_list_pct_y]
Print 'Drop Procedure [dbo].[sp_get_todo_list_pct_y]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_todo_list_pct_y]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_todo_list_pct_y]
GO

-- Create Procedure [dbo].[sp_get_todo_list_pct_y]
Print 'Create Procedure [dbo].[sp_get_todo_list_pct_y]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE PROCEDURE sp_get_todo_list_pct_y
(
	@ps_user_id varchar(24)
)
AS

-- This procedure displays all of the IN OFFICE pending services for the given user.

...
-- Drop Procedure [dbo].[sp_get_diseases_to_attach]
Print 'Drop Procedure [dbo].[sp_get_diseases_to_attach]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_diseases_to_attach]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_diseases_to_attach]
GO

-- Create Procedure [dbo].[sp_get_diseases_to_attach]
Print 'Create Procedure [dbo].[sp_get_diseases_to_attach]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_diseases_to_attach]
	TO [cprsystem]
GO

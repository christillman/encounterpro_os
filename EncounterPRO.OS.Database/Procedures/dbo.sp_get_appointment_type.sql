...
-- Drop Procedure [dbo].[sp_get_appointment_type]
Print 'Drop Procedure [dbo].[sp_get_appointment_type]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_appointment_type]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_appointment_type]
GO

-- Create Procedure [dbo].[sp_get_appointment_type]
Print 'Create Procedure [dbo].[sp_get_appointment_type]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_appointment_type]
	TO [cprsystem]
GO

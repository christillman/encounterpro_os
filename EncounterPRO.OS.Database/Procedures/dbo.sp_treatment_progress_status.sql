...
-- Drop Procedure [dbo].[sp_treatment_progress_status]
Print 'Drop Procedure [dbo].[sp_treatment_progress_status]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_treatment_progress_status]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_treatment_progress_status]
GO

-- Create Procedure [dbo].[sp_treatment_progress_status]
Print 'Create Procedure [dbo].[sp_treatment_progress_status]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_treatment_progress_status]
	TO [cprsystem]
GO

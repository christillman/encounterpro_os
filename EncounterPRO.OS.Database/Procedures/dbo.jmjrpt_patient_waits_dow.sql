...
-- Drop Procedure [dbo].[jmjrpt_patient_waits_dow]
Print 'Drop Procedure [dbo].[jmjrpt_patient_waits_dow]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_patient_waits_dow]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_patient_waits_dow]
GO

-- Create Procedure [dbo].[jmjrpt_patient_waits_dow]
Print 'Create Procedure [dbo].[jmjrpt_patient_waits_dow]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_patient_waits_dow]
	TO [cprsystem]
GO

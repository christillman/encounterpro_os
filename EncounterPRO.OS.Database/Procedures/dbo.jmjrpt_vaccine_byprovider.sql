...
-- Drop Procedure [dbo].[jmjrpt_vaccine_byprovider]
Print 'Drop Procedure [dbo].[jmjrpt_vaccine_byprovider]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[jmjrpt_vaccine_byprovider]') AND [type]='P'))
DROP PROCEDURE [dbo].[jmjrpt_vaccine_byprovider]
GO

-- Create Procedure [dbo].[jmjrpt_vaccine_byprovider]
Print 'Create Procedure [dbo].[jmjrpt_vaccine_byprovider]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[jmjrpt_vaccine_byprovider]
	TO [cprsystem]
GO

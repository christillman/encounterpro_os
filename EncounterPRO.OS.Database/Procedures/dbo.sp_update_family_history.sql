...
-- Drop Procedure [dbo].[sp_update_family_history]
Print 'Drop Procedure [dbo].[sp_update_family_history]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_update_family_history]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_update_family_history]
GO

-- Create Procedure [dbo].[sp_update_family_history]
Print 'Create Procedure [dbo].[sp_update_family_history]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_update_family_history]
	TO [cprsystem]
GO

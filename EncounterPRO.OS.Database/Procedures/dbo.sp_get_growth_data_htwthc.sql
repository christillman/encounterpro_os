...
-- Drop Procedure [dbo].[sp_get_growth_data_htwthc]
Print 'Drop Procedure [dbo].[sp_get_growth_data_htwthc]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_growth_data_htwthc]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_growth_data_htwthc]
GO

-- Create Procedure [dbo].[sp_get_growth_data_htwthc]
Print 'Create Procedure [dbo].[sp_get_growth_data_htwthc]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_growth_data_htwthc]
	TO [cprsystem]
GO

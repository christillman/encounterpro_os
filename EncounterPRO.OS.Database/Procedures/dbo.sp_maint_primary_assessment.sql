...
-- Drop Procedure [dbo].[sp_maint_primary_assessment]
Print 'Drop Procedure [dbo].[sp_maint_primary_assessment]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_maint_primary_assessment]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_maint_primary_assessment]
GO

-- Create Procedure [dbo].[sp_maint_primary_assessment]
Print 'Create Procedure [dbo].[sp_maint_primary_assessment]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_maint_primary_assessment]
	TO [cprsystem]
GO

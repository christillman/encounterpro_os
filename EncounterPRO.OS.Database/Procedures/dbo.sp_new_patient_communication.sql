...
-- Drop Procedure [dbo].[sp_new_patient_communication]
Print 'Drop Procedure [dbo].[sp_new_patient_communication]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_new_patient_communication]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_new_patient_communication]
GO

-- Create Procedure [dbo].[sp_new_patient_communication]
Print 'Create Procedure [dbo].[sp_new_patient_communication]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_new_patient_communication]
	TO [cprsystem]
GO

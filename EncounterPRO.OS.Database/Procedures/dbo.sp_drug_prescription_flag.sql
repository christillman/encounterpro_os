...
-- Drop Procedure [dbo].[sp_drug_prescription_flag]
Print 'Drop Procedure [dbo].[sp_drug_prescription_flag]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_drug_prescription_flag]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_drug_prescription_flag]
GO

-- Create Procedure [dbo].[sp_drug_prescription_flag]
Print 'Create Procedure [dbo].[sp_drug_prescription_flag]'
GO
SET ANSI_NULLS OFF
...
GO
GRANT EXECUTE
	ON [dbo].[sp_drug_prescription_flag]
	TO [cprsystem]
GO

...
-- Drop Procedure [dbo].[xml_set_document_mapping]
Print 'Drop Procedure [dbo].[xml_set_document_mapping]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[xml_set_document_mapping]') AND [type]='P'))
DROP PROCEDURE [dbo].[xml_set_document_mapping]
GO

-- Create Procedure [dbo].[xml_set_document_mapping]
Print 'Create Procedure [dbo].[xml_set_document_mapping]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[xml_set_document_mapping]
	TO [cprsystem]
GO

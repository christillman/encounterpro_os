...
-- Drop Procedure [dbo].[sp_Set_Encounter_Progress]
Print 'Drop Procedure [dbo].[sp_Set_Encounter_Progress]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_Set_Encounter_Progress]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_Set_Encounter_Progress]
GO

-- Create Procedure [dbo].[sp_Set_Encounter_Progress]
Print 'Create Procedure [dbo].[sp_Set_Encounter_Progress]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_Set_Encounter_Progress]
	TO [cprsystem]
GO

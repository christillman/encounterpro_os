...
-- Drop Procedure [dbo].[sp_get_package_id_record]
Print 'Drop Procedure [dbo].[sp_get_package_id_record]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_package_id_record]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_package_id_record]
GO

-- Create Procedure [dbo].[sp_get_package_id_record]
Print 'Create Procedure [dbo].[sp_get_package_id_record]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_package_id_record]
	TO [cprsystem]
GO

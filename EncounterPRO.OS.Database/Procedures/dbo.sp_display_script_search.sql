...
-- Drop Procedure [dbo].[sp_display_script_search]
Print 'Drop Procedure [dbo].[sp_display_script_search]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_display_script_search]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_display_script_search]
GO

-- Create Procedure [dbo].[sp_display_script_search]
Print 'Create Procedure [dbo].[sp_display_script_search]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_display_script_search]
	TO [cprsystem]
GO

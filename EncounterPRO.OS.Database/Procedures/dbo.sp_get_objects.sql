...
Print 'Drop Procedure [dbo].[sp_get_objects]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_objects]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_objects]
GO

-- Create Procedure [dbo].[sp_get_objects]
Print 'Create Procedure [dbo].[sp_get_objects]'
GO
SET ANSI_NULLS ON

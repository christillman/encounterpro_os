...
-- Drop Procedure [dbo].[sp_get_encounter_assessments]
Print 'Drop Procedure [dbo].[sp_get_encounter_assessments]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_get_encounter_assessments]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_get_encounter_assessments]
GO

-- Create Procedure [dbo].[sp_get_encounter_assessments]
Print 'Create Procedure [dbo].[sp_get_encounter_assessments]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_get_encounter_assessments]
	TO [cprsystem]
GO

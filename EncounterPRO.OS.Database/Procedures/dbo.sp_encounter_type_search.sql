...
-- Drop Procedure [dbo].[sp_encounter_type_search]
Print 'Drop Procedure [dbo].[sp_encounter_type_search]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[sp_encounter_type_search]') AND [type]='P'))
DROP PROCEDURE [dbo].[sp_encounter_type_search]
GO

-- Create Procedure [dbo].[sp_encounter_type_search]
Print 'Create Procedure [dbo].[sp_encounter_type_search]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[sp_encounter_type_search]
	TO [cprsystem]
GO

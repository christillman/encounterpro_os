...
-- Drop Function [dbo].[fn_is_unit_convertible]
Print 'Drop Function [dbo].[fn_is_unit_convertible]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_is_unit_convertible]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_is_unit_convertible]
GO

-- Create Function [dbo].[fn_is_unit_convertible]
Print 'Create Function [dbo].[fn_is_unit_convertible]'
GO
SET ANSI_NULLS ON

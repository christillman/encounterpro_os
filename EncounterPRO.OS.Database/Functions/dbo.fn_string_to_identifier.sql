...
-- Drop Function [dbo].[fn_string_to_identifier]
Print 'Drop Function [dbo].[fn_string_to_identifier]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_string_to_identifier]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_string_to_identifier]
GO

-- Create Function [dbo].[fn_string_to_identifier]
Print 'Create Function [dbo].[fn_string_to_identifier]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[fn_string_to_identifier]
	TO [cprsystem]
GO

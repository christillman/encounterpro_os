...
-- Drop Function [dbo].[fn_latest_system_version]
Print 'Drop Function [dbo].[fn_latest_system_version]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_latest_system_version]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_latest_system_version]
GO

-- Create Function [dbo].[fn_latest_system_version]
Print 'Create Function [dbo].[fn_latest_system_version]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[fn_latest_system_version]
	TO [cprsystem]
GO

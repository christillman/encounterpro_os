...
-- Drop Function [dbo].[fn_default_past_vaccine_date]
Print 'Drop Function [dbo].[fn_default_past_vaccine_date]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_default_past_vaccine_date]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_default_past_vaccine_date]
GO

-- Create Function [dbo].[fn_default_past_vaccine_date]
Print 'Create Function [dbo].[fn_default_past_vaccine_date]'
GO
SET ANSI_NULLS ON
...
GO
GRANT EXECUTE
	ON [dbo].[fn_default_past_vaccine_date]
	TO [cprsystem]
GO

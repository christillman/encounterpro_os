...
-- Drop Function [dbo].[fn_xml_epro_domain_ids]
Print 'Drop Function [dbo].[fn_xml_epro_domain_ids]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_xml_epro_domain_ids]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_xml_epro_domain_ids]
GO

-- Create Function [dbo].[fn_xml_epro_domain_ids]
Print 'Create Function [dbo].[fn_xml_epro_domain_ids]'
GO
SET ANSI_NULLS ON

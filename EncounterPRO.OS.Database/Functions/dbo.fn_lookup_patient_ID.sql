...
-- Drop Function [dbo].[fn_lookup_patient_ID]
Print 'Drop Function [dbo].[fn_lookup_patient_ID]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[fn_lookup_patient_ID]') AND ([type]='IF' OR [type]='FN' OR [type]='TF')))
DROP FUNCTION [dbo].[fn_lookup_patient_ID]
GO

-- Create Function [dbo].[fn_lookup_patient_ID]
Print 'Create Function [dbo].[fn_lookup_patient_ID]'
GO
SET ANSI_NULLS ON

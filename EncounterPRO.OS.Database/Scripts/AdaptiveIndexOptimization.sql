...
'AND i.object_id = ' + CAST(@objectID AS NVARCHAR(10)) + ' AND i.index_id = ' + CAST(@indexID AS NVARCHAR(10)) + ';'
		...
		BEGIN
			...
			END
			IF (@printCmds = 1 OR @debugMode = 1)					
				RAISERROR(' No remaining statistics to update...', 0, 42) WITH NOWAIT; 
		END
		ELSE
		BEGIN
			IF (@printCmds = 1 OR @debugMode = 1)
...
CREATE VIEW dbo.vw_ErrLst30Days
AS
...
CREATE VIEW dbo.vw_ErrLst24Hrs
AS
SELECT TOP 100 PERCENT dbName, objectName, indexName, partitionNumber, NULL AS statsName, dateTimeStart, dateTimeEnd, sqlStatement, errorMessage		
FROM dbo.tbl_AdaptiveIndexDefrag_log
...
CREATE VIEW dbo.vw_AvgTimeLst30Days
AS
...
CREATE VIEW dbo.vw_AvgFragLst30Days
AS
...
CREATE VIEW dbo.vw_AvgSamplingLst30Days
AS
...
CREATE VIEW dbo.vw_AvgLargestLst30Days
AS
...
CREATE VIEW dbo.vw_AvgMostUsedLst30Days
AS
...
CREATE VIEW dbo.vw_LastRun_Log
AS
...
CREATE PROCEDURE dbo.usp_AdaptiveIndexDefrag_PurgeLogs @daystokeep smallint = 90
AS
/*
...
CREATE PROCEDURE dbo.usp_AdaptiveIndexDefrag_CurrentExecStats @dbname NVARCHAR(255) = NULL
AS
/*

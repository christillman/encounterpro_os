...
-- Drop View [dbo].[v_External_Source_Component_Attribute]
Print 'Drop View [dbo].[v_External_Source_Component_Attribute]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[v_External_Source_Component_Attribute]') AND [type]='V'))
DROP VIEW [dbo].[v_External_Source_Component_Attribute]
GO
-- Create View [dbo].[v_External_Source_Component_Attribute]
Print 'Create View [dbo].[v_External_Source_Component_Attribute]'
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
CREATE VIEW v_External_Source_Component_Attribute (external_source, 
								param_mode,
								attribute_sequence, 
								attribute, 
								value, 
...
FROM c_External_Source r
	INNER JOIN c_Component_Registry c
	ON r.component_id = c.component_id
	INNER JOIN c_External_Source_Attribute a
	ON r.external_source = a.external_source
...
GO
GRANT SELECT ON [dbo].[v_External_Source_Component_Attribute] TO [cprsystem]
GO

...
-- Drop Table [dbo].[x_Translation_Rule]
Print 'Drop Table [dbo].[x_Translation_Rule]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[x_Translation_Rule]') AND [type]='U'))
DROP TABLE [dbo].[x_Translation_Rule]
GO
-- Create Table [dbo].[x_Translation_Rule]
Print 'Create Table [dbo].[x_Translation_Rule]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[x_Translation_Rule]
	ADD
	CONSTRAINT [PK_x_Translation_Rule_40]
	PRIMARY KEY
	NONCLUSTERED
	([external_application_id], [integration_operation], [epro_source_flag])
	WITH FILLFACTOR=80
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[x_Translation_Rule]
...
ALTER TABLE [dbo].[x_Translation_Rule]
	ADD
	CONSTRAINT [DF_x_Translation_Rule_id_40]

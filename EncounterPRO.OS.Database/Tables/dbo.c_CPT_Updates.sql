...
-- Drop Table [dbo].[c_CPT_Updates]
Print 'Drop Table [dbo].[c_CPT_Updates]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_CPT_Updates]') AND [type]='U'))
DROP TABLE [dbo].[c_CPT_Updates]
GO
-- Create Table [dbo].[c_CPT_Updates]
Print 'Create Table [dbo].[c_CPT_Updates]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_CPT_Updates]
	ADD
	CONSTRAINT [PK_c_CPT_Updates_128]
	PRIMARY KEY
	CLUSTERED
	([cpt_code], [cpt_sequence])
	WITH FILLFACTOR=90
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[c_CPT_Updates]
	ADD
	CONSTRAINT [DF_c_CPT_Updates_operation]
	DEFAULT ('New') FOR [operation]
GO
ALTER TABLE [dbo].[c_CPT_Updates] SET (LOCK_ESCALATION = TABLE)
GO

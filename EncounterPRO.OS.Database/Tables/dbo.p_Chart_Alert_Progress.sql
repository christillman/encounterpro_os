...
-- Drop Table [dbo].[p_Chart_Alert_Progress]
Print 'Drop Table [dbo].[p_Chart_Alert_Progress]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[p_Chart_Alert_Progress]') AND [type]='U'))
DROP TABLE [dbo].[p_Chart_Alert_Progress]
GO
-- Create Table [dbo].[p_Chart_Alert_Progress]
Print 'Create Table [dbo].[p_Chart_Alert_Progress]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[p_Chart_Alert_Progress]
	ADD
	CONSTRAINT [PK_p_Chart_Alert_Progress]
	PRIMARY KEY
	CLUSTERED
	([cpr_id], [alert_id], [alert_progress_sequence])
	WITH FILLFACTOR=80
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[p_Chart_Alert_Progress]
	ADD
	CONSTRAINT [DF__p_Chart_Aler__id__21C0F255]

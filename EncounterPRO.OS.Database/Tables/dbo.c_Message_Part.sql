...
-- Drop Table [dbo].[c_Message_Part]
Print 'Drop Table [dbo].[c_Message_Part]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Message_Part]') AND [type]='U'))
DROP TABLE [dbo].[c_Message_Part]
GO
-- Create Table [dbo].[c_Message_Part]
Print 'Create Table [dbo].[c_Message_Part]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Message_Part]
	ADD
	CONSTRAINT [PK_c_Message_Part_1__16]
	PRIMARY KEY
	CLUSTERED
	([message_type], [message_part_sequence])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO

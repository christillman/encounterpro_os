...
-- Drop Table [dbo].[c_User_Progress]
Print 'Drop Table [dbo].[c_User_Progress]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_User_Progress]') AND [type]='U'))
DROP TABLE [dbo].[c_User_Progress]
GO
-- Create Table [dbo].[c_User_Progress]
Print 'Create Table [dbo].[c_User_Progress]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_User_Progress]
	ADD
	CONSTRAINT [PK_c_Users_prog_40]
	PRIMARY KEY
	CLUSTERED
	([user_id], [user_progress_sequence])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[c_User_Progress]
...
ALTER TABLE [dbo].[c_User_Progress]
	ADD
	CONSTRAINT [DF_c_User_prog_current_flag_50]

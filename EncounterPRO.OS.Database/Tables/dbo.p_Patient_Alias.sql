...
-- Drop Table [dbo].[p_Patient_Alias]
Print 'Drop Table [dbo].[p_Patient_Alias]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[p_Patient_Alias]') AND [type]='U'))
DROP TABLE [dbo].[p_Patient_Alias]
GO
-- Create Table [dbo].[p_Patient_Alias]
Print 'Create Table [dbo].[p_Patient_Alias]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[p_Patient_Alias]
	ADD
	CONSTRAINT [PK_p_Patient_Alias]
	PRIMARY KEY
	NONCLUSTERED
	([cpr_id], [alias_type], [alias_sequence])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[p_Patient_Alias]
...
ALTER TABLE [dbo].[p_Patient_Alias]
	ADD
	CONSTRAINT [DF_p_Patient_Alias_id]
	DEFAULT (newid()) FOR [id]
GO
ALTER TABLE [dbo].[p_Patient_Alias]
	ADD
	CONSTRAINT [DF_p_Patient_Alias_status]
	DEFAULT ('Y') FOR [current_flag]
GO
ALTER TABLE [dbo].[p_Patient_Alias]
	ADD
	CONSTRAINT [DF_p_Patient_Alias_type]
	DEFAULT ('Primary') FOR [alias_type]
GO
CREATE NONCLUSTERED INDEX [idx_first_name]
	ON [dbo].[p_Patient_Alias] ([first_name])

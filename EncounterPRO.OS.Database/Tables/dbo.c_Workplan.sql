...
-- Drop Table [dbo].[c_Workplan]
Print 'Drop Table [dbo].[c_Workplan]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Workplan]') AND [type]='U'))
DROP TABLE [dbo].[c_Workplan]
GO
-- Create Table [dbo].[c_Workplan]
Print 'Create Table [dbo].[c_Workplan]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [PK_c_Workplan_1__10]
	PRIMARY KEY
	CLUSTERED
	([workplan_id])
	WITH FILLFACTOR=90
...
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [DF__c_Workpla__last___59DFF250]
	DEFAULT (dbo.get_client_datetime()) FOR [last_updated]
GO
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [DF__c_Workpla__owner__4A9DAEC0]
	DEFAULT ((-1)) FOR [owner_id]
GO
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [DF__c_Workpla__statu__6DE6EAFD]
	DEFAULT ('OK') FOR [status]
GO
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [DF__c_Workplan__id__6739ED6E]
	DEFAULT (newid()) FOR [id]
GO
ALTER TABLE [dbo].[c_Workplan]
	ADD
	CONSTRAINT [DF_c_Workplan_workplan_type]
	DEFAULT ('Patient') FOR [workplan_type]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_c_Workplan_id]

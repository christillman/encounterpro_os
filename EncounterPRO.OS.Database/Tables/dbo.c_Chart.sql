...
-- Drop Table [dbo].[c_Chart]
Print 'Drop Table [dbo].[c_Chart]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Chart]') AND [type]='U'))
DROP TABLE [dbo].[c_Chart]
GO
-- Create Table [dbo].[c_Chart]
Print 'Create Table [dbo].[c_Chart]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Chart]
	ADD
	CONSTRAINT [DF__c_Chart__id__6A165A19]
	DEFAULT (newid()) FOR [id]
GO
ALTER TABLE [dbo].[c_Chart]
	ADD
	CONSTRAINT [DF__c_Chart__last_up__608CEFDF]
	DEFAULT (dbo.get_client_datetime()) FOR [last_updated]
GO
ALTER TABLE [dbo].[c_Chart]
	ADD
	CONSTRAINT [DF__c_Chart__owner_i__5332F4C1]
	DEFAULT ((-1)) FOR [owner_id]
GO
ALTER TABLE [dbo].[c_Chart]
	ADD
	CONSTRAINT [DF__c_Chart__status__6EDB0F36]
	DEFAULT ('OK') FOR [status]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_c_Chart_id]

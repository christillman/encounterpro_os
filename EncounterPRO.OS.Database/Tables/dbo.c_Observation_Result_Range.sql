...
-- Drop Table [dbo].[c_Observation_Result_Range]
Print 'Drop Table [dbo].[c_Observation_Result_Range]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Observation_Result_Range]') AND [type]='U'))
DROP TABLE [dbo].[c_Observation_Result_Range]
GO
-- Create Table [dbo].[c_Observation_Result_Range]
Print 'Create Table [dbo].[c_Observation_Result_Range]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Observation_Result_Range]
	ADD
	CONSTRAINT [PK_c_Observation_Result_Range]
	PRIMARY KEY
	CLUSTERED
	([observation_id], [result_sequence], [result_range_sequence])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO

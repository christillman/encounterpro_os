...
-- Drop Table [dbo].[o_Message_Subscription]
Print 'Drop Table [dbo].[o_Message_Subscription]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[o_Message_Subscription]') AND [type]='U'))
DROP TABLE [dbo].[o_Message_Subscription]
GO
-- Create Table [dbo].[o_Message_Subscription]
Print 'Create Table [dbo].[o_Message_Subscription]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[o_Message_Subscription]
	ADD
	CONSTRAINT [PK_o_Message_Subscription1__13]
	PRIMARY KEY
	CLUSTERED
	([subscription_id])
	WITH FILLFACTOR=100

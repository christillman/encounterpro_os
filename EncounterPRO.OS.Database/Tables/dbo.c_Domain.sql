...
-- Drop Table [dbo].[c_Domain]
Print 'Drop Table [dbo].[c_Domain]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Domain]') AND [type]='U'))
DROP TABLE [dbo].[c_Domain]
GO
-- Create Table [dbo].[c_Domain]
Print 'Create Table [dbo].[c_Domain]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Domain]
	ADD
	CONSTRAINT [PK_c_Domain_1__10]
	PRIMARY KEY
	CLUSTERED
	([domain_id], [domain_sequence])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[c_Domain]
	ADD
	CONSTRAINT [DF__c_Domain_id]

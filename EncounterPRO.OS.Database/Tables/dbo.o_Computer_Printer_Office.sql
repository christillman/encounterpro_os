...
-- Drop Table [dbo].[o_Computer_Printer_Office]
Print 'Drop Table [dbo].[o_Computer_Printer_Office]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[o_Computer_Printer_Office]') AND [type]='U'))
DROP TABLE [dbo].[o_Computer_Printer_Office]
GO
-- Create Table [dbo].[o_Computer_Printer_Office]
Print 'Create Table [dbo].[o_Computer_Printer_Office]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[o_Computer_Printer_Office]
	ADD
	CONSTRAINT [PK_o_Computer_Printer_Office_40]
	PRIMARY KEY
	CLUSTERED
	([computer_id], [printer], [office_id])
	WITH FILLFACTOR=70
	ON [PRIMARY]
GO

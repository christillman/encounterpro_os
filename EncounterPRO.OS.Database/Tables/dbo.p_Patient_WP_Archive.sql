...
-- Drop Table [dbo].[p_Patient_WP_Archive]
Print 'Drop Table [dbo].[p_Patient_WP_Archive]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[p_Patient_WP_Archive]') AND [type]='U'))
DROP TABLE [dbo].[p_Patient_WP_Archive]
GO
-- Create Table [dbo].[p_Patient_WP_Archive]
Print 'Create Table [dbo].[p_Patient_WP_Archive]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[p_Patient_WP_Archive]
	ADD
	CONSTRAINT [PK_p_Patient_Workplan_Archive]
	PRIMARY KEY
	NONCLUSTERED
	([patient_workplan_id])
	WITH FILLFACTOR=100

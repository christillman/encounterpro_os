...
-- Drop Table [dbo].[em_Observation_Element]
Print 'Drop Table [dbo].[em_Observation_Element]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[em_Observation_Element]') AND [type]='U'))
DROP TABLE [dbo].[em_Observation_Element]
GO
-- Create Table [dbo].[em_Observation_Element]
Print 'Create Table [dbo].[em_Observation_Element]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[em_Observation_Element]
	ADD
	CONSTRAINT [PK_em_Observation_Element]
	PRIMARY KEY
	CLUSTERED
	([em_component], [em_type], [em_category], [em_element], [observation_id])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO

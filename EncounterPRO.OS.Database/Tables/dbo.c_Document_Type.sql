...
-- Drop Table [dbo].[c_Document_Type]
Print 'Drop Table [dbo].[c_Document_Type]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Document_Type]') AND [type]='U'))
DROP TABLE [dbo].[c_Document_Type]
GO
-- Create Table [dbo].[c_Document_Type]
Print 'Create Table [dbo].[c_Document_Type]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Document_Type]
	ADD
	CONSTRAINT [PK_c_Document_Type_40]
	PRIMARY KEY
	CLUSTERED
	([document_type])
	WITH FILLFACTOR=70
...
ALTER TABLE [dbo].[c_Document_Type]
	ADD
	CONSTRAINT [DF__c_Document_Type_id]
	DEFAULT (newid()) FOR [id]
GO
ALTER TABLE [dbo].[c_Document_Type]
	ADD
	CONSTRAINT [DF__c_Document_Type_last_updated]
	DEFAULT (dbo.get_client_datetime()) FOR [last_updated]
GO
ALTER TABLE [dbo].[c_Document_Type]
	ADD
	CONSTRAINT [DF__c_Document_Type_owner_id]

...
-- Drop Table [dbo].[c_Preferred_Provider]
Print 'Drop Table [dbo].[c_Preferred_Provider]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Preferred_Provider]') AND [type]='U'))
DROP TABLE [dbo].[c_Preferred_Provider]
GO
-- Create Table [dbo].[c_Preferred_Provider]
Print 'Create Table [dbo].[c_Preferred_Provider]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Preferred_Provider]
	ADD
	CONSTRAINT [PK_c_Preferred_Provider]
	PRIMARY KEY
	CLUSTERED
	([consultant_id], [authority_id])
	WITH FILLFACTOR=100
	ON [PRIMARY]
GO

...
-- Drop Table [dbo].[c_Drug_Instruction]
Print 'Drop Table [dbo].[c_Drug_Instruction]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Drug_Instruction]') AND [type]='U'))
DROP TABLE [dbo].[c_Drug_Instruction]
GO
-- Create Table [dbo].[c_Drug_Instruction]
Print 'Create Table [dbo].[c_Drug_Instruction]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Drug_Instruction]
	ADD
	CONSTRAINT [PK_c_Drug_Instruction_1__10]
	PRIMARY KEY
	CLUSTERED
	([instruction_id])
	WITH FILLFACTOR=100

...
-- Drop Table [dbo].[o_Active_Services]
Print 'Drop Table [dbo].[o_Active_Services]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[o_Active_Services]') AND [type]='U'))
DROP TABLE [dbo].[o_Active_Services]
GO
-- Create Table [dbo].[o_Active_Services]
Print 'Create Table [dbo].[o_Active_Services]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[o_Active_Services]
	ADD
	CONSTRAINT [PK_o_Active_Services]
	PRIMARY KEY
	NONCLUSTERED
...
CREATE CLUSTERED INDEX [idx_Active_Services_cluster]
	ON [dbo].[o_Active_Services] ([in_office_flag], [patient_workplan_item_id])

...
-- Drop Table [dbo].[c_Config_Object_Version]
Print 'Drop Table [dbo].[c_Config_Object_Version]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Config_Object_Version]') AND [type]='U'))
DROP TABLE [dbo].[c_Config_Object_Version]
GO
-- Create Table [dbo].[c_Config_Object_Version]
Print 'Create Table [dbo].[c_Config_Object_Version]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Config_Object_Version]
	ADD
	CONSTRAINT [PK_c_Config_Object_Version]
	PRIMARY KEY
	CLUSTERED
	([config_object_id], [version])
	WITH FILLFACTOR=70
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[c_Config_Object_Version]
	ADD
	CONSTRAINT [DF_c_Config_Object_Version_created]
	DEFAULT (dbo.get_client_datetime()) FOR [created]
GO
ALTER TABLE [dbo].[c_Config_Object_Version]
	ADD
	CONSTRAINT [DF_c_Config_Object_Version_owner_id]
	DEFAULT ([dbo].[fn_customer_id]()) FOR [owner_id]
GO
ALTER TABLE [dbo].[c_Config_Object_Version]
	ADD
	CONSTRAINT [DF_c_Config_Object_Version_rel_status]
	DEFAULT ('Production') FOR [release_status]
GO
ALTER TABLE [dbo].[c_Config_Object_Version]
	ADD
	CONSTRAINT [DF_c_Config_Object_Version_status]

...
-- Drop Table [dbo].[o_Service_Schedule]
Print 'Drop Table [dbo].[o_Service_Schedule]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[o_Service_Schedule]') AND [type]='U'))
DROP TABLE [dbo].[o_Service_Schedule]
GO
-- Create Table [dbo].[o_Service_Schedule]
Print 'Create Table [dbo].[o_Service_Schedule]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[o_Service_Schedule]
	ADD
	CONSTRAINT [PK_o_Service_Schedule_40]
	PRIMARY KEY
	CLUSTERED
	([user_id], [service_sequence])
	WITH FILLFACTOR=80
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[o_Service_Schedule]
...
ALTER TABLE [dbo].[o_Service_Schedule]
	ADD
	CONSTRAINT [DF_o_Service_Schedule_id]

...
-- Drop Table [dbo].[c_Maintenance_Policy]
Print 'Drop Table [dbo].[c_Maintenance_Policy]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[c_Maintenance_Policy]') AND [type]='U'))
DROP TABLE [dbo].[c_Maintenance_Policy]
GO
-- Create Table [dbo].[c_Maintenance_Policy]
Print 'Create Table [dbo].[c_Maintenance_Policy]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[c_Maintenance_Policy]
	ADD
	CONSTRAINT [PK_c_Maintenance_Policy]
	PRIMARY KEY
	CLUSTERED
	([maintenance_rule_id], [policy_sequence])
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[c_Maintenance_Policy]
	ADD
	CONSTRAINT [DF_c_mt_Policy_created]

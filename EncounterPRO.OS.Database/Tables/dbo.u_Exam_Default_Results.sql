...
-- Drop Table [dbo].[u_Exam_Default_Results]
Print 'Drop Table [dbo].[u_Exam_Default_Results]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[u_Exam_Default_Results]') AND [type]='U'))
DROP TABLE [dbo].[u_Exam_Default_Results]
GO
-- Create Table [dbo].[u_Exam_Default_Results]
Print 'Create Table [dbo].[u_Exam_Default_Results]'
GO
SET ANSI_NULLS OFF
...
ALTER TABLE [dbo].[u_Exam_Default_Results]
	ADD
	CONSTRAINT [PK_exam_def_res_40]
	PRIMARY KEY
	CLUSTERED
	([exam_sequence], [branch_id], [result_sequence], [location], [user_id])
	WITH FILLFACTOR=80
	ON [PRIMARY]
GO
ALTER TABLE [dbo].[u_Exam_Default_Results]
	ADD
	CONSTRAINT [DF__u_Exam_De__resul__544C7222]
	DEFAULT ('Y') FOR [result_flag]
GO
ALTER TABLE [dbo].[u_Exam_Default_Results]
	ADD

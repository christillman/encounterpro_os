...
-- Drop Table [dbo].[o_User_Logins]
Print 'Drop Table [dbo].[o_User_Logins]'
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[o_User_Logins]') AND [type]='U'))
DROP TABLE [dbo].[o_User_Logins]
GO
-- Create Table [dbo].[o_User_Logins]
Print 'Create Table [dbo].[o_User_Logins]'
GO
SET ANSI_NULLS ON
...
ALTER TABLE [dbo].[o_User_Logins]
	ADD
	CONSTRAINT [PK_o_user_logins_40]
	PRIMARY KEY
	CLUSTERED
	([user_id], [action_id])
	WITH FILLFACTOR=50
...
ALTER TABLE [dbo].[o_User_Logins]
	ADD
	CONSTRAINT [DF__o_user_logins_action_status]
	DEFAULT ('Success') FOR [action_status]
GO
ALTER TABLE [dbo].[o_User_Logins]
	ADD

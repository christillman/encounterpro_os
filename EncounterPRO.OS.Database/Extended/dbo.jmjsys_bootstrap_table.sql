DROP PROCEDURE [jmjsys_bootstrap_table]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO
CREATE PROCEDURE [jmjsys_bootstrap_table] (
...
if @pl_modification_level > 100
	BEGIN
	SET @ll_modification_level = @pl_modification_level
	END
ELSE
	BEGIN
	if exists (select * from dbo.sysobjects where id = object_id('dbo.c_Database_Status') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
		SELECT @ll_modification_level = modification_level
		FROM c_Database_Status
	ELSE
		SELECT @ll_modification_level = max(modification_level)
		FROM jmjtech.eproupdates.dbo.Epro_Modification_Level
...
AND modification_level <= @ll_modification_level

...
-- Drop Trigger [dbo].[tr_Patient_WP_Item_Delete]
Print 'Drop Trigger [dbo].[tr_Patient_WP_Item_Delete]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[tr_Patient_WP_Item_Delete]') AND [type]='TR'))
DROP TRIGGER [dbo].[tr_Patient_WP_Item_Delete]
GO

-- Create Trigger [dbo].[tr_Patient_WP_Item_Delete]
Print 'Create Trigger [dbo].[tr_Patient_WP_Item_Delete]'
GO
SET ANSI_NULLS ON

...
-- Drop Trigger [dbo].[tr_o_preferences_script]
Print 'Drop Trigger [dbo].[tr_o_preferences_script]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[tr_o_preferences_script]') AND [type]='TR'))
DROP TRIGGER [dbo].[tr_o_preferences_script]
GO

-- Create Trigger [dbo].[tr_o_preferences_script]
Print 'Create Trigger [dbo].[tr_o_preferences_script]'
GO
SET ANSI_NULLS ON

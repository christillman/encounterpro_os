...
-- Drop Trigger [dbo].[tr_c_config_object_insert_update]
Print 'Drop Trigger [dbo].[tr_c_config_object_insert_update]'
GO
IF (EXISTS(SELECT * FROM sys.objects WHERE [object_id] = OBJECT_ID(N'[dbo].[tr_c_config_object_insert_update]') AND [type]='TR'))
DROP TRIGGER [dbo].[tr_c_config_object_insert_update]
GO

-- Create Trigger [dbo].[tr_c_config_object_insert_update]
Print 'Create Trigger [dbo].[tr_c_config_object_insert_update]'
GO
SET ANSI_NULLS ON
...
/*
	...
	FROM dbo.c_config_object c
		INNER JOIN inserted i
		ON c.config_object_id = i.config_object_id
		INNER JOIN (
			SELECT id, max(menu_id) as menu_id
			FROM c_Menu
			GROUP BY id) x

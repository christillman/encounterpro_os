HA$PBExportHeader$w_report_configure_and_test.srw
forward
...
if isnull(report_document) then
	log.log(this, "xx_printreport()", "error getting component (" + report.report_id + ")", 4)
	return -1
end if
...
If Isnull(luo_report) Then
	log.log(This, "run_rtf_report()", "Error getting report component (" + ls_component_id + ")", 4)
	Return -1
End If
...
if li_sts < 0 then
	log.log(this, "xx_printreport()", "Error configuring document component (" + report.report_id + ")", 4)
	return -1
end if
...
if isnull(ls_report_id) then
	log.log(this, "initialize_from_document()", "Null report_id", 4)
	Return -1
end if
...
If Isnull(ls_component_id) Then
	log.log(this, "initialize_from_document()", "Null component_id (" + ls_report_id + ")", 4)
	Return -1
end if

report = component_manager.get_component(ls_component_id, lstr_attributes)
If Isnull(report) Then
	log.log(This, "initialize_from_document()", "Error getting report component (" + &
				ls_component_id + ")", 4)
	Return -1
End If
...
CHOOSE CASE lower(ls_classname)
	...
	CASE "u_component_wp_item_document"
		...
		if li_sts < 0 then
			log.log(this, "open", "Error initializing from document", 4)
			close(this)
			return
		end if
	...
	CASE ELSE
		log.log(this, "open", "Invalid caller class (" + ls_classname + ")", 4)
		close(this)
		return
END CHOOSE

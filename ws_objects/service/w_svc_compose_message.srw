HA$PBExportHeader$w_svc_compose_message.srw
forward
...
//				log.log(this, "open", "Null observation_sequence", 4)
...
//			message_attributes.attribute[2].value = string(service.observation_sequence)
		CASE "treatment"
			if isnull(service.treatment) then
				log.log(this, "open", "Null treatment_id", 4)
				closewithreturn(this, popup_return)
				return
			end if
		...
		CASE ELSE
			log.log(this, "open", "Invalid message_object (" + message_object + ")", 4)
			closewithreturn(this, popup_return)
			return
	END CHOOSE

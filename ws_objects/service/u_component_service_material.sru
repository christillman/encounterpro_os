HA$PBExportHeader$u_component_service_material.sru
forward
	...
	CASE "provider reference", "patient reference"
		...
		if isnull(ll_material_id) then
			if cpr_mode = "CLIENT" then
				...
				log.log(this, "xx_do_service()", "reference material_id could not be determined", 4)
				return -1
			end if
		end if
	CASE ELSE
...
if isnull(ll_material_id) then
	log.log(this, "xx_do_service()", "material_id could not be determined", 4)
	return -1
end if
...
CHOOSE CASE lower(ls_action)
	CASE "open"
		...
		if li_sts <= 0 then
			log.log(this, "xx_do_service()", "Error opening patient material", 4)
			return -1
		end if
	...
	CASE "print"
		...
		if li_sts <= 0 then
			log.log(this, "xx_do_service()", "Error printing patient material", 4)

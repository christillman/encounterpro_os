HA$PBExportHeader$u_tabpage_allergy_injection.sru
forward
...
if isnull(ll_treatment_id) then
	log.log(this, "initialize()", "Null treatment_id", 4)
	return -1
end if

li_sts = current_patient.treatments.treatment(treatment, ll_treatment_id)
if li_sts <= 0 then
	log.log(this, "initialize()", "Error gettting treatment (" + string(ll_treatment_id) + ")", 4)
	return -1
end if
...
if first_time then
	...
	if isnull(dose_unit) then
		...
		if isnull(dose_unit) then
			log.log(this, "refresh()", "Error - Unable to find the unit object", 4)
			return
		end if
	end if
...
if isnull(ls_last_location) then
	...
	if not tf_check() then return
	if isnull(ls_last_location) then
		log.log(this, "refresh()", "Error - Unable to find a valid location (" + location_domain + ")", 4)
		return
	end if
end if

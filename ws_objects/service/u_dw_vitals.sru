HA$PBExportHeader$u_dw_vitals.sru
forward
...
if isnull(ll_observation_sequence) then
	...
	if ll_observation_sequence < 0 then
		log.log(this, "observation_sequence()", "Error adding observation", 4)
		return -1
	end if
...
if ls_location = "" then
	...
	if ll_observation_sequence < 0 then
		log.log(this, "get_collection_location()", "Error adding observation", 4)
		return -1
	end if
...
if lstr_tree.branch_count = 0 then
	log.log(this, "initialize()", "No children found (" + parent_observation_id + ")", 4)
	return -1
end if
...
for i = 1 to lstr_tree.branch_count
	...
	if not isnull(li_result_sequence) then
		...
		if not isnull(lstr_result_1.result_unit) then
			...
			if isnull(luo_unit) then
				log.log(this, "initialize()", "result unit not found (" + lstr_result_1.result_unit + ")", 3)
			else
	...
	if not isnull(li_result_sequence_2) then
		...
		if not isnull(lstr_result_2.result_unit) then
			...
			if isnull(luo_unit) then
				log.log(this, "initialize()", "result unit not found (" + lstr_result_2.result_unit + ")", 3)
			else
				ls_display_unit = luo_unit.pretty_unit(ls_unit_preference, ls_display_unit_id)
				object.unit[ll_row] = ls_display_unit
		...
		for k = 1 to puo_external_observation.observations[i].result[j].id_count
			...
			DO WHILE ll_row > 0 and ll_row <= ll_rows
				...
				if ls_manual_override = "N" then
					...
					if ll_observation_sequence < 0 then
						log.log(this, "results_posted()", "Error adding observation", 4)
						return -1
					end if
...
if not isnull(ls_collection_location_domain) and ls_collection_location_domain <> "NA" and ps_which = "result" then
	if isnull(ps_display_value) or trim(ps_display_value) = "" then
		...
		if li_sts < 0 then
			log.log(this, "update_row()", "Error getting collection location", 4)
			return -1
		end if
	end if
...
if ls_result_amount_flag = "Y" then
	...
	if isnull(ls_display_unit_id) then
		log.log(this, "refresh_row()", "Null display unit (" + ls_observation_id + ")", 4)
		return -1
	end if
	
	luo_display_unit = unit_list.find_unit(ls_display_unit_id)
	if isnull(luo_display_unit) then
		log.log(this, "refresh_row()", "Invalid display unit (" + ls_display_unit_id + ")", 4)
		return -1
	end if
	...
	if ll_row <= 0 then
		...
		if isnull(ls_result_unit) then
			log.log(this, "refresh_row()", "Null result unit 1 (" + ls_observation_id + ")", 4)
			return -1
		end if
		
		luo_result_unit = unit_list.find_unit(ls_result_unit)
		if isnull(luo_result_unit) then
			log.log(this, "refresh_row()", "Invalid unit (" + ls_result_unit + ")", 4)
			return -1
		end if
	...
	if not isnull(li_result_sequence_2) then
		...
		if ll_row <= 0 then
			...
			// display amount
			if isnull(ls_result_unit) then
				log.log(this, "refresh_row()", "Null result unit 2 (" + ls_observation_id + ")", 4)
				return -1
			end if
...
ls_display_unit_id = object.display_unit_id[pl_row]
if isnull(ls_display_unit_id) then
	log.log(this, "clicked", "Null result unit id", 4)
	return -1
end if
...
if isnull(luo_new_value_unit) then
	log.log(this, "clicked", "Error finding new value unit (" + ps_new_value_unit + ")", 4)
	return -1
end if

luo_display_unit = unit_list.find_unit(ls_display_unit_id)
if isnull(luo_display_unit) then
	log.log(this, "clicked", "Error finding new value unit (" + ls_display_unit_id + ")", 4)
	return -1
end if
...
if dwo.name = "more" then
	...
	if not isnull(li_result_sequence_2) then
		...
		if isnull(luo_unit) then
			log.log(this, "clicked", "Error finding unit (" + ls_unit_id + ")", 4)
			return
		end if
...
elseif dwo.name = "observation" then
	...
	if isnull(ll_observation_sequence) then
		...
		ll_observation_sequence = treatment.add_observation(parent_observation_sequence, &
											...
											true)
		if ll_observation_sequence < 0 then
			log.log(this, "clicked", "Error adding observation", 4)
			return
		end if

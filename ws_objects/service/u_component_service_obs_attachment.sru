HA$PBExportHeader$u_component_service_obs_attachment.sru
forward
...
CHOOSE CASE lower(ls_attachment_object)
	...
	CASE ELSE
		mylog.log(this, "xx_do_service()", "Invalid attachment_object (" + ls_attachment_object + ")", 4)
		return -1
END CHOOSE

ls_observation_id = get_attribute("observation_id")
if isnull(ls_observation_id) then
	mylog.log(this, "xx_do_service", "No observation_id", 4)
	return -1
end if
...
if isnull(ls_comment_title) then
	...
	if sqlca.sqlcode = 100 then
		log.log(this, "xx_do_service()", "Material_id not found (" + string(ll_material_id) + ")", 4)
		return -1
	end if
...
if cprdb.sqlcode = 100 then
	mylog.log(this, "xx_do_service", "material_id not found (" + string(ll_material_id) + ")", 4)
	return -1
end if

...
Function ULong AssocQueryString ( &
	Integer flags, &
	Integer str, &
	String pszAssoc, &
	Longptr pszExtra, &
	Ref String pszOut, &
	Ref ULong pcchOut &
	) Library "shlwapi.dll" Alias For "AssocQueryStringW"
...
forward prototypes
public function boolean shellrun (string as_filename, string as_shellverb, long al_showcmd)
private function string getlasterrortext ()
public function boolean shellrunandwait (string as_filename, string as_shellverb, long al_showcmd)
public function boolean runandwait (string as_exefullpath, long al_showcmd)
public function boolean runandcapture (string as_exefullpath, ref string as_output)
public function string getexecutable (string as_extension)
end prototypes
...
public function string getexecutable (string as_extension);// -----------------------------------------------------------------------------
// SCRIPT:     GetExecutable
//
// PURPOSE:    This function determines the executable for a file extension.
//
// ARGUMENTS:  as_extension - File extension
//
// RETURN:     Full path of associated executable
//
// DATE        PROG/ID		DESCRIPTION OF CHANGE / REASON
// ----------  --------		-----------------------------------------------------
// 11/29/2022	RolandS		Initial Coding
// -----------------------------------------------------------------------------

Constant Integer ASSOCF_NONE = 0
Constant Integer ASSOCSTR_EXECUTABLE = 2
String ls_OutputPath, ls_Extension
ULong lul_Return, lul_OutputSize

lul_OutputSize = 260
ls_OutputPath = Space(lul_OutputSize)
ls_Extension = "." + as_extension

lul_return = AssocQueryString(ASSOCF_NONE, &
						ASSOCSTR_EXECUTABLE, ls_Extension, &
						NULL, ls_OutputPath, lul_OutputSize)

Return ls_OutputPath

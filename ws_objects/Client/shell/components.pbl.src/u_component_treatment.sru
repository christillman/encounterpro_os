...
if isnull(lstr_c_result.result_sequence) then
	log.log(this, "u_component_treatment.get_location_description:0015", "Error getting result (" + ls_observation_id + ", " + string(pi_result_sequence) + ")", 4)
	return -1
end if
...
if isnull(pl_observation_sequence) then
	log.log(this, "u_component_treatment.remove_comment:0013", "Null observation_sequence", 4)
	return -1
end if
...
ll_row = p_observation.find(ls_find, 1, ll_o_count)
if ll_row <= 0 then
	log.log(this, "u_component_treatment.get_observation_description:0015", "Observation_sequence not found (" + string(pl_observation_sequence) + ")", 4)
	return ls_observation_id
end if
...
for i = 1 to pstr_tree.branch_count
	...
	if ll_child_observation_sequence <= 0 then
		log.log(this, "u_component_treatment.set_exam_defaults:0031", "Error adding new observation", 4)
	...
	if ls_composite_flag = "Y" then
		...
		if li_sts < 0 then
			log.log(this, "u_component_treatment.set_exam_defaults:0041", "Error adding child defaults", 4)
			if lb_wait then close(w_pop_please_wait)
			return -1
		...
		if li_sts < 0 then
			log.log(this, "u_component_treatment.set_exam_defaults:0050", "Error setting default results", 4)
			if lb_wait then close(w_pop_please_wait)
			return -1
		elseif li_sts > 0 then
			li_found = 1
...
ll_row = p_observation.find(ls_find, 1, ll_o_count)
if ll_row <= 0 then
	log.log(this, "u_component_treatment.get_observation_service:0015", "Observation_sequence not found (" + string(pl_observation_sequence) + ")", 4)
	return ls_service
end if
...
if isnull(pl_observation_sequence) then
	log.log(this, "u_component_treatment.set_exam_defaults_old:0015", "Null root_observation_sequence", 4)
	return -1
end if

ls_root_observation_id = get_observation_id(pl_observation_sequence) 
if isnull(ls_root_observation_id) then
	log.log(this, "u_component_treatment.set_exam_defaults_old:0021", "No exam root (" + string(pl_observation_sequence) + ")", 4)
	return -1
end if
...
if isnull(pl_exam_sequence) or pl_exam_sequence <= 0 then
	log.log(this, "u_component_treatment.set_exam_defaults_old:0030", "Invalid exam_sequence", 4)
	return -1
end if
...
if isnull(ls_exam_root_observation_id) then
	log.log(this, "u_component_treatment.set_exam_defaults_old:0037", "exam_sequence not found (" + string(pl_exam_sequence) + ")", 4)
	return -1
end if
...
if pl_branch_id > 0 then
	...
	if isnull(lstr_branch.child_observation_id) then
		log.log(this, "u_component_treatment.set_exam_defaults_old:0046", "branch not found (" + string(pl_branch_id) + ")", 4)
		return -1
	end if
...
if isnull(pi_result_sequence) then
	log.log(this, "u_component_treatment.set_result:0040", "Null result sequence", 4)
	return -1
end if

if isnull(ps_location) then
	log.log(this, "u_component_treatment.set_result:0045", "Null location", 4)
	return -1
end if

ls_observation_id = get_observation_id(pl_observation_sequence)
if isnull(ls_observation_id) then
	log.log(this, "u_component_treatment.set_result:0051", "Observation Not Found (" + string(pl_observation_sequence) + ")", 4)
	return -1
end if

lstr_result = datalist.observation_result(ls_observation_id, pi_result_sequence)
if isnull(lstr_result.result_sequence) then
	log.log(this, "u_component_treatment.set_result:0057", "Observation Result Not Found (" + ls_observation_id + ", " + string(pi_result_sequence) + ")", 4)
	return -1
end if
	...
	CASE "PERFORM", "COLLECT"
		if lstr_result.result_amount_flag = "Y" and not isnull(lstr_result.result_unit) then
			...
			if isnull(luo_result_unit) then
				log.log(this, "u_component_treatment.set_result:0093", "Error finding unit (" + ls_result_unit + ")", 4)
				return -1
			end if
			...
			if isnull(luo_display_unit) then
				log.log(this, "u_component_treatment.set_result:0104", "Error finding display unit (" + ls_display_unit_id + ")", 4)
				return -1
			end if
		...
		li_sts = add_result(pl_observation_sequence, &
									...
									ls_abnormal_nature )
		if li_sts <= 0 then
			log.log(this, "u_component_treatment.set_result:0151", "Error adding result", 4)
			return -1
		end if
	CASE "COMMENT"
...
for i = 1 to ll_rowcount
	...
	elseif not isnull(ls_child_observation_id) then
		...
		if lla_observation_sequence[i] <= 0 then
			log.log(this, "u_component_treatment.set_exam_defaults_add_observations:0096", "Error adding new observation", 4)
			if lb_wait then close(w_pop_please_wait)
			return -1
		end if
	end if
		...
		if (isnull(pi_result_sequence) or (pi_result_sequence = li_result_sequence)) &
			and (isnull(ps_location) or (ls_location = ps_location)) then
				li_sts = add_result(lla_observation_sequence[i], &
											...
											ls_null )
				if li_sts < 0 then
					log.log(this, "u_component_treatment.set_exam_defaults_add_observations:0116", "Error adding default results", 4)
					if lb_wait then close(w_pop_please_wait)
					return -1
				end if
				li_found += 1
...
if isnull(pl_parent_observation_sequence) then
	log.log(this, "u_component_treatment.add_observation_stage:0006", "This method is not allowed to add a root observation", 4)
	return -1
end if
...
if pb_find_existing then
	...
	if ll_observation_sequence < 0 then
		log.log(this, "u_component_treatment.add_observation:0026", "Error finding observation (" + ps_observation_id + ")", 4)
		return -1
	end if
...
if li_sts <= 0 then
	log.log(this, "u_component_treatment.add_observation:0079", "Error adding p_observation record", 4)
	return -1
end if
...
if isnull(ll_observation_sequence) then
	log.log(this, "u_component_treatment.add_observation:0086", "Error adding p_observation record, null observation_sequence", 4)
	return -1
end if
...
if (sqlca.sqlcode = 100) or (ll_check_treatment_id <> treatment_id) then
	...
	if not tf_check() then return -1
	if isnull(ll_observation_sequence) then
		log.log(this, "u_component_treatment.add_observation:0108", "Unable to determine the correct observation_sequence (" + string(ll_incorrect_observation_sequence) + ")", 4)
		return -1
	end if
...
// We must have have an observation sequence
if isnull(pl_observation_sequence) then
	log.log(this, "u_component_treatment.get_result:0021", "Null observation sequence", 4)
	return -1
end if
...
if isnull(treatment_description) then
	...
	elseif not isnull(procedure_id) then
		...
		if cprdb.sqlcode = 100 then
			mylog.log(this, "u_component_treatment.order_treatment_old:0122", "Unable to get procedure definition (" + procedure_id + ")", 4)
...
ll_row = p_observation.find(ls_find, 1, p_observation.rowcount())
if ll_row <= 0 then
	log.log(this, "u_component_treatment.add_result_record:0025", "observation sequence not found (" + string(pl_observation_sequence) + ")", 4)
	return -1
end if
...
lstr_result = datalist.observation_result(ls_observation_id, pi_result_sequence)
if isnull(lstr_result.result_sequence) then
	log.log(this, "u_component_treatment.add_result_record:0033", "observation result not found (" + ls_observation_id + ", " + string(pi_result_sequence) + ")", 4)
	return -1
end if
...
if upper(lstr_result.result_amount_flag) = "Y" then
	...
	if isnull(luo_unit) then
		log.log(this, "u_component_treatment.add_result_record:0055", "Unit not found (" + ps_result_unit + ")", 4)
		return -1
	end if
	...
	if li_sts < 0 then
		log.log(this, "u_component_treatment.add_result_record:0062", "Result value not valid (" + ps_result_value + ")", 4)
		return -1
	end if
end if
...
if li_sts <= 0 then
	log.log(this, "u_component_treatment.add_result_record:0107", "Error adding p_observation_result record", 4)
	tf_rollback()
	return -1
end if
tf_commit()

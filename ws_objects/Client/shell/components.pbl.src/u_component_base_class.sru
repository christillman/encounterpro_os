...
CATCH (throwable lo_error)
	...
	log.log(this, "u_component_base_class:time", ls_error, 4)
	li_sts = -1
FINALLY
...
public function integer timer_ding ();mylog.log(this, "u_component_base_class.timer_ding:0001", "ding...", 1)
...
if isnull(ps_database) or trim(ps_database) = "" then
	...
	if isnull(ps_database) then
		mylog.log(this, "u_component_base_class.db_connect:0026", "Unable to determine database", 4)
		return -1
	end if
end if
...
ll_count = luo_data.retrieve(component_id, gnv_app.office_id, gnv_app.computer_id)
...
if debug_mode then
	log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0026", "Attempting to instantiate com object (" + dotnet_component_wrapper_class + ")", 2)
end if
li_sts = com_wrapper.connecttonewobject(dotnet_component_wrapper_class)
if li_sts = 0 then
	if debug_mode then
		log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0031", "instantiation successful (" + dotnet_component_wrapper_class + ")", 2)
	end if
else
	log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0034", "Error connecting to com source (" + dotnet_component_wrapper_class + ", " + string(li_sts) + ")", 4)
	return -1
end if
...
if li_sts < 0 or isnull(dotnet_component_attributes_xml) then
	log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0050", "Error getting context attributes", 4)
	return -1
end if
...
if isnull(dotnet_credential_attributes_xml) then
	log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0059", "Error getting credential attributes", 4)
	return -1
end if
...
CATCH (oleruntimeerror lt_error)
	...
	log.log(this, "u_component_base_class.initialize_dotnet_wrapper:0076", ls_message, 4)

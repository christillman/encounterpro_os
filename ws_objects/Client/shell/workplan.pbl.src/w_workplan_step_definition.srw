...
if isnull(ls_ordered_service) then
	log.log(this, "w_workplan_step_definition.configure_service:0034", "Null service", 4)
	item_attributes.setfilter("")
	item_attributes.filter()
	return -1
end if
...
if isnull(luo_service) then
	log.log(this, "w_workplan_step_definition.configure_service:0041", "Error getting service component (" + ls_ordered_service + ")", 4)
	item_attributes.setfilter("")
	item_attributes.filter()
	return -1
end if
...
if not tf_check() then return -1
if sqlca.sqlcode = 100 then
	log.log(this, "w_workplan_step_definition.load_workplan_items:0032", "Workplan step not found (" + string(workplan_id) + ", " + string(step_number) + ")", 4)
	return -1
end if
...
if popup.data_row_count <> 2 then
	log.log(this, "w_workplan_step_definition:open", "Invalid parameters", 4)
	close(this)
	return
end if
...
if isnull(workplan_id) then
	log.log(this, "w_workplan_step_definition:open", "Null workplan_id", 4)
	close(this)
	return
end if
...
if isnull(workplan_id) then
	log.log(this, "w_workplan_step_definition:open", "Null step_number", 4)
	close(this)
	return
end if
...
if li_sts <= 0 then
	log.log(this, "w_workplan_step_definition:post", "Error loading workplan items (" + string(workplan_id) + ", " + string(step_number) + ")", 4)
	close(this)
	return
end if
...
if isnull(luo_treatment_component) then
	log.log(this, "w_workplan_step_definition.cb_add_treatment.clicked:0029", "Unable to get treatment object (" + ls_treatment_type + ")", 4)
	return
end if

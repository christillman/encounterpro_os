...
DO
	...
	if ll_handle > 0 then
		...
		if li_sts <= 0 then
			log.log(this, "u_tv_epro_objects.item_selected:0046", "Error getting treeview item (" + string(pl_handle) + ")", 4)
			return -1
		end if
...
if li_sts <= 0 then
	log.log(this, "u_tv_epro_objects.load_children:0022", "Error getting new treeview item (" + string(handle) + ")", 4)
	return 1
end if
...
if li_sts <= 0 then
	log.log(this, "u_tv_epro_objects:item", "Error getting treeview item (" + string(handle) + ")", 4)
	return -1
end if

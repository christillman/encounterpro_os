$PBExportHeader$u_component_xml_handler.sru
forward
...
if not isnull(pstr_treatment.begin_date) &
  ...
  and pb_create_automatically then
	...
	log.log(this, "u_component_xml_handler.find_treatment:0185", "Error creating new treatment (" + pstr_treatment.treatment_description + ")", 4)
end if
...
if not isnull(pstr_patient.last_name) &
  ...
  and pb_create_automatically then
	...
	if li_sts <= 0 then
		log.log(this, "u_component_xml_handler.find_patient:0121", "Error creating new patient (" + pstr_patient.last_name + ", " + pstr_patient.first_name +")", 4)
		return -1
	end if

	li_sts = f_set_patient(pstr_patient.cpr_id)
	if li_sts <= 0 then
		log.log(this, "u_component_xml_handler.find_patient:0127", "Error setting patient (" + pstr_patient.cpr_id + ")", 4)
		return -1
	end if

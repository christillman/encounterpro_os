$PBExportHeader$u_dw_progress_display.sru
forward
...
CHOOSE CASE lower(progress_object)
	...
	CASE "encounter"
		...
		if li_sts <= 0 then
			log.log(this, "u_dw_progress_display.initialize:0025", "Error getting assessment object (" + string(service.problem_id) + ")", 4)
			return -1
		end if
	...
	CASE "assessment"
		if isnull(service.problem_id) then
			log.log(this, "u_dw_progress_display.initialize:0034", "Null problem_id", 4)
			return -1
		end if
		li_sts = current_patient.assessments.assessment(assessment, service.problem_id)
		if li_sts <= 0 then
			log.log(this, "u_dw_progress_display.initialize:0039", "Error getting assessment object (" + string(service.problem_id) + ")", 4)
			return -1
		end if
	...
	CASE "treatment"
		if isnull(service.treatment) then
			log.log(this, "u_dw_progress_display.initialize:0048", "treatment progress must have treatment object", 4)
			return -1
		end if
	...
	CASE ELSE
		log.log(this, "u_dw_progress_display.initialize:0056", "invalid progress_object (" + progress_object + ")", 4)
		return -1
END CHOOSE

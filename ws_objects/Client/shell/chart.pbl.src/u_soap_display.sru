$PBExportHeader$u_soap_display.sru
forward
...
if li_sts <= 0 then
	log.log(this, "u_soap_display.assessment_menu:0025", "Assessment not found (" + current_patient.cpr_id + ", " + ls_problem_id + ")", 4)
	return
end if
...
CHOOSE CASE display_mode
	CASE "ATAT"
		...
		elseif ls_soap_type = "ASSESSMENT" then
			ls_temp = object.key[pl_parent_row]
			if isnull(ls_temp) then
				log.log(this, "u_soap_display.load_treatment_items:0047", "Null assessment key", 4)
				return -1
			end if
			
			f_split_string(ls_temp, ",", ls_left, ls_right)
			if ls_left = "" then
				log.log(this, "u_soap_display.load_treatment_items:0053", "Invalid assessment key (" + ls_temp + ")", 4)
				return -1
			end if
	...
	CASE "AATT", "TT"
		...
		if ls_soap_type = "TREATMENT" then
			ls_temp = object.key[pl_parent_row]
			if isnull(ls_temp) then
				log.log(this, "u_soap_display.load_treatment_items:0074", "Null treatment_id", 4)
				return -1
			else
				ll_parent_treatment_id = long(ls_temp)
			end if
...
if display_mode = "ATAT" then
	...
	li_sts = load_treatment_items()
	if li_sts < 0 then
		log.log(this, "u_soap_display.load_assessments_and_treatments:0016", "Error loading stand-alone treatments", 4)
		return -1
	end if
...
if display_mode = "TT" then
	...
	li_sts = load_treatment_items()
	if li_sts < 0 then
		log.log(this, "u_soap_display.load_assessments_and_treatments:0039", "Error loading treatments", 4)
		return -1
	end if
...
if display_mode = "AATT" then
	...
	li_sts = load_treatment_items()
	if li_sts < 0 then
		log.log(this, "u_soap_display.load_assessments_and_treatments:0084", "Error loading treatments after assessments", 4)
		return -1
	end if

...
for i = 1 to 3
	...
	elseif i <= li_displayed_count then
		...
		li_sts = qualifier_domain[i].initialize(	cpr_id, &
																...
																selected_qualifiers)
		if li_sts < 0 then
			log.log(this, "w_pick_qualifiers.display_qualifier_domains:0091", "Error initializing qualifier domain", 4)
			return -1
		end if
	else
		qualifier_domain[i].visible = false
...
if popup.data_row_count <> 6 then
	log.log(this, "w_pick_qualifiers:open", "Invalid Parameters", 4)
...
if sqlca.sqlcode = 100 then
	log.log(this, "w_pick_qualifiers:open", "encounter not found (" + cpr_id + ", " + string(encounter_id) + ")", 4)
	return -1
end if
...
if not tf_check() then
	log.log(this, "w_pick_qualifiers:open", "Error getting result", 4)
	popup_return.item_count = 0
	closewithreturn(this, popup_return)
	return
end if
if sqlca.sqlcode = 100 then
	log.log(this, "w_pick_qualifiers:open", "result not found", 4)
	popup_return.item_count = 0
	closewithreturn(this, popup_return)
	return
end if
...
if not isvalid(non_enumerated_qualifier_domain) or isnull(non_enumerated_qualifier_domain) then
	log.log(this, "w_pick_qualifiers:open", "Error opening non-enumerated qualifier domain", 4)
	popup_return.item_count = 0
	closewithreturn(this, popup_return)
	return
end if
...
for i = 1 to 3
	...
	if not isvalid(qualifier_domain[i]) or isnull(qualifier_domain[i]) then
		log.log(this, "w_pick_qualifiers:open", "Error opening qualifier domain", 4)
		popup_return.item_count = 0
		closewithreturn(this, popup_return)
		return
	end if
...
if li_sts < 0 then
	log.log(this, "w_pick_qualifiers:open", "Error displaying qualifiers", 4)
	popup_return.item_count = 0
	closewithreturn(this, popup_return)
	return

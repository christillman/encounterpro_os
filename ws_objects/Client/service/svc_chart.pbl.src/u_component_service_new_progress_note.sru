...
//		description = current_patient.name()
	...
	CASE "encounter"
		...
		if li_sts <= 0 then
			log.log(this, "u_component_service_new_progress_note.set_progress_type:0030", "Error getting assessment object (" + string(problem_id) + ")", 4)
			return -1
		end if
...
//		description = encounter.description
	...
	CASE "assessment"
		if isnull(problem_id) then
			log.log(this, "u_component_service_new_progress_note.set_progress_type:0058", "Null problem_id", 4)
			return -1
		end if
		li_sts = current_patient.assessments.assessment(assessment, problem_id)
		if li_sts <= 0 then
			log.log(this, "u_component_service_new_progress_note.set_progress_type:0063", "Error getting assessment object (" + string(problem_id) + ")", 4)
			return -1
		end if
...
//		description = assessment.assessment
	...
	CASE "treatment"
		if isnull(treatment) then
			log.log(this, "u_component_service_new_progress_note.set_progress_type:0091", "treatment progress must have treatment object", 4)
			return -1
		end if
...
//		description = treatment.treatment_description
	...
	CASE ELSE
		log.log(this, "u_component_service_new_progress_note.set_progress_type:0121", "invalid context_object (" + context_object + ")", 4)
		return -1
END CHOOSE
...
if li_sts <= 0 then
	log.log(this, "u_component_service_new_progress_note.xx_do_service:0014", "Error setting progress object", 4)
	return -1
end if

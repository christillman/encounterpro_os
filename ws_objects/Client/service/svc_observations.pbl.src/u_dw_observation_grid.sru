...
//			if ll_observation_sequence < 0 then
//				log.log(this, "u_dw_observation_grid.results_posted:0038", "Error adding observation", 4)
//				return -1
//			end if
//	
...
if isnull(ll_observation_sequence) then
	...
	ll_observation_sequence = treatment.add_observation_stage(ll_parent_observation_sequence, &
										...
										true)
	if ll_observation_sequence < 0 then
		log.log(this, "u_dw_observation_grid.observation_sequence:0027", "Error adding observation", 4)
		return -1
	end if
...
if isnull(root_observation_id) then
	log.log(this, "u_dw_observation_grid.initialize:0024", "No Root Observation_id", 4)
	return -1
end if

root_observation_sequence = service.get_root_observation()
if isnull(root_observation_sequence) then
	log.log(this, "u_dw_observation_grid.initialize:0030", "Error getting root observation", 4)
	return -1
end if
...
if isnull(ls_composite_flag) or ls_composite_flag <> "Y" then
	log.log(this, "u_dw_observation_grid.initialize:0042", "Parent observation not composite (" + root_observation_id + ")", 4)
	return -1
end if

lstr_tree = datalist.observation_tree(root_observation_id)
if lstr_tree.branch_count <= 0 then
	log.log(this, "u_dw_observation_grid.initialize:0048", "No children found (" + root_observation_id + ")", 4)
	return -1
end if
...
if isnull(ll_parent_observation_sequence) then
	...
	if ll_parent_observation_sequence < 0 then
		log.log(this, "u_dw_observation_grid.parent_observation_sequence:0022", "Error adding observation", 4)
		return -1
	end if

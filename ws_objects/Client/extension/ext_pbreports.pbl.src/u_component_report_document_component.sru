...
if isnull(luo_document) then
	log.log(this, "u_component_report_document_component.xx_printreport:0043", "error getting component (" + ls_document_component_id + ")", 4)
	printreport_status = -1
	return -1
end if
...
TRY
	...
	if li_sts < 0 then
		log.log(this, "u_component_report_document_component.xx_printreport:0051", "Error calling document component (" + ls_document_component_id + ")", 4)
		printreport_status = -1
		return -1
	end if
CATCH (exception le_error)
	log.log(this, "u_component_report_document_component.xx_printreport:0056", "Error calling document component (" + ls_document_component_id + "): " + le_error.text, 4)
	printreport_status = -1
	return -1
END TRY
...
if luo_document.document_count > 0 then
	...
	if luo_document.document_count > 1 then
		...
		log.log(this, "u_component_report_document_component.xx_printreport:0069", ls_message, 3)
	end if
end if

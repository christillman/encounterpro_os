...
elseif li_count = 0 then
	mylog.log(this, "u_component_serverservice_incoming.xx_initialize:0017", "No incoming subscribers", 2)
end if
...
for i = 1 to li_count
	...
	if isnull(ls_transport_component_id) then
		mylog.log(this, "u_component_serverservice_incoming.xx_initialize:0027", "Null transport_component_id", 4)
		continue
	end if

	luo_transport = my_component_manager.get_component(ls_transport_component_id)
	if isnull(luo_transport) then
		mylog.log(this, "u_component_serverservice_incoming.xx_initialize:0033", "Unable to get component (" + ls_transport_component_id + ")", 4)
		continue
	end if
	...
	if li_sts <= 0 then
		mylog.log(this, "u_component_serverservice_incoming.xx_initialize:0042", "Error starting receiving (" + ls_transport_component_id + ")", 4)
		continue
	end if
next

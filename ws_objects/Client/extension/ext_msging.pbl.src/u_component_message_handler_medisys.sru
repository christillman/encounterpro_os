$PBExportHeader$u_component_message_handler_medisys.sru
forward
...
IF li_sts < 0 THEN
		mylog.log(this, "u_component_message_handler_medisys.medisys_diagnosis:0011", "The parse_csv() function failed Aborting Appointment Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "u_component_message_handler_medisys.medisys_procedure:0016", "The parse_csv() function failed Aborting Appointment Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF
ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
...
DO WHILE lb_loop 
	...
	IF li_comma > 0 THEN									// We're still in a valid string
		...
		IF li_length > li_value_max_len THEN 
			mylog.log(this, "u_component_message_handler_medisys.parse_csv:0092", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
			RETURN -1
		END IF
		if li_length = 0 then
			ls_value = ""
			...
			if li_quote = li_start then 
				...
				if lb_quote_true = false then 	//Adjust for special situation where there is a comma in middle of text field of form "xxx,xxx" 
					...
					if li_quote_comma > li_comma then
						...
						mylog.log(this, "u_component_message_handler_medisys.parse_csv:0111", "A field of form 'x,x' does not have an ending quote PARSING FAILED for (" + is_stg + ")",4)
						RETURN -1
					end if
				else
					li_length = li_end - li_start
	...
	ELSE
		...
		IF li_ctr = 1 THEN
			...
			mylog.log(this, "u_component_message_handler_medisys.parse_csv:0129", "Never found an occurrance of quote-comma-quote in the input value, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
			RETURN -1
		ELSE
			...
			IF li_length > li_value_max_len THEN 
				mylog.log(this, "u_component_message_handler_medisys.parse_csv:0138", "A field length exceeds maximum number of characters,string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
				RETURN -1
			END IF	
			ls_value = mid(is_stg,li_start,li_length)		// Assign the currently selected value to ls_value
			is_array [li_ctr] = ls_value					//	Assign ls_value to the current array value
	...
	IF li_ctr > li_max_fields THEN
		mylog.log(this, "u_component_message_handler_medisys.parse_csv:0150", "Number of fields in record " + string(li_ctr) + " exceeds the maximum of " + string(li_max_fields) + " PARSING FAILED for (" + is_stg + ", " + + ")",4)
		RETURN -1
	END IF
LOOP	
...
IF lb_empty_flag THEN
	mylog.log(this, "u_component_message_handler_medisys.parse_csv:0174", "None of the fields in record contains an entry, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
	RETURN -1	
END IF
		...
		IF li_sts_stg > 0 THEN
			...
			IF ll_str_len > 0 then
				...
				IF li_pos = 1 THEN 
					li_start = li_pos + 1		// If so, it;s OK to start
				ELSE
					mylog.log(this, "u_component_message_handler_medisys.xx_handle_message:0085", "The input string does not start with a quote, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
					RETURN -1
				END IF
				...
				IF li_comma > 0 THEN				// We're still in a valid string
					...
					IF li_length > li_value_max_len THEN 
						mylog.log(this, "u_component_message_handler_medisys.xx_handle_message:0093", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
						RETURN -1
					END IF	
...
IF li_filenum >0 THEN
	...
	IF li_sts < 1 THEN
		mylog.log(this, "u_component_message_handler_medisys.xx_handle_message:0128", "Unable to close the medisys.mmi incoming transfer file...Aborting Appointment Creation for Message ID (" + string(li_message_id) + ")", 4)
		RETURN -1									
	END IF
END IF
yield()
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "u_component_message_handler_medisys.medisys_insurance:0022", "The parse_csv() function failed Aborting Appointment Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF	

ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
		mylog.log(this, "u_component_message_handler_medisys.medisys_insurance:0028", "The parse_csv() function failed Message ID ("+ string(message_id) + ")", 4)
		RETURN -1
else 
	li_sts = ll_array_count
END IF	
...
if ll_array_count < 5 then
	mylog.log(this, "u_component_message_handler_medisys.medisys_insurance:0035","Interface did not provide insurance sequence (" + is_stg + ")",2)
	return -1
end if
...
SELECT cpr_id
	...
	if cprdb.sqlcode = 100 then 
		mylog.log(this, "u_component_message_handler_medisys.medisys_insurance:0053", "Patient does not exist for  ("+ ls_billing_id + ")", 4)
		RETURN -1
	end if
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
	mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0041", "The parse_csv() function failed Aborting Appointment Creation)", 4)
	RETURN -1
END IF	

ls_billing_id = is_array[1] 
mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0046", "The patient billing_id=" + ls_billing_id, 2)
SELECT cpr_id
INTO :ls_cpr_id
FROM p_Patient
WHERE billing_id = :ls_billing_id
...
if (is_array[7] = "0" or isnull(is_array[7]) or is_array[7] = "") then
	ls_error = is_array[7]
	mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0083", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
	...
	if isdate(is_array[7]) then
		...
		ls_error = is_array[7]
		mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0092", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
		setnull(ld_birthdate)
		setnull(ldt_date_of_birth)
	end if
end if
...
if isnull(ls_cpr_id) then
	mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0112", "create new patient, lname=" + ls_last_name, 1)
...
mylog.log(this, "u_component_message_handler_medisys.medisys_patient:0151", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0052", "The parse_csv() function failed Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
		RETURN -1
END IF	
...
If isnull(ls_billing_id) or ls_billing_id = "" then
	mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0078", "Billingid not found Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
END IF	
If isnumber(ls_billing_id) then
	ll_encounter_billing_id = long(ls_billing_id)
else
	mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0084", "Billingid not numeric Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
END IF
...
IF NOT cprdb.check() THEN RETURN -1
IF cprdb.sqlcode = 100 THEN
	mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0183", "Unable to retrieve a CPR_ID to match the Billing_ID..Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN 1
END IF
...
If Isnull(ls_primary_provider_id) then
	ls_error = is_array[4]
	mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0198", "Unable to retrieve a provider_ID to match the input provider_ID " + ls_error + " ..Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
End if			
...
IF NOT cprdb.check() THEN 
	...
	mylog.log(this, "u_component_message_handler_medisys.medisys_arrived:0238", "Unable write a record to x_medisys_Arrived...Aborting Appointment Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(li_message_id) + ")", 4)
	RETURN -1									
END IF

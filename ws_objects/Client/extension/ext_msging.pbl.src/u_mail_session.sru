$PBExportHeader$u_mail_session.sru
forward
...
for i = 1 to ll_count
	...
	if lm_sts <> mailReturnSuccess! then
		mylog.log(this, "u_mail_session.get_messages:0021", "Error getting mail message (" + string(lm_sts) + ")", 4)
		return -1
	end if
	...
	if not isnull(luo_to_user) then
		...
		if li_sts <= 0 then
			mylog.log(this, "u_mail_session.get_messages:0030", "Error posting mail message (" + luo_message.subject + ")", 4)
			return -1
		end if
		
		lm_sts = mailDeleteMessage(MessageID[i])
		if lm_sts <> mailReturnSuccess! then
			mylog.log(this, "u_mail_session.get_messages:0036", "Error deleting mail message (" + luo_message.subject + ", " + string(lm_sts) + ")", 4)
			return -1
		end if
	end if
next
...
if lm_sts <> mailReturnSuccess! then
	mylog.log(this, "u_mail_session.initialize:0013", "Error starting mail session (" + string(lm_sts) + ")", 4)
	return -1
end if

	...
	//	Read each line of the file contents into the string ls_stg
		...
		IF li_sts_stg > 0 THEN
			...
			IF ll_str_len > 0 then
				...
				ELSE
					mylog.log(this, "u_component_message_handler_raintree.xx_handle_message:0062", "The input string does not start with a quote, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
					RETURN -1
				END IF
				...
				IF li_comma > 0 THEN				// We're still in a valid string
					...
					IF li_length > li_value_max_len THEN 
						mylog.log(this, "u_component_message_handler_raintree.xx_handle_message:0070", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
						RETURN -1
					END IF	
...
DO WHILE lb_loop 
	...
	IF li_comma > 0 THEN									// We're still in a valid string
		...
		IF li_length > li_value_max_len THEN 
			mylog.log(this, "u_component_message_handler_raintree.parse_csv:0088", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
			RETURN -1
		END IF	
		...
		if li_quote = li_start then 
			...
			if lb_quote_true = false then 	//Adjust for special situation where ther is a comma in middle of text field of form "xxx,xxx" 
					...
					if li_quote_comma > li_comma then
						...
						mylog.log(this, "u_component_message_handler_raintree.parse_csv:0102", "A field of form 'x,x' does not have an ending quote PARSING FAILED for (" + is_stg + ")",4)
						RETURN -1
					end if
				else
					li_length = li_end - li_start
	...
	ELSE
		...
		IF li_ctr = 1 THEN
			...
			mylog.log(this, "u_component_message_handler_raintree.parse_csv:0118", "Never found an occurrance of quote-comma-quote in the input value, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
			RETURN -1
		ELSE
			...
			IF li_length > li_value_max_len THEN 
				mylog.log(this, "u_component_message_handler_raintree.parse_csv:0127", "A field length exceeds maximum number of characters,string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
				RETURN -1
			END IF
			ls_value = mid(is_stg,li_start,li_length)		// Assign the currently selected value to ls_value
			is_array [li_ctr] = ls_value					//	Assign ls_value to the current array value
	...
	IF li_ctr > li_max_fields THEN
		mylog.log(this, "u_component_message_handler_raintree.parse_csv:0139", "Number of fields in record " + string(li_ctr) + " exceeds the maximum of " + string(li_max_fields) + " PARSING FAILED for (" + is_stg + ", " + + ")",4)
		RETURN -1
	END IF
LOOP	
...
IF lb_empty_flag THEN
	mylog.log(this, "u_component_message_handler_raintree.parse_csv:0158", "None of the fields in record contains an entry, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
	RETURN -1	
END IF
...
mylog.log(this, "u_component_message_handler_raintree.raintree_patient:0054", "The patient billing_id=" + ls_billing_id, 2)
SELECT cpr_id
INTO :ls_cpr_id
FROM p_Patient
...
if (is_array[7] = "0" or isnull(is_array[7]) or is_array[7] = "") then
	ls_error = is_array[7]
	mylog.log(this, "u_component_message_handler_raintree.raintree_patient:0089", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
	...
	if isdate(fix_date) then
		...
		ls_error = is_array[7]
		mylog.log(this, "u_component_message_handler_raintree.raintree_patient:0099", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
		return -1
	end if
end if
...
if isnull(ls_cpr_id) then
	mylog.log(this, "u_component_message_handler_raintree.raintree_patient:0122", "create new patient, lname=" + ls_last_name, 2)
...
mylog.log(this, "u_component_message_handler_raintree.raintree_patient:0161", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)

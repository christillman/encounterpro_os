...
//if ll_count = 0 then
//	mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0043", "no results found ..Aborting, Message ID (" + string(is_message_id) + "," + ls_external_id + ")", 4)
//	return -1
//end if
//j = integer(ll_count)
//
...
//		mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0053", "id not provided ..Aborting, Message ID (" + string(is_message_id) + ")", 4)
//		return -1
//	end if
//
//	mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0057", "The patient id=" + ls_external_id, 1)
...
//			mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0066", "cpr id not found ..Aborting, Message ID (" + string(is_message_id) + "," + ls_external_id + ")", 4)
//			return -1
//		end if
...
//		mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0075", "specimen id not provided ..Aborting, Message ID (" + string(is_message_id) + ")", 4)
//		return -1
//	end if
//		
...
//			if not cprdb.check() then 	return -1
//			if cprdb.sqlcode = 100 then
//				mylog.log(this, "u_component_incoming_filecopy_hl7.labresults:0142", "no lab order found ..Aborting, Message ID (" + string(is_message_id) + "," + ls_external_id + "," + ls_specimen_id+ ")", 4)
//				return -1
//			end if 
//		end if
//		
...
if isnull(dcom_app) or dcom_app = '' then
	mylog.log(this, "u_component_incoming_filecopy_hl7.xx_initialize:0058", "no object source.", 4)
	return -1
end if

get_attribute("dcom_app_ack",dcom_ack_app)
if isnull(dcom_ack_app) or dcom_ack_app = '' then
	mylog.log(this, "u_component_incoming_filecopy_hl7.xx_initialize:0064", "no object source acknowledgement.", 4)
	return -1
end if
...
li_rtn = ahc_connect()
if li_rtn <= 0 Then 
	mylog.log(this, "u_component_incoming_filecopy_hl7.xx_initialize:0093", "Unable to connect to AHC Interface.", 4)
	return -1
end if
...
If ll_pending > 0 Then
	...
	For i = 1 to ll_pending
   	...
   	// get messages
		...
		setnull(coll)
	Next
	mylog.log(this, "u_component_incoming_filecopy_hl7.timer_ding:0033", "messages processed (" + string(ll_pending) + ")", 2)
	Return 2
Else
	mylog.log(this, "u_component_incoming_filecopy_hl7.timer_ding:0036", "No incoming messages to process ..", 1)
End If
...
mylog.log(this, "u_component_incoming_filecopy_hl7.ahc_connect:0007", "connecting to AHCMessager.", 1)
...
mylog.log(this, "u_component_incoming_filecopy_hl7.ahc_connect:0017", "connection to AHCMessager success", 1)

// Connect to AHC Messanger and returns AHC Message Manager
mylog.log(this, "u_component_incoming_filecopy_hl7.ahc_connect:0020", "connection to IHCMessageManager object.", 1)

omsgman = oAHC.Connect(dcom_app)
If isnull(omsgman) Then
	mylog.log(this, "u_component_incoming_filecopy_hl7.ahc_connect:0024", "ERROR: connection to IHCMessageManager object failed.", 4)
	oAHC.Disconnect()
	oahc.disconnectobject()
	DESTROY oAHC
...
mylog.log(this, "u_component_incoming_filecopy_hl7.ahc_connect:0034", "connection to IHCMessageManager object successful.", 1)
...
mylog.log(this, "u_component_incoming_filecopy_hl7.message_parser:0039", "message object " + ls_msgtypeobj + " From " + is_FromApp, 2)	
setnull(is_cpr_id)
...
Choose Case ls_msgtype
	Case "ADT"
		Choose Case ls_msgevent
			...
			Case Else
				mylog.log(this, "u_component_incoming_filecopy_hl7.message_parser:0061", "Cannot handle type message " + ls_msgtypeobj + "Message ID (" + is_message_id + ")", 4)	
		End Choose
	Case "ORU"
		Choose Case ls_msgevent
			Case 	"O02"	
	...
	Case "SIU" 
		Choose Case ls_msgevent
			...
			Case "S14"
				...
				if li_sts = 1 Then li_sts = arrived(omsg)
			Case Else
				mylog.log(this, "u_component_incoming_filecopy_hl7.message_parser:0079", "Cannot handle type message " + ls_msgtypeobj + "Message ID (" + is_message_id + ")", 4)	
				li_sts = 1
		End Choose
	Case "ACK"
		li_sts = get_billing_acks(omsg)
	Case Else
		mylog.log(this, "u_component_incoming_filecopy_hl7.message_parser:0085", "Cannot handle type message " + ls_msgtypeobj + "Message ID (" + is_message_id + ")", 4)	
End Choose
...
IF bs_system = 'IDX' or bs_system = 'MEDIC' or bs_system='PRACTICEPOINT' then // use socket messanger to send ack's
	...
	if lb_ackfile then
		...
		if li_sts <> 0 then
			DESTROY jsocket
			setnull(jsocket)
			mylog.log(this, "u_component_incoming_filecopy_hl7.send_acks:0127", "ERROR: connection to jsocket object failed.", 4)
			return -1
		end if
		...
		if socket_return = 'Success' then
			...
			mylog.log(this,"u_component_incoming_filecopy_hl7.send_acks:0134","error with send to jsocket " + socket_return,4)
			li_sts = -1
		end if
		DESTROY jsocket
		setnull(jsocket)
...
if li_sts <> 0 then
	mylog.log(this, "u_component_incoming_filecopy_hl7.send_acks:0163", "ERROR: connection to AHC messagetype object failed.", 4)
	DESTROY oMsgtype
	setnull(oMsgtype)
	return -1
end if
...
mylog.log(this, "u_component_incoming_filecopy_hl7.send_acks:0229", "sendmessage to " + is_FromApp + " from " + dcom_ack_app, 1)	
...
protected function integer xx_shutdown ();if not isnull(oAHC) Then
	...
	oAHC.Disconnect()
	mylog.log(this,"u_component_incoming_filecopy_hl7.xx_shutdown:0004","disconnected from AHC Interface",1)
End If
Destroy oAHC
Destroy omsgMan
Setnull(oAHC)
...
Using cprdb;
If cprdb.sqlcode = 100 Then
	mylog.log(this, "u_component_incoming_filecopy_hl7.get_billing_acks:0029", "No records found for message ID (" + ls_ack_message_id + ")", 2)
	Return 1
End If
...
If ls_ack_type = "AA" or ls_ack_type = "AL" or ls_ack_type = 'CA' then // bill Accepted
	...
	mylog.log(this, "u_component_incoming_filecopy_hl7.get_billing_acks:0041", "Acknowledge received for message ID (" + ls_ack_message_id + ")", 2)
Elseif ls_ack_type = "AE" then
	...
	mylog.log(this, "u_component_incoming_filecopy_hl7.get_billing_acks:0051", "Ack Reject received for message ID (" + ls_ack_message_id + "), Error: "+ls_error, 4)	
Else
	Update o_message_log
	Set status = 'ACK_NACK'
	Where id like :ls_ack_message_id
	...
	mylog.log(this, "u_component_incoming_filecopy_hl7.get_billing_acks:0061", "NonAcknowledge received for message ID (" + ls_ack_message_id + ")" , 2)	
End if	
Destroy ack
Setnull(ack)
...
if isnull(ls_patient_name) or len(ls_patient_name) = 0 Then
	log.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0105", "Message id "+is_message_id+ " did not have valid PID-PatientName, message rejected ", 4)
	return -1
end if
// Gettting Billing Id
if isnull(ls_external_id) or ls_external_id = "" then
	if isnull(ls_internal_id) or ls_internal_id = "" then
		if isnull(ls_alternate_PID) or ls_alternate_PID = "" then
			mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0112", "External Patient Id(Billing Id) not provided for ( "+ls_last_name+","+ls_first_name+ " Message (" + string(is_message_id) + ") rejected", 4)
			return -1
		else
...
if isnull(ls_billing_id) then
	setnull(ls_cpr_id)
	mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0127", "External Patient Id(Billing Id) not provided for ( "+ls_last_name+","+ls_first_name+ " Message (" + string(is_message_id) + ") rejected", 4)
	return -1
end if
...
if (ls_birthdate = "0" or isnull(ls_birthdate) or ls_birthdate = "") then
	...
	log.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0166", "Patient "+ls_patient_name+" didnt have valid birthdate (" + ls_error  + ")", 3)
	...
	if isdate(fix_date) then
		...
		ls_error = ls_birthdate
		log.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0176", "Patient "+ls_patient_name+" didnt have valid birthdate (" + ls_error  + ")", 3)
		setnull(ld_birthdate)
	end if
end if
...
if ll_count > 0 then
	...
	//	ls_gnumber = omsg.guarantor[0].guarantornumber[0].id.valuestring
		...
		if ll_count = 0 then
			mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0230", "Guarantor name is not provided and so ignore this..", 1)
		else
...
elseif not isnull(ls_pcp) and len(ls_pcp) > 0 then
	...
	if isnull(ls_primary_provider_id) then 
		...
		log.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0291",ls_temp, 3)
	end if		
...
mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0347", "patient info=" + ls_billing_id + " " + ls_last_name + "," + ls_first_name, 1)
...
IF isnull(ls_cpr_id) THEN
	...
	li_sts = f_create_new_patient( &
									...
									ls_ssn)
	if li_sts <= 0 then 
		mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0399","create failed",4)
		li_rtn = -4
		Return li_rtn
	end if
	lb_new_patient = true
...
If not cprdb.check() then 
	mylog.log(this, "u_component_incoming_filecopy_hl7.patient_adt:0504","update failed " + string(cprdb.sqlcode) + " " + cprdb.sqlerrtext,4)
	return -1
End If
...
if ll_count > 0 then
	...
	mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0079", "PatientIdentification Segment not filled in.. Can't process this message id (" + string(is_message_id) + ")", 4)
	return -1
end if	
...
if isnull(ls_external_id) or ls_external_id = "" then
	if isnull(ls_internal_id) or ls_internal_id = "" then
		if isnull(ls_alternate_PID) or ls_alternate_PID = "" then
			mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0087", "id not provided, Message ID (" + string(is_message_id) + ")", 4)
...
if isnull(ls_billing_id) then
	setnull(ls_cpr_id)
	mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0100","null billing_id",4)
	return -1
end if
...
if (ls_birthdate = "0" or isnull(ls_birthdate) or ls_birthdate = "") then
	ls_error = ls_birthdate
	log.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0139", "birthdate error=" + ls_error  + ")", 3)
	setnull(ldt_date_of_birth)
	setnull(ld_birthdate)
else	
	fix_date = left(ls_birthdate,4) + ' ' + mid(ls_birthdate,5,2) + ' ' + mid(ls_birthdate,7,2)
	if isdate(fix_date) then
		...
		ls_error = ls_birthdate
		mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0149", "birthdate error=" + ls_error + ")", 3)
		setnull(ld_birthdate)
	end if
end if
...
if not isnull(ls_attending_doctor) and len(ls_attending_doctor) > 0 then
	...
	if isnull(ls_primary_provider_id) or len(ls_primary_provider_id) = 0 then 
		...
		log.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0209",ls_temp, 3)
	end if
...
mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0233", "patient info=" + ls_billing_id + " " + ls_last_name + "," + ls_first_name, 2)
...
if isnull(ls_cpr_id) then
	li_sts = f_create_new_patient( &
									...
									ls_ssn)
	if li_sts <= 0 then 
		mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0285","create failed",4)
		li_rtn = -4
	else
		lb_new_patient = true
		// add patient progress
...
USING cprdb;
If not cprdb.check() then 
	mylog.log(this, "u_component_incoming_filecopy_hl7.patient_siu:0401","update failed " + string(cprdb.sqlcode) + " " + cprdb.sqlerrtext,4)
	return -1
End If

$PBExportHeader$u_component_serverservice_messages.sru
forward
...
if isnull(ls_handler_component_id) then
	log.log(this, "u_component_serverservice_messages.get_handler:0023", "Null message handler component for message type (" + ps_message_type + ")", 4)
	return luo_handler
end if

luo_handler = my_component_manager.get_component(ls_handler_component_id)
if isnull(luo_handler) then
	log.log(this, "u_component_serverservice_messages.get_handler:0029", "Error getting message handler component (" + ls_handler_component_id + ")", 4)
	return luo_handler
end if
/*
...
if ll_count > 0 then
	...
	if li_sts <= 0 then
		log.log(this, "u_component_serverservice_messages.timer_ding:0042", "Error handling message (" + string(ll_message_id) + ")", 4)
		return -1
	end if

...
if ll_count > 0 then
	...
	mylog.log(this, "u_hl7_arrived_nterprise.arrived:0103", "PID Segment not provided, Message ID (" + string(is_message_id) + ")", 4)
	return -1
end if	
	...
	if isnull(ls_external_id) or ls_external_id = "" then
		if isnull(ls_internal_id) or ls_internal_id = "" then
			if isnull(ls_alternate_PID) or ls_alternate_PID = "" then
				mylog.log(this, "u_hl7_arrived_nterprise.arrived:0111", "Patient External ID not provided, Message ID (" + string(is_message_id) + ")", 4)
				return -1
			else
...
//	Populate the appointment type which need to be written to x_encounter_Arrived
		...
		if upper(ls_arrived) <> 'ARRIVED' then 
			mylog.log(this, "u_hl7_arrived_nterprise.arrived:0131", "SIU not arrived (" + ls_arrived  + ")", 2)
			return 1
		end if	
...
if not isnull(ls_scheduledatetime) then
	...
	if ld_scheduledate <> ld_thisdate then
		...
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0218", ls_message , 4)
		GOTO error
	end if
end if
...
if not isnull(ls_attending_doctor_id) and len(ls_attending_doctor_id) > 0 Then
	...
	If sqlca.sqlcode = 100 then // no match
		...
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0248", ls_provider_message , 3)
		Setnull(ls_attending_doctor)
	End if
end if
...
If not isnull(ls_resourceid) AND trim(ls_resourceid) <> "" then
	mylog.log(this, "u_hl7_arrived_nterprise.arrived:0260", "Resource Id (" + ls_resourcetext +")" , 2)
	mylog.log(this, "u_hl7_arrived_nterprise.arrived:0261", "Appointment Type (" + ls_appointment_type +")" , 2)
// min of Sort Sequence
	...
	// get the encounter type for a resource	
		...
		If cprdb.sqlcode = 100 Then
			mylog.log(this, "u_hl7_arrived_nterprise.arrived:0292", "Resource not mapped (" + ls_resourcetext + ")", 3)
		Else
			lb_resource = true
		End if
	End If
	if not lb_resource then
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0298", "Resource not mapped (" + ls_resourcetext + ")", 3)
	end if
Elseif isnull(ls_attending_doctor) Then // may be in resource table and AIG may not be filled in all the time with ntierprise
	...
	If sqlca.sqlcode = 100 then // no match
		...
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0308", ls_temp , 3)
		Setnull(ls_attending_doctor)
	End if
End If
...
If isnull(ls_encounter_type) and isnull(ls_new_flag) Then // Not a resource Visit
	if isnull(ls_appointment_type) or ls_appointment_type = "" then
		...
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0316",ls_appointment_message, 3)
	else	
		SELECT encounter_type,
			 new_flag
		INTO :ls_encounter_type,
		...
		If cprdb.sqlcode = 100 then
			...
			mylog.log(this, "u_hl7_arrived_nterprise.arrived:0329",ls_appointment_message, 3)
			Setnull(ls_encounter_type)
			Setnull(ls_new_flag)
		Else
			mylog.log(this, "u_hl7_arrived_nterprise.arrived:0333", "appointment type ("+ls_appointment_type+")",2)
		End If
	end if
End If
...
IF isnull(is_cpr_id) THEN
	...
	IF cprdb.sqlcode = 100 THEN
		...
		mylog.log(this, "u_hl7_arrived_nterprise.arrived:0374", ls_message, 4)
		GOTO error
	end if
ELSE
	ls_cprid = is_cpr_id
	...
	IF cprdb.sqlcode = 100 THEN
			...
			mylog.log(this, "u_hl7_arrived_nterprise.arrived:0388",ls_message, 4)
			GOTO error
	END IF
END IF
...
IF NOT cprdb.check() THEN 
	...
	mylog.log(this, "u_hl7_arrived_nterprise.arrived:0458", ls_message, 4)
	GOTO error
END IF

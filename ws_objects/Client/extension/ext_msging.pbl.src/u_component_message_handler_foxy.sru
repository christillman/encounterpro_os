...
mylog.log(this, "u_component_message_handler_foxy.parse_csv:0090", "Message received (" + is_stg + ")",1)
...
DO WHILE lb_loop 
	...
	IF li_comma > 0 THEN									// We're still in a valid string
		...
		IF li_length > li_value_max_len THEN 
			mylog.log(this, "u_component_message_handler_foxy.parse_csv:0108", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
			RETURN -1
		END IF
		if li_length = 0 then
			ls_value = ""
			...
			if li_quote = li_start then 
				...
				if lb_quote_true = false then 	//Adjust for special situation where there is a comma in middle of text field of form "xxx,xxx" 
					...
					if li_quote_comma > li_comma then
						...
						mylog.log(this, "u_component_message_handler_foxy.parse_csv:0127", "A field of form 'x,x' does not have an ending quote PARSING FAILED for (" + is_stg + ")",4)
						RETURN -1
					end if
				else
					li_length = li_end - li_start
	...
	ELSE
		...
		IF li_ctr = 1 THEN
			...
			mylog.log(this, "u_component_message_handler_foxy.parse_csv:0145", "Never found an occurrance of quote-comma-quote in the input value, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
			RETURN -1
		ELSE
			...
			IF li_length > li_value_max_len THEN 
				mylog.log(this, "u_component_message_handler_foxy.parse_csv:0154", "A field length exceeds maximum number of characters,string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
				RETURN -1
			END IF	
			ls_value = mid(is_stg,li_start,li_length)		// Assign the currently selected value to ls_value
			is_array [li_ctr] = ls_value					//	Assign ls_value to the current array value
	...
	IF li_ctr > li_max_fields THEN
		mylog.log(this, "u_component_message_handler_foxy.parse_csv:0166", "Number of fields in record " + string(li_ctr) + " exceeds the maximum of " + string(li_max_fields) + " PARSING FAILED for (" + is_stg + ", " + + ")",4)
		RETURN -1
	END IF
LOOP	
...
IF lb_empty_flag THEN
	mylog.log(this, "u_component_message_handler_foxy.parse_csv:0190", "None of the fields in record contains an entry, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
	RETURN -1	
END IF
...
ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
		mylog.log(this, "u_component_message_handler_foxy.foxmeadows_insurance:0028", "The parse_csv() function failed Message ID ("+ string(message_id) + ")", 4)
		RETURN -1
else 
	li_sts = ll_array_count
END IF	
...
if ls_insurance_type = '' then
	...
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_insurance:0053", "Patient billing id  ("+ ls_billing_id + ")", 2)
	SELECT cpr_id
	INTO :ls_cpr_id
	FROM p_Patient
	...
	if cprdb.sqlcode = 100 then 
		mylog.log(this, "u_component_message_handler_foxy.foxmeadows_insurance:0062", "Patient does not exist for  ("+ ls_billing_id + ")", 4)
		RETURN -1
	end if
	
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_insurance:0066", "Patient billing id  ("+ ls_insurance_id + ")",1)
	SELECT authority_id
...
mylog.log(this, "u_component_message_handler_foxy.foxmeadows_insurance:0129", "end of insurance ", 1)
...
ls_billing_id = is_array[1] 
mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0078", "The patient billing_id=" + ls_billing_id, 2)
SELECT cpr_id
INTO :ls_cpr_id
FROM p_Patient
WHERE billing_id = :ls_billing_id
...
if isnull(ls_DoctorId) or len(ls_doctorid) = 0 then
	...
	if not cprdb.check() then return -1
	if cprdb.sqlcode = 100 then
		mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0103", "provider id =" + ls_doctorid+ " is not mapped", 3)
...
if (is_array[7] = "0" or isnull(is_array[7]) or is_array[7] = "") then
	ls_error = is_array[7]
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0116", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
...
//reformat... try yyyymmdd	
		...
		if isdate(fix_date) then
			...
			ls_error = is_array[7]
			mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0131", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
			setnull(ld_birthdate)
			setnull(ldt_date_of_birth)
		end if	
	end if
...
if isnull(ls_consultant) or len(ls_consultant) = 0 then
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0199", "Referring provider id=" + ls_ref_provider_id+" is not found in c_Consultant", 3)
	setnull(ls_ref_provider_id)
end if
if isnull(ls_cpr_id) then
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0203", "create new patient, lname=" + ls_last_name, 1)
...
mylog.log(this, "u_component_message_handler_foxy.foxmeadows_patient:0255", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)
...
if is_interface = 'COMPUMEDIC' THEN
	if li_array_upper < 9 then
		...
		if isdate(ls_fixer) then
			...
			mylog.log(this, "u_component_message_handler_foxy.foxmeadows_arrived:0112", "date not provided", 2)
		end if	
	end if	
else
	//foxmeadows
...
if not cprdb.check() then return -1
if ll_count > 0 then 
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_arrived:0175", "billing id,scheduledatetime "+ls_billing_id+","+string(ld_scheduledatetime,"mm/dd/yyyy hh:mm:ss")+" already arrived", 3)
	return 1
end if
mylog.log(this, "u_component_message_handler_foxy.foxmeadows_arrived:0178", "billing id,scheduledatetime "+ls_billing_id+","+string(ld_scheduledatetime,"mm/dd/yyyy hh:mm:ss")+" arrived", 2)
...
if isnull(is_Array[4]) then
	...
	mylog.log(this, "u_component_message_handler_foxy.foxmeadows_arrived:0223", "Doctor Code: " + string(ls_doctor_id) , 2)
...
ELSE
	mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0062", "Unable to find the message_file at the expected location, message id (" + ls_filepath + ", " + string(li_message_id) + ")",4)
	RETURN -1
END IF
			...
			IF ll_str_len > 0 then
				...
				ELSE
					...
					IF li_pos = 1 THEN 
						...
						li_start = li_pos + 1		// If so, it;s OK to start
					ELSE
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0095", "The input string does not start with a quote, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
						RETURN -1
					END IF
				END IF
				if ib_doublequote then
				...
				IF li_comma > 0 THEN				// We're still in a valid string
					...
					IF li_length > li_value_max_len THEN 
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0108", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
					...
					CASE "PAT" 
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0120","entering foxmeadows_patient()",1)
						li_sts = foxmeadows_patient()
					CASE "INS"
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0123","entering foxmeadows_insurance()",1)
						li_sts = foxmeadows_insurance()	
					CASE "REF"
						li_sts = referring_provider()	
					CASE "DGN"
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0128","entering foxmeadows_diagnosis()",1)
						li_sts = foxmeadows_diagnosis()		
					CASE "CHK"
						mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0131","entering foxmeadows_arrived()",1)
...
IF li_filenum >0 THEN
	...
	IF li_sts < 1 THEN
		mylog.log(this, "u_component_message_handler_foxy.xx_handle_message:0153", "Unable to close the file (" + ls_filepath + ")", 3)
	END IF
END IF
RETURN 1

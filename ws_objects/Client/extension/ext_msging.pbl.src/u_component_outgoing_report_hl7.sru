$PBExportHeader$u_component_outgoing_report_hl7.sru
forward
...
mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0005", "connecting to AHCMessager.", 1)
...
mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0014", "connection to AHCMessager success", 1)

// Connect to AHC Messanger and returns AHC Message Manager
mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0017", "connection to IHCMessageManager object.", 1)
omsgman = oAHC.Connect(source_app)
If isnull(omsgman) Then
	mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0020", "ERROR: connection to IHCMessageManager object failed.", 4)
	DESTROY oMsgMan
	oAHC.Disconnect()
	DESTROY oAHC
	setnull(oMsgMan)
...
mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0028", "connection to IHCMessageManager object successful.", 1)

mylog.log(this, "u_component_outgoing_report_hl7.ahc_connect:0030", "ahc_connect() - Successful", 1)
...
public function integer ahc_disconnect ();if not isnull(oAHC) and isvalid(oAHC) Then
	...
	FINALLY
		...
		Setnull(oAHC)
	END TRY
	mylog.log(this,"u_component_outgoing_report_hl7.ahc_disconnect:0012","disconnected from AHC Interface",1)
End If
...
if isnull(destination_app) or destination_app = '' then
	mylog.log(this, "u_component_outgoing_report_hl7.xx_initialize:0012", "no object destination.", 4)
	return -1
end if	

get_attribute("receiving_app_path", recv_app_path)
if isnull(recv_app_path) or recv_app_path = '' then
	mylog.log(this, "u_component_outgoing_report_hl7.xx_initialize:0018", "recev app path is not specified", 4)
	return -1
end if	
...
if li_sts <> 0 then
	mylog.log(this, "u_component_outgoing_report_hl7.sendreport:0025", "ERROR: connection to AHC messagetype object failed.", 4)
	DESTROY omsgtype
	setnull(omsgtype)
	return -1
end if
...
if isnull(oenv) then
	mylog.log(this, "u_component_outgoing_report_hl7.sendreport:0039", "Error in getting message evnvelope object", 4)	
	return -1
end if
...
if isnull(hl7oru) then
	mylog.log(this, "u_component_outgoing_report_hl7.sendreport:0048", "Error in getting message type object from message envelope", 4)	
	return -1
end if
...
If ll_pending > 0 Then
	...
	For i = 1 to ll_pending
	   coll = omsgMan.PollForMessage(1, count)
		count = Coll.Count
   	// get messages
		mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0023", "Polling Count (" + string(count) + ")", 1)
		...
		FOR j = 0 to count - 1
			...
			mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0031", "Collection Count (" + string(j) + ") message type ( "+ls_message_type+")", 1)
			If coll.Item(j).Type.MessageTypeCode = "ACK" Then
   			omsg = coll.Item(j)
				...
				Using cprdb;
				If cprdb.sqlcode = 100 Then
					mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0057", "No records found for message ID (" + ls_ack_message_id + ")", 3)
					continue
				End If
				...
				If ls_ack_type = "AA" or ls_ack_type = "AL" or ls_ack_type = 'CA' then // bill Accepted
					...
					Using cprdb;
					If not cprdb.check() THEN RETURN -1
					mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0067", "Acknowledge received for message ID (" + ls_ack_message_id + ")", 2)
				Elseif ls_ack_type = "AE" then
					...
					Using cprdb;
					If Not cprdb.check() THEN Return -1		
					mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0075", "Ack Reject received for message ID (" + ls_ack_message_id + "), Error: "+ls_error, 4)	
				Else
					Update o_message_log
					Set status = 'LCR_NACK'
					Where id = :ls_ack_message_id
					...
					using cprdb;
					if not cprdb.check() then return -1		
					mylog.log(this, "u_component_outgoing_report_hl7.timer_ding:0083", "NonAcknowledge received for message ID (" + ls_ack_message_id + ")" , 2)	
				End if	
...
If upperbound(ls_fields) < 13 Then
	log.log(this,"u_component_outgoing_report_hl7.lcr_report:0038","Send Failed:message id ("+string(message_id)+") message count is invalid ",3)
...
if fileexists(ls_filename) then
	mylog.log(this, "u_component_outgoing_report_hl7.lcr_report:0131", "Error getting next file number", 4)
	return -1
end if

if mylog.file_write(lblob_report,ls_filename) < 0 then
	mylog.log(this, "u_component_outgoing_report_hl7.lcr_report:0136", "Error writing the report into a file", 4)
	Update o_message_log

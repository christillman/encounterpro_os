$PBExportHeader$u_component_message_handler_medware.sru
forward
...
DO WHILE lb_loop 
	...
	IF li_comma > 0 THEN									// We're still in a valid string
		...
		IF li_length > li_value_max_len THEN 
			mylog.log(this, "u_component_message_handler_medware.parse_csv:0097", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
			RETURN -1
		END IF
		if li_length = 0 then
			ls_value = ""
			...
			if li_quote = li_start then 
				...
				if lb_quote_true = false then 	//Adjust for special situation where there is a comma in middle of text field of form "xxx,xxx" 
					...
					if li_quote_comma > li_comma then
						...
						mylog.log(this, "u_component_message_handler_medware.parse_csv:0116", "A field of form 'x,x' does not have an ending quote PARSING FAILED for (" + is_stg + ")",4)
						RETURN -1
					end if
				else
					li_length = li_end - li_start
	...
	ELSE
		...
		IF li_ctr = 1 THEN
			...
			mylog.log(this, "u_component_message_handler_medware.parse_csv:0134", "Never found an occurrance of quote-comma-quote in the input value, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
			RETURN -1
		ELSE
			...
			IF li_length > li_value_max_len THEN 
				mylog.log(this, "u_component_message_handler_medware.parse_csv:0143", "A field length exceeds maximum number of characters,string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
				RETURN -1
			END IF	
			ls_value = mid(is_stg,li_start,li_length)		// Assign the currently selected value to ls_value
			is_array [li_ctr] = ls_value					//	Assign ls_value to the current array value
	...
	IF li_ctr > li_max_fields THEN
		mylog.log(this, "u_component_message_handler_medware.parse_csv:0155", "Number of fields in record " + string(li_ctr) + " exceeds the maximum of " + string(li_max_fields) + " PARSING FAILED for (" + is_stg + ", " + + ")",4)
		RETURN -1
	END IF
LOOP	
...
IF lb_empty_flag THEN
	mylog.log(this, "u_component_message_handler_medware.parse_csv:0179", "None of the fields in record contains an entry, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
	RETURN -1	
END IF
		...
		IF li_sts_stg > 0 THEN
			...
			IF ll_str_len > 0 then
				...
				IF li_pos = 1 THEN 
					li_start = li_pos + 1		// If so, it;s OK to start
				ELSE
					mylog.log(this, "u_component_message_handler_medware.xx_handle_message:0065", "The input string does not start with a quote, string PARSING FAILED for (" + is_stg + ", " + + ")",4)
					RETURN -1
				END IF
				...
				IF li_comma > 0 THEN				// We're still in a valid string
					...
					IF li_length > li_value_max_len THEN 
						mylog.log(this, "u_component_message_handler_medware.xx_handle_message:0073", "A field length exceeds maximum number of characters, string PARSING FAILED for (" + is_stg + ", " + string(li_value_max_len) + ")",4)
						RETURN -1
					END IF	
...
IF li_filenum >0 THEN
	...
	IF li_sts < 1 THEN
		mylog.log(this, "u_component_message_handler_medware.xx_handle_message:0098", "Unable to close the medware.mmi incoming transfer file...Aborting Encounter Creation for Message ID (" + string(li_message_id) + ")", 4)
		RETURN -1									
	END IF
END IF
...
if isnull(ls_billing_id) or ls_billing_id = '' then
	mylog.log(this, "u_component_message_handler_medware.medware_patient:0052", "no patient billing_id " + is_array[2] + "," + is_array[3], 2)
	return 1
end if

mylog.log(this, "u_component_message_handler_medware.medware_patient:0056", "The patient billing_id=" + ls_billing_id, 2)
SELECT cpr_id
INTO :ls_cpr_id
FROM p_Patient
...
if (is_array[6] = "0" or isnull(is_array[6]) or is_array[6] = "") then
	ls_error = is_array[6]
	mylog.log(this, "u_component_message_handler_medware.medware_patient:0092", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
...
//reformat... try mmddyyyy	
		...
		if isdate(fix_date) then
			...
			ls_error = is_array[6]
			mylog.log(this, "u_component_message_handler_medware.medware_patient:0107", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
			setnull(ld_birthdate)
			setnull(ldt_date_of_birth)
		end if	
	end if
...
if isnull(ls_cpr_id) then
	mylog.log(this, "u_component_message_handler_medware.medware_patient:0128", "create new patient, lname=" + ls_last_name, 1)
...
mylog.log(this, "u_component_message_handler_medware.medware_patient:0166", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)

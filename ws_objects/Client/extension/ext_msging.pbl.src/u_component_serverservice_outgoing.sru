...
if isnull(ls_transport_component_id) then
	log.log(this, "u_component_serverservice_outgoing.get_transport:0023", "Null transport component for subscription (" + string(pl_subscription_id) + ")", 4)
	return luo_outgoing
end if

luo_outgoing = my_component_manager.get_component(ls_transport_component_id)
if isnull(luo_outgoing) then
	log.log(this, "u_component_serverservice_outgoing.get_transport:0029", "Error getting transport component (" + ls_transport_component_id + ")", 4)
	return luo_outgoing
end if
...
If ll_count > 0 Then
		...
		if isnull(luo_transport) then
			log.log(this, "u_component_serverservice_outgoing.timer_ding:0024", "Error getting transport component (" + string(ll_subscription_id) + ")", 4)
			return -1
		end if
	
		li_sts = luo_transport.send_file(ll_message_id)
		if li_sts <= 0 then
			log.log(this, "u_component_serverservice_outgoing.timer_ding:0030", "Error sending (" + string(ll_message_id) + ")", 4)
			return -1
		end if

...
if isnull(is_ClinicCodes[1]) or is_ClinicCodes[1] = "" then
	mylog.log(this, "u_component_schedule_medicat.xx_initialize:0035", "ERROR: Schedule Clinic Code not entered.", 4)
	ii_office_count = 0
	return -1
end if	
		...
		If not Isnull(thisnull) then
			for li_count = 1 to ii_office_count
				if is_ClinicCodes[li_count] = ls_facilitycode then
					...
					mylog.log(this, "u_component_schedule_medicat.xx_initialize:0063", "office=" + ls_office + ", facility=" + ls_facilitycode,2)
					exit
				end if	
			next
		end if

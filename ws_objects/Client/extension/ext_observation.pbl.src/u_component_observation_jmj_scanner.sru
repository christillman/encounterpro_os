...
/*	if ll_result = 0 then
		log.log(this, "u_component_observation_jmj_scanner.load_and_annotate_file:0033", "RunTiffDLL returned zero.  Check tiffdll license (" + ls_param + ")", 4)
		return -1
	end if */
	if ll_result < 0 then
		log.log(this, "u_component_observation_jmj_scanner.load_and_annotate_file:0037", "RunTiffDLL failed (" + string(ll_result) + ", " + ls_param + ")", 4)
		return -1
	end if
...
if li_sts <= 0 then
	log.log(this, "u_component_observation_jmj_scanner.load_and_annotate_file:0049", "Read into blob failed", 4)
	return -1
end if
...
if lb_reverse then
	...
	for i = 1 to popup_return.item_count
		li_sts = add_attachment(observation_count, popup_return.items[i])
		if li_sts < 0 then
			log.log(this, "u_component_observation_jmj_scanner.xx_do_source:0053", "Error loading scanned file", 4)
			return -1
		end if
	next
end if
...
DO WHILE true
	...
	If li_sts <= 0 Then
		...
		elseif li_choice = 2 then
			...
			if li_sts <= 0 then
				log.log(this, "u_component_observation_jmj_scanner.add_attachment:0064", "Read into blob failed", 4)
				return -1
			end if
			exit
		else

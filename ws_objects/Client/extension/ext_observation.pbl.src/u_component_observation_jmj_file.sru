...
if pi_status > 0 then
	if not isnull(ls_move_to_success) then
		...
		if li_sts<= 0 then
			log.log(this, "u_component_observation_jmj_file.xx_set_processed:0035", "Error moving file (" + ps_id + ", " + ls_temp + ", " + string(li_sts) + ")", 4)
		end if
	elseif lb_delete_file then
		// If we're supposed to delete it then delete it
		if not filedelete(ps_id) then
			log.log(this, "u_component_observation_jmj_file.xx_set_processed:0040", "Error deleting file (" + ps_id + ")", 4)
			li_sts = -1
		end if
	end if
else
	if not isnull(ls_move_to_error) then
		...
		li_sts = filemove(ps_id, ls_temp)
		if li_sts<= 0 then
			log.log(this, "u_component_observation_jmj_file.xx_set_processed:0049", "Error moving file (" + ps_id + ", " + ls_temp + ", " + string(li_sts) + ")", 4)
		end if
	elseif lb_delete_file then
		// If we're supposed to delete it then delete it
		if not filedelete(ps_id) then
			log.log(this, "u_component_observation_jmj_file.xx_set_processed:0054", "Error deleting file (" + ps_id + ")", 4)
			li_sts = -1
		end if
	end if
end if
...
//		set_processed(lsa_paths[i], -1)
	...
	if li_sts > 0 then
		...
		if lb_read_xml  then
			...
			if li_sts2 < 0 then
				log.log(this, "u_component_observation_jmj_file.get_attachments:0138", "Error reading xml document (" + lsa_paths[i] + ")", 4)

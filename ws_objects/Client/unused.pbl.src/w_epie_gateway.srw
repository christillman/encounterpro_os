...
TRY
	...
	if now() > DailyMaintenanceTime and date(last_dailymaintenance) < today() then
		...
		if li_sts <= 0 then
			log.log(this, "w_epie_gateway:time", "Error doing daily maintenance", 4)
		end if
	end if
	...
	if li_sts <= 0 then
		log.log(this, "w_epie_gateway:time", "Error doing epie download", 4)
	end if
CATCH (throwable lo_error)
	...
	log.log(this, "w_epie_gateway:time", ls_error, 4)
	li_sts = -1
FINALLY

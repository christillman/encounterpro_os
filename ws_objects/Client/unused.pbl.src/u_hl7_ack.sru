...
//if li_sts <> 0 then
//	mylog.log(this, "u_hl7_ack.app_ack:0007", "ERROR: connection to AHC Messenger object failed.", 4)
//	DESTROY oAHC_ack
//	setnull(oAHC_Ack)
//	return -1
//end if	
...
//	mylog.log(this, "u_hl7_ack.app_ack:0020", "ERROR: connection to IHCMessageManager object failed.", 4)
//	return -1
//end if
//mylog.log(this, "u_hl7_ack.app_ack:0023", "connection to IHCMessageManager object.", 2)
...
if li_sts < 0 then
	mylog.log(this, "u_hl7_ack.ack:0061", "error native pointer to message object ", 4)	
	return -1
end if
...
if bs_system = 'IDX' or bs_system = 'MEDIC' then 
	...
	if lb_ackfile then
		...
		if isnull(dcom_app_port) then
			mylog.log(this, "u_hl7_ack.ack:0096", "error no port specified for ack ", 4)	
			return -1
		end if	
		oleobject jsocket
		...
		if li_sts <> 0 then
			DESTROY jsocket
			setnull(jsocket)
			mylog.log(this, "u_hl7_ack.ack:0106", "ERROR: connection to jsocket object failed.", 4)
			return -1
		end if
		...
		if socket_return = 'Success' then
			...
			mylog.log(this,"u_hl7_ack.ack:0113","error with send to jsocket " + socket_return,4)
			li_sts = -1
		end if
		DESTROY jsocket
		setnull(jsocket)
...
if li_sts < 0 then
	mylog.log(this, "u_hl7_ack.ack:0124", "error native pointer to message object ", 4)	
	return -1
end if
...
mylog.log(this, "u_hl7_ack.ack:0142", "sendmessage to" + is_FromApp + " from " + ack_appl, 1)	

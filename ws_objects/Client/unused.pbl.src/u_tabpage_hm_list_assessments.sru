$PBExportHeader$u_tabpage_hm_list_assessments.sru
forward
global type u_tabpage_hm_list_assessments from u_tabpage_hm_class_base
end type
type st_further from statictext within u_tabpage_hm_list_assessments
end type
type dw_assessments from u_dw_pick_list within u_tabpage_hm_list_assessments
end type
type st_title from statictext within u_tabpage_hm_list_assessments
end type
type cb_add_assessment from commandbutton within u_tabpage_hm_list_assessments
end type
type dw_rule_assessments from u_dw_pick_list within u_tabpage_hm_list_assessments
end type
type st_rule_title from statictext within u_tabpage_hm_list_assessments
end type
end forward

global type u_tabpage_hm_list_assessments from u_tabpage_hm_class_base
integer width = 3063
st_further st_further
dw_assessments dw_assessments
st_title st_title
cb_add_assessment cb_add_assessment
dw_rule_assessments dw_rule_assessments
st_rule_title st_rule_title
end type
global u_tabpage_hm_list_assessments u_tabpage_hm_list_assessments
...
for i = 1 to lstr_assessments.assessment_count
	...
	if sqlca.sqlcode = 100 then
		log.log(this, "u_tabpage_hm_list_assessments.add_assessment:0038", "assessment_type not found for assessment (" + lstr_assessments.assessments[i].assessment_id + ")", 4)
		return -1
	end if
...
if HMClassTab.hm_class.filter_maintenance_rule_id > 0 then
	li_sts = dw_rule_assessments.retrieve(HMClassTab.hm_class.filter_maintenance_rule_id)
end if
...
ll_dw_height = (height - 650) / 2

st_rule_title.x = (width - st_rule_title.width) / 2

dw_rule_assessments.x = ((width - dw_rule_assessments.width) / 2) + 50
dw_rule_assessments.height = ll_dw_height

st_further.x = (width - st_further.width) / 2
st_further.y = dw_rule_assessments.y + dw_rule_assessments.height + 50

dw_assessments.x = ((width - dw_assessments.width) / 2) + 50
dw_assessments.y = st_further.y + st_further.height + 20
dw_assessments.height = ll_dw_height

cb_add_assessment.x = (width - cb_add_assessment.width) / 2
cb_add_assessment.y = dw_assessments.y + dw_assessments.height + 24

dw_assessments.settransobject(sqlca)
dw_rule_assessments.settransobject(sqlca)
...
on u_tabpage_hm_list_assessments.create
int iCurrent
call super::create
this.st_further=create st_further
this.dw_assessments=create dw_assessments
this.st_title=create st_title
this.cb_add_assessment=create cb_add_assessment
this.dw_rule_assessments=create dw_rule_assessments
this.st_rule_title=create st_rule_title
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_further
this.Control[iCurrent+2]=this.dw_assessments
this.Control[iCurrent+3]=this.st_title
this.Control[iCurrent+4]=this.cb_add_assessment
this.Control[iCurrent+5]=this.dw_rule_assessments
this.Control[iCurrent+6]=this.st_rule_title
end on

on u_tabpage_hm_list_assessments.destroy
call super::destroy
destroy(this.st_further)
destroy(this.dw_assessments)
destroy(this.st_title)
destroy(this.cb_add_assessment)
destroy(this.dw_rule_assessments)
destroy(this.st_rule_title)
end on
...
string text = "Further filter the Maintenance Rule patients by also requiring at least one of the following assessments"
...
integer height = 400
integer taborder = 10
string dataobject = "dw_maintenance_rule_assessments_display"
boolean vscrollbar = true
end type
...
long textcolor = 33554432
long backcolor = 33538240
string text = "The Maintenance Rule requires that patients must have at least one of the following assessments"
alignment alignment = center!
boolean focusrectangle = false
end type

HA$PBExportHeader$u_component_route_dotnet.sru
forward
...
CATCH (oleruntimeerror lt_error)
	...
	log.log(this, "xx_do_source()", "Error calling SendDocument:~r~n" + ls_message, 4)
...
if ll_return_status <= 0 OR isnull(ll_return_status)  then
	...
	log.log(this, "xx_send_document()", ls_message, 4)
	return -1
end if
...
if li_sts <= 0 then
	log.log(this, "add_document_to_componentdata()", "Error sending document (" + string(puo_document.patient_workplan_item_id) + ")", 4)
	return -1
end if
	...
	CASE "base64"
		TRY
			...
			if isnull(ls_encoded_data) or len(ls_encoded_data) = 0 then
				log.log(this, "add_document_to_componentdata()", "Error converting file data to base64 (" + string(lstr_document.attachment) + ")", 4)
				return -1
			end if
		CATCH (oleruntimeerror lt_error)
			log.log(this, "add_document_to_componentdata()", "Error converting document datar ~r~n" + lt_error.text + "~r~n" + lt_error.description, 4)
		END TRY
	CASE "hex", "binhex"
		...
		TRY
			...
			if isnull(ls_encoded_data) or len(ls_encoded_data) = 0 then
				log.log(this, "add_document_to_componentdata()", "Error converting file data to hex (" + string(lstr_document.attachment) + ")", 4)
				return -1
			end if
		CATCH (oleruntimeerror lt_error2)
			log.log(this, "add_document_to_componentdata()", "Error converting document datar ~r~n" + lt_error2.text + "~r~n" + lt_error2.description, 4)
		END TRY
	CASE "string"

HA$PBExportHeader$u_component_message_handler_lytec.sru
forward
...
IF li_filenum >0 THEN
	...
	IF li_sts < 1 THEN
		mylog.log(this, "xx_handle_message()", "Unable to close the lytec.mmi incoming transfer file...Aborting Encounter Creation for Message ID (" + string(li_message_id) + ")", 4)
		RETURN -1									
	END IF
	...
	IF NOT lb_sts THEN
		mylog.log(this, "xx_handle_message()", "Unable to delete the lytec.mmi incoming transfer file...Aborting Encounter Creation for Message ID (" + string(li_message_id) + ")", 4)
		RETURN -1									
	END IF
END IF
...
li_sts = parse_csv(10)
IF li_sts < 0 THEN
	mylog.log(this, "lytec_patient()", "The parse_csv() function failed Aborting Encounter Creation for Message ID (" + string(ii_message_id) + ")", 4)
	RETURN -1
END IF	

ls_billing_id = is_array[1] 
mylog.log(this, "lytec_patient()", "The patient billing_id=" + ls_billing_id, 2)
...
if (is_array[7] = "0" or isnull(is_array[7]) or is_array[7] = "") then
	ls_error = is_array[7]
	mylog.log(this, "lytec_patient()", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
	...
	setnull(ld_birthdate)
		...
		ls_error = is_array[7]
		mylog.log(this, "lytec_patient()", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
		return -1
	end if
end if
...
if isnull(ls_cpr_id) then
	mylog.log(this, "lytec_patient()", "create new patient, lname=" + ls_last_name, 2)
...
mylog.log(this, "lytec_patient()", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)

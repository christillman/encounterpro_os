HA$PBExportHeader$u_component_send_report_lcr.sru
forward
...
if isnull(ls_cpr_id) then
	log.log(this,"xx_sendreport","no cpr id",4)
	Return -1
end if
...
if isnull(ls_cpr_id) then
	log.log(this,"xx_sendreport","no encounter id",4)
	Return -1
end if
...
if isnull(ls_external_id) or len(ls_external_id) = 0 then
	log.log(this,"xx_sendreport","NO billing id:LCR Report for encounter ("+ls_cpr_id+","+string(ll_encounter_id)+") failed ",4)
	return -1	
end if
...
if isnull(ls_report_text) or len(ls_report_text) = 0 then
	log.log(this,"xx_sendreport","ERROR: LCR Report for encounter ("+ls_cpr_id+","+string(ll_encounter_id)+") failed",4)
	Return -1
else
	log.log(this,"xx_sendreport","LCR Report for encounter ("+ls_cpr_id+","+string(ll_encounter_id)+") "+ls_report_text,1)
end if
...
If ll_linecount > 150 Then
	...
	Do while ll_pos > 0
		...
		If len(ls_record) > 0 Then
			log.log(this,"xx_sendreport","Logging LCR report for encounter ("+ls_cpr_id+","+string(ll_encounter_id)+") "+ls_record,1)
			li_sts = log_message(ls_temp1)
		End If

		If li_sts <= 0 Then
			log.log(this,"xx_sendreport","Error logging report for encounter ("+ls_cpr_id+","+string(ll_encounter_id)+") ",4)
			Return -1
		End If

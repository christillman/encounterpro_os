HA$PBExportHeader$u_component_message_handler_qrs.sru
forward
...
IF li_sts < 0 THEN
		mylog.log(this, "qrs_diagnosis()", "The parse_csv() function failed Aborting Encounter Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF
ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
		mylog.log(this, "qrs diagnosis()", "The parse_csv() function failed Message ID (" + string(message_id) + ")", 4)
		RETURN -1
else 
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "qrs_procedure()", "The parse_csv() function failed Aborting Encounter Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF
ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
		mylog.log(this, "qrs procedure()", "The parse_csv() function failed Message ID (" + string(message_id) + ")", 4)
		RETURN -1
else 
	li_sts = ll_array_count
END IF	
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
	mylog.log(this, "qrs_patient()", "The parse_csv() function failed Aborting Encounter Creation)", 4)
	RETURN -1
END IF	

ls_billing_id = is_array[1] 
mylog.log(this, "qrs_patient()", "The patient billing_id=" + ls_billing_id, 2)
SELECT cpr_id
INTO :ls_cpr_id
FROM p_Patient
...
if (is_array[7] = "0" or isnull(is_array[7]) or is_array[7] = "") then
	ls_error = is_array[7]
	mylog.log(this, "qrs_patient()", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 3)
...
//reformat... try yyyymmdd	
		...
		if isdate(fix_date) then
			...
			ls_error = is_array[7]
			mylog.log(this, "qrs_patient()", "birthdate error=" + ls_error + " (" + is_stg + ", " + + ")", 4)
			setnull(ld_birthdate)
			setnull(ldt_date_of_birth)
		end if	
	end if
...
if isnull(ls_cpr_id) then
	mylog.log(this, "qrs_patient()", "create new patient, lname=" + ls_last_name, 1)
...
mylog.log(this, "qrs_patient()", "The patient funcion complete, cpr_id=" + ls_cpr_id, 1)
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "qrs insurance()", "The parse_csv() function failed Aborting Encounter Creation for Message ID (" + string(message_id) + ")", 4)
		RETURN -1
END IF	

ll_array_count = UpperBound(is_array)
if (isnull(ll_array_count) or ll_array_count = -1) then 
		mylog.log(this, "qrs insurance()", "The parse_csv() function failed Message ID ("+ string(message_id) + ")", 4)
		RETURN -1
else 
	li_sts = ll_array_count
END IF	
...
if ls_insurance_type = '' then
	...
	if not cprdb.check() then return -1
	if cprdb.sqlcode = 100 then 
		mylog.log(this, "qrs insurance()", "Patient does not exist for  ("+ ls_billing_id + ")", 4)
		RETURN -1
	end if
...
li_sts = parse_csv(255)
IF li_sts < 0 THEN
		mylog.log(this, "qrs arrived()", "The parse_csv() function failed Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
		RETURN -1
END IF	
...
If isnull(ls_billing_id) or ls_billing_id = "" then
	mylog.log(this, "qrs arrived()", "Billingid not found Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
END IF	
If isnumber(ls_billing_id) then
	ll_encounter_billing_id = long(ls_billing_id)
else
	mylog.log(this, "qrs arrived()", "Billingid not numeric Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
END IF
...
//	billing_id value
			...
			IF cprdb.sqlcode = 100 THEN
				...
				mylog.log(this, "qrs_arrived()", "Unable to retrieve a CPR_ID to match the Billing_ID..Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
		 		RETURN 1
			END IF
...
If Isnull(ls_primary_provider_id) then
	ls_error = is_array[4]
	mylog.log(this, "qrs_arrived()", "Unable to retrieve a provider_ID to match the input provider_ID " + ls_error + " ..Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(message_id) + ")", 4)
	RETURN -1
End if			
...
//	It will be processed by the Scheduler
			...
			IF NOT cprdb.check() THEN 
				...
				mylog.log(this, "xx_qrs_arrived()", "Unable write a record to x_qrs_Arrived...Aborting Encounter Creation for Billing ID, Message ID (" + ls_billing_id + ", " + string(li_message_id) + ")", 4)
				RETURN -1									
			END IF

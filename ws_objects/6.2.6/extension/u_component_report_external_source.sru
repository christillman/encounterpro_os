HA$PBExportHeader$u_component_report_external_source.sru
forward
...
if isnull(ls_external_source) then
	log.log(this, "xx_printreport()", "Null external source", 4)
	return -1
end if
...
if li_sts <= 0 then
	...
	log.log(this, "xx_printreport()", ls_message, 4)
	return -1
end if
...
if isnull(luo_external_source) then
	log.log(this, "xx_printreport()", "error getting component (" + ls_component_id + ")", 4)
	return -1
end if

li_sts = luo_external_source.do_source()
if li_sts < 0 then
	log.log(this, "xx_printreport()", "Error calling external source (" + ls_external_source + ")", 4)
	return -1
end if

HA$PBExportHeader$u_hl7_arrived_mpm.sru
forward
...
If is_msgtype = "SIU" then
	...
	if upper(ls_arrived) <> 'ARRIVED' then 
		mylog.log(this, "arrived_mpm()", "SIU not arrived (" + ls_arrived  + ")", 3)
		return 1
	end if	
...
Else	
	...
	If cprdb.sqlcode = 100 then
		...
		If cprdb.sqlcode = 100 then
			...
			mylog.log(this, "arrived_mpm()", "appointment type ("+ls_appointment_type+") is not mapped; Now Adding ..",3)
			Setnull(ls_encounter_type)
			Setnull(ls_new_flag)
	...
	if isnull(ls_encounter_type) or len(ls_encounter_type) = 0 Then 
		...
		mylog.log(this, "arrived_mpm()", "Appointment Type, Encounter type (" + ls_appointment_type +", "+ls_encounter_type+" )" , 2)
	end if
End If
...
If not isnull(ls_resourcetext) AND trim(ls_resourcetext) <> "" then
	mylog.log(this, "arrived_mpm()", "Resource Id and text (" + ls_resourceid + ", "+ ls_resourcetext +")" , 2)
	mylog.log(this, "arrived_mpm()", "Appointment Type (" + ls_appointment_type +")" , 2)
	...
	// min of Sort Sequence
		...
		// get the encounter type for a resource	
			...
			If not cprdb.check() then Return -1
			If cprdb.sqlcode = 100 Then
				mylog.log(this, "arrived_mpm()", "Resource not mapped ( " + ls_resourceid + "," + ls_resourcetext + ")", 3)
			Else
				lb_resource = true
			End if
		End If
...
mylog.log(this, "arrived_mpm()", "Encounter arrived for ("+ls_cprid+")", 1)

HA$PBExportHeader$u_component_property.sru
forward
...
if isnull(pl_property_id) then
	mylog.log(this, "execute_script()", "Null Property_id", 4)
	return ls_property
end if

property = datalist.find_property(pl_property_id)
if isnull(property.property_id) then
	mylog.log(this, "execute_script()", "Property not found (" + string(pl_property_id) + ")", 4)
	return ls_property
end if

if isnull(property.status) or property.status <> "OK" then
	mylog.log(this, "execute_script()", "Property Status not OK", 4)
	return ls_property
end if

li_sts = common_thread.get_adodb(adodb)
if li_sts <= 0 then
	mylog.log(this, "execute_script()", "Unable to establish ADO Connection", 4)
	return ls_property
end if

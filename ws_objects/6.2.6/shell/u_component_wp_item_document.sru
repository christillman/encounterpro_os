HA$PBExportHeader$u_component_wp_item_document.sru
forward
...
if isnull(ls_report_id) then
	...
	if isnull(ll_material_id) then
		log.log(this, "document_create()", "No report_id", 4)
...
CHOOSE CASE lower(ls_document_type)
	CASE "report"
		...
		if li_sts < 0 then
			log.log(this, "document_create()", "Error running report (" + ls_report_id + ")", 4)
			document_error("Create")
			return -1
		end if
		if isnull(lstr_document_file.attachment) or len(lstr_document_file.attachment) <= 0 then
			log.log(this, "document_create()", "Document ran successfully but returned no data (" + ls_report_id + ")", 3)
			return 0
		end if
		...
		if ll_attachment_id < 0 then
			log.log(this, "document_create()", "Error creating new attachment", 4)
			document_error("Create")
			return -1
		end if
		...
		if li_sts < 0 then
			log.log(this, "document_create()", "Error setting 'created' progress", 4)
			document_error("Create")
			return -1
		end if
...
if li_sts < 0 then
	log.log(this, "document_create()", "Error setting 'cancelled' progress", 4)
	return -1
end if
...
if li_sts < 0 then
	log.log(this, "document_confirm_receipt()", "Error setting 'Success' progress", 4)
	return -1
end if
...
if isnull(ls_send_from) then
	...
	if sqlca.sqlnrows <> 1 then
		log.log(this, "document_send()", "Invalid dispatch_method (" + dispatch_method + ")", 4)
		return -1
	end if
end if
...
if cpr_mode = "CLIENT" THEN
	...
	if lower(ls_send_from) = "client" then
		log.log(this, "document_send()", "Unable to send this document because it must be sent from a client (" + string(patient_workplan_item_id) + ")", 4)
		return -1
	else
		lb_send_from_here = true
	end if
...
if isnull(dispatch_method) then
	...
	if not tf_check() then return -1
	if sqlca.sqlnrows = 0 then
		log.log(this, "run_report()", "No dispatch_method is supplied and this recipients actor_class does not imply one", 4)
		return -1
	end if
...
if not tf_check() then return -1
if sqlca.sqlnrows = 0 then
	log.log(this, "run_report()", "dispatch_method not valid (" + dispatch_method + ")", 4)
	return -1
end if
...
if lower(ls_this_document_format) <> lower(ls_required_document_format) and not viewing_document then
	...
	if isnull(ll_max_attribute_sequence) then
		log.log(this, "run_report()", "A " + ls_required_document_format + " readable document is required", 4)
		return -1
	end if
...
If Isnull(ls_component_id) Then
	mylog.log(this, "run_report()", "Null component_id (" + ps_report_id + ")", 4)
	Return -1
Else
...
mylog.log(this, "run_report()","processing report ("+ps_report_id+") on mode "+cpr_mode,1)
...
if luo_report.printreport_status > 0 then
	if len(luo_report.document_file.attachment) > 0 then
		...
		log.log(this, "run_report()", "The report/datafile returned success but no report document is found + (" + ps_report_id + ")", 4)
		li_sts = -1
	end if
elseif luo_report.printreport_status = 0 then
	...
	if lb_auto_cancel_when_no_document then
		...
		log.log(this, "run_report()", "The report/datafile returned no documents + (" + ps_report_id + ")", 4)
		li_sts = -1
	end if
else
	log.log(this, "run_report()", "The report/datafile returned an error + (" + ps_report_id + ")", 4)
	li_sts = -1
end if
...
if li_sts < 0 then
	log.log(this, "document_sign()", "Error setting 'signed' progress", 4)
	return -1
end if
...
if isnull(ls_report_id) then
	...
	get_attribute("material_id", ll_material_id)
	if isnull(ll_material_id) then
		log.log(this, "document_subtype()", "No report_id or material_id", 4)
		return ls_null
	end if
	return "Material"
else
...
if li_sts < 0 then
	log.log(this, "document_send()", "Error setting 'Ready' progress (" + string(patient_workplan_item_id) + ")", 4)
	return -1
end if
...
if li_sts <= 0 then
	log.log(this, "document_send()", "Error sending document: Unable to get sender component (" + string(patient_workplan_item_id) + ")", 4)
	document_error("Send")
	return -1
end if
...
if li_sts < 0 then
	log.log(this, "document_send()", "Error sending document: sender component returned error (" + string(patient_workplan_item_id) + ")", 4)
	document_error("Send")
	return -1
elseif li_sts > 0 then
	...
	if li_sts < 0 then
		log.log(this, "document_send()", "Error setting 'sent' progress", 4)
		document_error("Send")
		return -1
	end if
end if
...
//						log.log(this, "document_send()", "Document created but not found", 4)
//						return -1
//					end if
//				else
//					log.log(this, "document_send()", "Document not found", 4)
//					return -1
//				end if
//			end if
//		elseif isnull(ll_attachment_id) then
//			log.log(this, "document_send()", "Status is 'Ready' but document not found (" + string(patient_workplan_item_id) + ")", 4)
//			return -1
//		end if
//		
//		SELECT ordered_for,
...
//				if li_sts <= 0 then
//					log.log(this, "document_send()", "Error printing attachment", 4)
...
//					log.log(this, "document_send()", dispatch_method + " dispatch_method does not have a valid send-to-addressee", 4)
//					return -1
//				end if
//
//				li_sts = f_get_attachment(ll_attachment_id, lstr_attachment)
//				if li_sts <= 0 then
//					log.log(this, "document_send()", "Error getting attacment file", 4)
//					return -1
//				end if
//				
...
//				if isnull(ls_temp) then
//					log.log(this, "document_send()", dispatch_method + " dispatch_method is not properly configured - missing attachment_location_id", 4)
//					return -1
//				end if
...
//					log.log(this, "document_send()", dispatch_method + " dispatch_method is not properly configured - missing directory", 4)
//					return -1
//				end if
//				
...
//					log.log(this, "document_send()", dispatch_method + " dispatch_method is not properly configured - invalid attachment location", 4)
//					return -1
//				end if
//				
...
//				if li_sts < 0 then
//					log.log(this, "document_send()", dispatch_method + "Error saving document (" + ls_save_path + ")", 4)
//					return -1
//				end if
//
//			CASE "sender_epie"
...
//					log.log(this, "document_send()", "This installation is not licensed to send documents via eMail", 4)
//					if cpr_mode = "CLIENT" then
...
//					log.log(this, "document_send()", "This installation is not licensed to send documents via Fax", 4)
//					if cpr_mode = "CLIENT" then
...
//					log.log(this, "document_send()", dispatch_method + " dispatch_method does not have a valid document type", 4)
//					return -1
//				end if
//				
//				li_sts = f_get_attachment(ll_attachment_id, lstr_attachment)
...
//				if li_sts <= 0 then
//					log.log(this, "document_send()", "Error printing patient material", 4)
//					f_please_wait_close(li_wait)
//					return -1
//				end if
//				if len(ls_printer) > 0 then
...
//					log.log(this, "document_send()", "Error getting material file", 4)
//					return -1
//				end if
//				
//				luo_data = CREATE u_ds_data
...
if isnull(ps_dispatch_method) then
	log.log(this, "route_component()", "No dispatch_method has been set for this document (" + string(patient_workplan_item_id) + ")", 4)
	return -1
end if
...
if not tf_check() then return -1
if sqlca.sqlnrows <> 1 then
	log.log(this, "route_component()", "Route not found for this document (" + string(patient_workplan_item_id) + ", " + ps_dispatch_method + ")", 4)
	return -1
end if

puo_route = component_manager.get_component(ls_component_id)
if isnull(puo_route) then
	log.log(this, "route_component()", "Error getting route component (" +  string(patient_workplan_item_id) + ", " + ls_component_id + ")", 4)
	return -1
end if
		...
		if not document_created or failed_mappings then
			...
			if failed_mappings then
				...
				if not lb_force_send then
					log.log(this, "document_send()", "Document mapping failed.  Document will not be sent (" + string(patient_workplan_item_id) + ")", 4)
...
if li_sts < 0 then
	log.log(this, "document_reset()", "Error resetting document", 4)
	return -1
end if

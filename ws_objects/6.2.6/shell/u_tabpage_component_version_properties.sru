HA$PBExportHeader$u_tabpage_component_version_properties.sru
forward
...
if sqlca.sqlnrows = 0 then
	log.log(this, "upgrade_component()", "Component not found (" + component_version.component_id + ")", 4)
	return -1
end if
...
openwithparm(w_pop_yes_no, "Would you like to install version " + component_version.version_name + " on this computer now?")
popup_return = message.powerobjectparm
if popup_return.item = "YES" then
	li_sts = component_manager.install_component_version(component_version.component_id, component_version.version_name)
	if li_sts <= 0 then

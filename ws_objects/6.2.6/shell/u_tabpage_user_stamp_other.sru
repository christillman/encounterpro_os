HA$PBExportHeader$u_tabpage_user_stamp_other.sru
forward
...
if signature_changed then
	...
	if isnull(lbl_signature_stamp) then
		...
		CATCH (throwable lo_error)
			log.log(this, "save()", "Error converting user stamp to hex (" + lo_error.text + ")", 4)
			return -1
		END TRY
	end if
...
if len(ls_stamp_data) > 0 then
	...
	CATCH (throwable lo_error)
		log.log(this, "initialize()", "Error converting user stamp to binary (" + lo_error.text + ")", 4)
		return -1
	END TRY

HA$PBExportHeader$w_workplan_criteria.srw
forward
...
type st_authority_title from statictext within w_workplan_criteria
end type
type st_authority from statictext within w_workplan_criteria
end type
type cb_clear_authority from commandbutton within w_workplan_criteria
end type
type st_authority_category_title from statictext within w_workplan_criteria
end type
type st_authority_category from statictext within w_workplan_criteria
end type
type cb_clear_authority_category from commandbutton within w_workplan_criteria
end type
end forward
...
if isnull(criteria.authority_id) then
	...
	SELECT name
	INTO :st_authority.text
	FROM c_Authority
	WHERE authority_id = :criteria.authority_id;
	if not tf_check() then
		log.log(this, "open", "Error getting stage description", 4)
		close(this)
		return
...
criteria.authority_id = popup_return.items[1]
text = popup_return.descriptions[1]
cb_clear_authority.visible = true
...
popup.displaycolumn = 3
popup.argument_count = 1
popup.argument[1] = "PAYOR"
openwithparm(w_pop_pick, popup)
popup_return = message.powerobjectparm
if popup_return.item_count <> 1 then return

criteria.authority_category = popup_return.items[1]
text = popup_return.descriptions[1]

cb_clear_authority_category.visible = true

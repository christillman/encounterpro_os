HA$PBExportHeader$w_pop_buttons.srw
forward
...
for i = 1 to max_rows
	for j = 1 to max_buttons_on_row
		...
		if li_sts < 0 then
			log.log(this, "create_buttons()", "Error creating button object", 4)
			return -1
		end if
		if button_popup.button_titles_used then
			...
			if li_sts < 0 then
				log.log(this, "create_buttons()", "Error creating button title object", 4)
				return -1
			end if
			titles_used = true
		end if

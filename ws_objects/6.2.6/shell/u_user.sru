...
log.log(this, "logoff()", "User " + user_short_name + " logging off...", 2)
// Force this message to the o_log table
if log.dbloglevel > 2 then
	log.log_db(this, "logoff()", "User " + user_short_name + " logging off...", 2)
end if
...
if isnull(current_user) then
	log.log(this, "logoff()", "current_user is null", 3)
elseif current_user.user_id <> user_id then
	log.log(this, "logoff()", "current_user is " + current_user.user_short_name, 3)
end if
...
if ll_count = 0 then
	...
	log.log(this, "check_logon()", "User '" + user_full_name + "' is not logged in at computer # " + string(computer_id), 3)
...
if pi_comm_index <= 0 or pi_comm_index > communication_count then
	log.log(this, "update_communication()", "Invalid index", 4)
	return -1
end if
...
if pi_address_index <= 0 or pi_address_index > address_count then
	log.log(this, "update_address()", "Invalid index", 4)
	return -1
end if

HA$PBExportHeader$u_param_encounter_type.sru
forward
global type u_param_encounter_type from u_param_base
end type
type st_popup_values from statictext within u_param_encounter_type
end type
end forward
...
global u_param_encounter_type u_param_encounter_type
...
if preference_in_use then
	...
	ls_description = datalist.encounter_type_description(ls_initial_value)
...
popup.items[1] = "PICK"
popup.items[2] = "D"
openwithparm(lw_pick, popup, "w_pick_encounter_type")
ls_encounter_type = message.stringparm
if isnull(ls_encounter_type) or trim(ls_encounter_type) = "" then return 0

ls_display = datalist.encounter_type_description(ls_encounter_type)
param_value = ls_encounter_type
...
on u_param_encounter_type.create
int iCurrent
call super::create
...
on u_param_encounter_type.destroy
call super::destroy
destroy(this.st_popup_values)
end on

type st_preference from u_param_base`st_preference within u_param_encounter_type
end type

type st_preference_title from u_param_base`st_preference_title within u_param_encounter_type
end type

type cb_clear from u_param_base`cb_clear within u_param_encounter_type
end type
...
type st_required from u_param_base`st_required within u_param_encounter_type
integer x = 1358
integer y = 536
end type

type st_helptext from u_param_base`st_helptext within u_param_encounter_type
end type

type st_title from u_param_base`st_title within u_param_encounter_type
integer x = 46
integer y = 616
integer height = 76
end type

HA$PBExportHeader$w_observation_result_definition.srw
forward
...
if isnull(observation_id) or observation_id = "" then
	log.log(this, "load_result()", "Invalid observation id", 4)
	return -1
end if
...
if isnull(result_sequence) then
	log.log(this, "load_result()", "Invalid result sequence", 4)
	return -1
end if
...
if sqlca.sqlcode = 100 then
	log.log(this, "load_result()", "Result not found", 4)
	return -1
end if
...
if isnull(result_sequence) then
	...
	li_sts = load_result()
	if li_sts < 0 then
		log.log(this, "open", "Error loading result", 4)
		close(this)
		return
	end if
end if

HA$PBExportHeader$w_drug_package.srw
forward
...
if popup.data_row_count <> 2 then
	log.log(this, "open", "Invalid Parameter", 4)
	close(this)
	return
end if
...
if isnull(drug_id) then
	log.log(this, "open", "Null Drug ID", 4)
	close(this)
	return
end if

if isnull(package_id) then
	log.log(this, "open", "Null Package ID", 4)
	close(this)
	return
end if
...
if sqlca.sqlcode = 100 then
	log.log(this, "open", "Drug pakcage not found (" + drug_id + ", " + package_id + ")", 4)
	close(this)
	return
end if

HA$PBExportHeader$w_workplan_step_definition.srw
forward
...
type cbx_mutually_exclusive_items from checkbox within w_workplan_step_definition
end type
end forward
...
cb_ok cb_ok
cb_cancel cb_cancel
cbx_mutually_exclusive_items cbx_mutually_exclusive_items
end type
...
string ls_room_name
string ls_room_id
integer li_mutually_exclusive_items
...
if li_mutually_exclusive_items = 0 then
	cbx_mutually_exclusive_items.checked = false
else
	cbx_mutually_exclusive_items.checked = true
end if
...
CHOOSE CASE lsa_buttons[li_button_pressed]
	...
	CASE "EDIT"
		...
		lstr_criteria.authority_id = dw_workplan_items.object.authority_id[pl_row]
		lstr_criteria.authority_category = dw_workplan_items.object.authority_category[pl_row]
		...
		dw_workplan_items.object.authority_id[pl_row] = lstr_criteria.authority_id
		dw_workplan_items.object.authority_category[pl_row] = lstr_criteria.authority_category
	CASE "CONFIG"
...
this.cbx_mutually_exclusive_items=create cbx_mutually_exclusive_items
...
this.Control[iCurrent+21]=this.st_step_in_office_title
this.Control[iCurrent+22]=this.st_step_in_office_flag
this.Control[iCurrent+23]=this.cb_ok
this.Control[iCurrent+24]=this.cb_cancel
this.Control[iCurrent+25]=this.cbx_mutually_exclusive_items
end on
...
destroy(this.cbx_mutually_exclusive_items)
end on
...
long backcolor = 33538240
string text = "Mutually Exclusive Items (Only first qualifying item is dispatched)"
end type

event clicked;integer li_mutually_exclusive_items

if cbx_mutually_exclusive_items.checked then
	li_mutually_exclusive_items = 1
else
	li_mutually_exclusive_items = 0
end if

UPDATE s
SET mutually_exclusive_items = :li_mutually_exclusive_items

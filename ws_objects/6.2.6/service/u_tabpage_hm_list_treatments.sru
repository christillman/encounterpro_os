HA$PBExportHeader$u_tabpage_hm_list_treatments.sru
forward
global type u_tabpage_hm_list_treatments from u_tabpage_hm_class_base
end type
type st_further from statictext within u_tabpage_hm_list_treatments
end type
type st_rule_title from statictext within u_tabpage_hm_list_treatments
end type
type dw_rule_treatments from u_dw_pick_list within u_tabpage_hm_list_treatments
end type
type st_title from statictext within u_tabpage_hm_list_treatments
end type
type cb_add_treatment from commandbutton within u_tabpage_hm_list_treatments
end type
type dw_treatments from u_dw_pick_list within u_tabpage_hm_list_treatments
end type
end forward
...
dw_rule_treatments dw_rule_treatments
st_title st_title
cb_add_treatment cb_add_treatment
dw_treatments dw_treatments
end type
global u_tabpage_hm_list_treatments u_tabpage_hm_list_treatments
...
if HMClassTab.hm_class.filter_maintenance_rule_id > 0 then
	li_sts = dw_rule_treatments.retrieve(HMClassTab.hm_class.filter_maintenance_rule_id)
end if
...
dw_rule_treatments.x = ((width - dw_rule_treatments.width) / 2) + 50
dw_rule_treatments.height = ll_dw_height

st_further.x = (width - st_further.width) / 2
st_further.y = dw_rule_treatments.y + dw_rule_treatments.height + 50

dw_treatments.x = ((width - dw_treatments.width) / 2) + 50
dw_treatments.y = st_further.y + st_further.height + 20
dw_treatments.height = ll_dw_height

cb_add_treatment.x = (width - cb_add_treatment.width) / 2
cb_add_treatment.y = dw_treatments.y + dw_treatments.height + 24

dw_treatments.settransobject(sqlca)
dw_rule_treatments.settransobject(sqlca)
...
on u_tabpage_hm_list_treatments.create
int iCurrent
call super::create
this.st_further=create st_further
this.st_rule_title=create st_rule_title
this.dw_rule_treatments=create dw_rule_treatments
...
this.Control[iCurrent+2]=this.st_rule_title
this.Control[iCurrent+3]=this.dw_rule_treatments
this.Control[iCurrent+4]=this.st_title
this.Control[iCurrent+5]=this.cb_add_treatment
this.Control[iCurrent+6]=this.dw_treatments
end on

on u_tabpage_hm_list_treatments.destroy
call super::destroy
destroy(this.st_further)
destroy(this.st_rule_title)
destroy(this.dw_rule_treatments)
...
integer height = 448
integer taborder = 10
string dataobject = "dw_maintenance_rule_treatments_display"

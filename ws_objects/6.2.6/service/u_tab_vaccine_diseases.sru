HA$PBExportHeader$u_tab_vaccine_diseases.sru
forward
...
if newindex = 3 then
	if isnull(disease_id) then
		...
		if ll_row > 0 then
			...
			log.log(this, "selectionchanging", "Disease_id not found (" + string(disease_id) + ")", 4)
			selecttab(oldindex)
			return 1
		end if
	end if
...
DO
	...
	if not tf_check() then
		log.log(this, "clicked", "Error checking for duplicate vaccine_id", 4)
...
if li_sts < 0 then
	log.log(this, "clicked", "Error adding new vaccine", 4)
	openwithparm(w_pop_message, "Error adding vaccine")
	return
end if
...
if not (ld_age >= immunization_date_of_birth) then
	log.log(this, "clicked", "Age not valid", 3)
	return
end if

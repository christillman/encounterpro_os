HA$PBExportHeader$w_svc_patient_service_list.srw
forward
...
for i = 1 to ll_count
	...
	luo_service = service_list.get_service_component(ll_patient_workplan_item_id)
 	If isnull(luo_service) Then
		log.log(This, "clicked", "Error Getting Service (" + string(ll_patient_workplan_item_id) + ")", 4)
		Return -1
	End If
...
if li_sts <= 0 then
	log.log(this, "open", "Error loading service", 4)

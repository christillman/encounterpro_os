HA$PBExportHeader$w_pop_which_mode.srw
forward
global type w_pop_which_mode from window
end type
type st_version from statictext within w_pop_which_mode
end type
type cb_dbmaint from commandbutton within w_pop_which_mode
end type
type st_office_id_title from statictext within w_pop_which_mode
end type
type st_office_id from statictext within w_pop_which_mode
end type
type cb_delete from commandbutton within w_pop_which_mode
end type
type cb_edit from commandbutton within w_pop_which_mode
end type
type cb_new from commandbutton within w_pop_which_mode
end type
type st_db from statictext within w_pop_which_mode
end type
type st_server from statictext within w_pop_which_mode
end type
type st_db_title from statictext within w_pop_which_mode
end type
type st_server_title from statictext within w_pop_which_mode
end type
type dw_databases from u_dw_pick_list within w_pop_which_mode
end type
type cb_cancel from commandbutton within w_pop_which_mode
end type
type cb_2 from commandbutton within w_pop_which_mode
end type
type cb_1 from commandbutton within w_pop_which_mode
end type
type st_dbms_title from statictext within w_pop_which_mode
end type
type st_dbms from statictext within w_pop_which_mode
end type
end forward
...
global w_pop_which_mode w_pop_which_mode
...
if common_thread.default_database = default_database then
	ls_current = "*"
	db_selected(ll_row)
else
	ls_current = ""
end if
dw_databases.object.current[ll_row] = ls_current
...
for i = 1 to li_section_count
	...
	if common_thread.default_database = ls_sections[i] then
		ls_current = "*"
		db_selected(ll_row)
	else
		ls_current = ""
...
on w_pop_which_mode.create
this.st_version=create st_version
this.cb_dbmaint=create cb_dbmaint
...
on w_pop_which_mode.destroy
destroy(this.st_version)
destroy(this.cb_dbmaint)
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "DB Maintenance"
end type

event clicked;common_thread.default_database = selected_database

closewithreturn(parent, "DBMAINT")

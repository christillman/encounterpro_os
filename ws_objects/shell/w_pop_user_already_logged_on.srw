HA$PBExportHeader$w_pop_user_already_logged_on.srw
forward
global type w_pop_user_already_logged_on from window
end type
type st_1 from statictext within w_pop_user_already_logged_on
end type
type cb_ok from commandbutton within w_pop_user_already_logged_on
end type
type cb_override from commandbutton within w_pop_user_already_logged_on
end type
type dw_message from datawindow within w_pop_user_already_logged_on
end type
end forward

global type w_pop_user_already_logged_on from window
integer x = 434
integer y = 604
integer width = 2359
integer height = 1108
windowtype windowtype = response!
long backcolor = 33538240
st_1 st_1
cb_ok cb_ok
cb_override cb_override
dw_message dw_message
end type
global w_pop_user_already_logged_on w_pop_user_already_logged_on

event open;str_security_alert lstr_security_alert

lstr_security_alert = message.powerobjectparm

dw_message.reset()
dw_message.insertrow(0)
dw_message.setitem(1,1,lstr_security_alert.alert_message )

if lstr_security_alert.allow_override then
	cb_override.visible = true
else
	cb_override.visible = false
end if


end event

on w_pop_user_already_logged_on.create
this.st_1=create st_1
this.cb_ok=create cb_ok
this.cb_override=create cb_override
this.dw_message=create dw_message
this.Control[]={this.st_1,&
this.cb_ok,&
this.cb_override,&
this.dw_message}
end on

on w_pop_user_already_logged_on.destroy
destroy(this.st_1)
destroy(this.cb_ok)
destroy(this.cb_override)
destroy(this.dw_message)
end on

type st_1 from statictext within w_pop_user_already_logged_on
integer width = 2350
integer height = 136
integer textsize = -18
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 33554432
long backcolor = 33538240
string text = "Security Alert"
alignment alignment = center!
boolean focusrectangle = false
end type

type cb_ok from commandbutton within w_pop_user_already_logged_on
integer x = 1861
integer y = 940
integer width = 402
integer height = 112
integer taborder = 10
integer textsize = -10
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "OK"
boolean default = true
end type

event clicked;closewithreturn(parent, "OK")
end event

type cb_override from commandbutton within w_pop_user_already_logged_on
integer x = 46
integer y = 904
integer width = 297
integer height = 148
integer taborder = 20
integer textsize = -10
integer weight = 700
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Override"
end type

on clicked;str_popup_return popup_return

popup_return.item = "OVERRIDE"

closewithreturn(parent, popup_return)


end on

type dw_message from datawindow within w_pop_user_already_logged_on
integer x = 50
integer y = 200
integer width = 2258
integer height = 652
boolean enabled = false
string dataobject = "dw_security_alert_message"
boolean vscrollbar = true
boolean border = false
end type


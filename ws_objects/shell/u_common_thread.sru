HA$PBExportHeader$u_common_thread.sru
forward
...
// variables for keeping track of the current powerbuilder library search list
private string base_library_list
private string pblibrary_path_root
private str_pblibraries pblibraries

// Variables for keeping track of the current contraindication components
...
public subroutine set_library_list ()
public function integer install_library (str_component_definition pstr_component_definition, str_component_version pstr_component_version)
public function boolean is_library_installed (str_component_definition pstr_component_definition, str_component_version pstr_component_version)
public function str_pblibraries installed_libraries ()
public function integer install_debug_library (str_component_definition pstr_component, long pl_version, string ps_compile_name, string ps_debug_library_path)
private function integer set_library (str_pblibrary pstr_pblibrary)
public function integer get_library (string ps_component_id, long pl_version, ref str_pblibrary pstr_pblibrary)
public subroutine remove_all_libraries ()
private function integer remove_library (string ps_component_id, long pl_version)
end prototypes
...
if isnull(adodb) then
	...
	if li_sts < 0 then
		DESTROY adodb
		setnull(adodb)
		log.log(this, "get_adodb()", "Error creating connection object", 4)
		return -1
	end if
	...
	CATCH (throwable lo_error1)
		log.log(this, "get_adodb()", "Error opening ADODB Connection (" + lo_error1.text + ")", 3)
		li_sts = -1
	END TRY
	
	if li_sts < 0 or ll_state = adStateClosed then
		log.log(this, "get_adodb()", "Unable to open ADODB Connection", 3)
		adodb.disconnectobject()
		DESTROY adodb
		setnull(adodb)
		return -1
	...
	if sqlca.is_approle_set then
		...
		CATCH (throwable lo_error2)
			log.log(this, "get_adodb()", "Error setting app role (" + lo_error2.text + ")", 3)
			return -1
		END TRY
	end if
...
base_library_list = getlibrarylist()
...
if li_sts > 0 then
	if lo_env.ostype <> WindowsNT! then
		log.log(this, "initialize()", "EncounterPRO is only support on the Windows Operation System", 3)
		osversion = 0
	else
		...
		if lo_env.OSMajorRevision = 5 and lo_env.OSMinorRevision = 0 then
			...
			log.log(this, "initialize()", "EncounterPRO version 5 is only supported on Windows XP or later", 3)
			osversion = 4
		end if
	end if
else
	log.log(this, "initialize()", "Error getting environment information", 3)
	osversion = 0
end if
...
if isnull(lstr_printer.printername) or trim(lstr_printer.printername) = "" then
	...
	log.log(this, "set_printer()", "Printer not found (" + ps_printername + ")", 3)
	return
end if
...
if osversion <= 4 then
	...
	if li_sts <= 0 then
		log.log(this, "set_printer()", "Error setting default printer in registry (" + current_printer.printername + ")", 4)
	end if
else
	TRY
		eprolibnet4.SetDefaultPrinter(current_printer.printername)
	CATCH (oleruntimeerror lt_error)
		log.log(this, "set_printer()", "Error calling SetDefaultPrinter ~r~n" + lt_error.text + "~r~n" + lt_error.description, 4)
	END TRY
end if
...
if li_sts < 0 then
	log.log(this, "set_printer()", "Error setting printer (" + current_printer.printername + ")", 4)
end if
...
if not default_printer_set then
	...
	if printers.printer_count > 0 then
		if osversion <= 4 then
			...
			CATCH (oleruntimeerror lt_error)
				log.log(this, "set_printer()", "Error calling GetDefaultPrinter ~r~n" + lt_error.text + "~r~n" + lt_error.description, 4)
			END TRY
		end if
		...
		if isnull(common_thread.default_printer.printername) or trim(common_thread.default_printer.printername) = "" then
			...
			log.log(this, "get_default_printer()", "Unable to determine default printer", 3)
		else
...
if env.OSType = WindowsNT! and env.osmajorrevision = 5 and env.osminorrevision = 1 then
	...
	if li_sts < 0 then
		log.log(this, "get_external_sources()", "Error connecting to wia.devicemanager object (" + string(li_sts) + ")", 3)
	else
...
li_sts = lo_wia_devicemanager.connecttonewobject("wia.devicemanager")
if li_sts < 0 then
	log.log(this, "get_external_sources()", "Error connecting to wia.devicemanager object (" + string(li_sts) + ")", 4)
...
if li_sts < 0 then
	log.log(this, "get_external_sources()", "Error connecting to wia.commondialog object (" + string(li_sts) + ")", 4)
	return lstr_attachments
end if
...
if not lb_found then
	log.log(this, "get_attachments()", "Device not found (" + ps_device_id + ")", 4)
	return lstr_attachments
end if
...
if ll_count > 1 then
	log.log(this, "load_preferences()", "Multiple vaccine schedule config records found.  Only the first will be used.", 3)
end if
if ll_count > 0 then
	ls_temp = luo_data.object.domain_item[1]
	li_sts = f_get_config_object_info(ls_temp, lstr_config_object)
	if li_sts < 0 then
		log.log(this, "load_preferences()", "Error:  Vaccine Schedule load failed (" + ls_temp + ")", 4)
	end if
end if
...
if epie_initialized then
	...
	if li_sts < 0 then
		log.log(this, "xx_do_source()", "Error initializing EpIE Gateway", 4)
		return -1
	end if
end if
...
CATCH ( SoapException lt_error )
	log.log(this, "xx_do_source()", "Error calling EpIE upload gateway (" + lt_error.text + ")", 4)
	return -1
END TRY
...
public subroutine set_library_list ();string ls_liblist
long i

// This method constructs a library search list string from the base library list and any installed powerbuilder library components

ls_liblist = base_library_list

for i = 1 to pblibraries.pblibrary_count
	ls_liblist += "," + pblibraries.pblibrary[i].path
next

setlibrarylist(ls_liblist)


end subroutine

public function integer install_library (str_component_definition pstr_component_definition, str_component_version pstr_component_version);integer li_sts
str_pblibrary lstr_pblibrary
long i
boolean lb_found

lstr_pblibrary.component_id = pstr_component_version.component_id
lstr_pblibrary.version = pstr_component_version.version
lstr_pblibrary.compile_name = pstr_component_version.compile_name
lstr_pblibrary.component_description = pstr_component_definition.description

if isnull(pblibrary_path_root) or trim(pblibrary_path_root) = "" then
	pblibrary_path_root = temp_path + "\pblibrary"
end if

lstr_pblibrary.path = pblibrary_path_root
lstr_pblibrary.path += "\" + f_string_to_filename(pstr_component_version.component_id)
lstr_pblibrary.path += "\pblibrary_" + f_string_to_filename(pstr_component_version.compile_name)
if isnull(pstr_component_version.component_data) or len(pstr_component_version.component_data) <= 0 then
	log.log(this, "install_library()", "Error installing library:  component_data field may not be null", 4)
	return -1
end if

lstr_pblibrary.path += "." + f_string_to_filename(pstr_component_version.component_data)

if fileexists(lstr_pblibrary.path) then
	filedelete(lstr_pblibrary.path)
end if

if isnull(pstr_component_version.objectdata) or len(pstr_component_version.objectdata) = 0 then
	// Assume the caller didn't include the objectdata
	pstr_component_version = f_get_component_version(pstr_component_definition.component_id, pstr_component_version.version_name, true)
	if isnull(pstr_component_version.objectdata) or len(pstr_component_version.objectdata) = 0 then
		// If it's still null then we have a problem
		log.log(this, "install_library()", "Error installing library:  objectdata may not be null", 4)
		return -1
	end if
end if

li_sts = log.file_write(pstr_component_version.objectdata , lstr_pblibrary.path)
if li_sts <= 0 then
	log.log(this, "install_library()", "Error saving library file (" + lstr_pblibrary.path + ")", 4)
	return -1
end if

return set_library(lstr_pblibrary)


end function

public function boolean is_library_installed (str_component_definition pstr_component_definition, str_component_version pstr_component_version);long i

for i = 1 to pblibraries.pblibrary_count
	if pblibraries.pblibrary[i].component_id = pstr_component_version.component_id and pblibraries.pblibrary[i].version = pstr_component_version.version then return true
next
...
public function str_pblibraries installed_libraries ();return pblibraries

end function

public function integer install_debug_library (str_component_definition pstr_component, long pl_version, string ps_compile_name, string ps_debug_library_path);integer li_sts
str_pblibrary lstr_pblibrary
long i
boolean lb_found

lstr_pblibrary.component_id = pstr_component.component_id
lstr_pblibrary.version = pl_version
lstr_pblibrary.compile_name = ps_compile_name
lstr_pblibrary.component_description = pstr_component.description
lstr_pblibrary.path = ps_debug_library_path
lstr_pblibrary.debug = true

if not fileexists(lstr_pblibrary.path) then
	openwithparm(w_pop_message, "File does not exist (" + ps_debug_library_path + ")")
	return -1
end if
...
private function integer set_library (str_pblibrary pstr_pblibrary);long i
boolean lb_found

// We only want one version of any library installed at a time so see if it's already there
lb_found = false
for i = 1 to pblibraries.pblibrary_count
	if pstr_pblibrary.component_id = pblibraries.pblibrary[i].component_id then
		pblibraries.pblibrary[i] = pstr_pblibrary
		lb_found = true
	end if
next

if not lb_found then
	// The library isn't already in the list so add it
	pblibraries.pblibrary_count += 1
	pblibraries.pblibrary[pblibraries.pblibrary_count] = pstr_pblibrary
end if
...
public function integer get_library (string ps_component_id, long pl_version, ref str_pblibrary pstr_pblibrary);long i

for i = 1 to pblibraries.pblibrary_count
	if lower(pblibraries.pblibrary[i].component_id) = lower(ps_component_id) and pblibraries.pblibrary[i].version = pl_version then
		pstr_pblibrary =  pblibraries.pblibrary[i]
		return 1
	end if
next
...
public subroutine remove_all_libraries ();
pblibraries.pblibrary_count = 0

set_library_list()

end subroutine

private function integer remove_library (string ps_component_id, long pl_version);long i
long j
boolean lb_found
...
for i = 1 to pblibraries.pblibrary_count
	if lower(pblibraries.pblibrary[i].component_id) = lower(ps_component_id) and pblibraries.pblibrary[i].version = pl_version then
		// Take the library out of the list
		for j = i to pblibraries.pblibrary_count - 1
			pblibraries.pblibrary[j] = pblibraries.pblibrary[j + 1]
		next
		pblibraries.pblibrary_count -= 1
		lb_found = true
	end if

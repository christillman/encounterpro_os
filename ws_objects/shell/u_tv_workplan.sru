HA$PBExportHeader$u_tv_workplan.sru
forward
...
if isnull(pl_workplan_id) then
	log.log(this, "display_workplan()", "Null workplan_id", 4)
	return -1
end if

workplan = datalist.get_workplan(pl_workplan_id)
if isnull(workplan.workplan_id) then
	log.log(this, "display_workplan()", "workplan not found (" + string(pl_workplan_id) + ")", 4)
	return -1
end if
...
if isnull(ls_node_type) or trim(ls_node_type) = "" then
	log.log(this, "itempopulate", "No node_type", 4)
	return 1
end if

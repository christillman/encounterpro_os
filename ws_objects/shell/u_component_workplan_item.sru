HA$PBExportHeader$u_component_workplan_item.sru
forward
...
if isnull(patient_workplan_item_id) then
	mylog.log(this, "set_progress()", "Null patient_workplan_item_id", 4)
	return -1
end if
...
if lb_already_locked then
	...
	if cpr_mode = "CLIENT" then
		...
		log.log(this, "doing_service()", ls_message, 4)
	end if
	return 0
end if
...
if not isnull(treatment) and not isnull(ll_root_observation_sequence) then
	...
	if isnull(ls_root_observation_id) then
		log.log(this, "get_root_observation()", "An observation_sequence was specified but can't be found (" + string(ll_root_observation_sequence) + ")", 4)
		return ll_null
	end if
...
if isnull(ls_root_observation_id) or li_sts <= 0 then
	log.log(this, "get_root_observation()", "Unable to determine root observation_id", 4)
	return ll_null
end if
...
If cprdb.sqlcode = 100 Then
	mylog.log(this, "do_service()", "Workplan record not found (" + &
...
public function integer get_service_properties ();
if isnull(patient_workplan_item_id) then
	mylog.log(this, "get_workplan_properties()", "Null patient_workplan_item_id", 4)
	return -1
end if
...
if cprdb.sqlcode = 100 then
	mylog.log(this, "get_workplan_properties()", "Workplan Item Record not found (" + string(patient_workplan_item_id) + ")", 4)
	return -1
end if

//// Next, is the item a service
//If lower(item_type) <> "service" or isnull(item_type) Then
//	mylog.log(this, "do_service()", "Workplan item not a service (" + &
...
//	mylog.log(this, "do_service()", "Null ordered_service (" +&
//							string(patient_workplan_item_id) + ")", 4)
//	Return -1
//End If
...
if isnull(treatment.observation_id) then
	...
	if isnull(treatment.observation_id) then
		log.log(this, "root_observation_id()", "No treatment-root observation found", 4)
		return 0
	else
...
CHOOSE CASE lower(context_object)
	...
	CASE "patient"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "patient context but no cpr_id", 4)
			return -1
		end if
		setnull(object_key)
	CASE "encounter"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "encounter context but no cpr_id", 4)
			return -1
		end if
		if isnull(encounter_id) then
			get_attribute("object_key", ll_object_key)
			if isnull(ll_object_key) then
				log.log(this, "do_service()", "encounter context but no encounter_id", 4)
				return -1
			else
	...
	CASE "assessment"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "assessment context but no cpr_id", 4)
			return -1
		end if
		if isnull(problem_id) then
			get_attribute("object_key", ll_object_key)
			if isnull(ll_object_key) then
				log.log(this, "do_service()", "assessment context but no problem_id", 4)
				return -1
			else
	...
	CASE "treatment"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "treatment context but no cpr_id", 4)
			return -1
		end if
		if isnull(treatment_id) then
			get_attribute("object_key", ll_object_key)
			if isnull(ll_object_key) then
				log.log(this, "do_service()", "treatment context but no treatment object", 4)
				return -1
			else
	...
	CASE "observation"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "observation context but no cpr_id", 4)
			return -1
		end if
		if isnull(observation_sequence) then
			get_attribute("object_key", ll_object_key)
			if isnull(ll_object_key) then
				log.log(this, "do_service()", "observation context but no observation_sequence", 4)
				return -1
			else
	...
	CASE "attachment"
		if isnull(cpr_id) then
			log.log(this, "do_service()", "attachment context but no cpr_id", 4)
			return -1
		end if
		if isnull(attachment_id) then
			get_attribute("object_key", ll_object_key)
			if isnull(ll_object_key) then
				log.log(this, "do_service()", "attachment context but no attachment_id", 4)
				return -1
			else
...
//		AND status NOT IN('COMPLETED','CANCELLED') // msc 9/20/02 order new service under existing wp even if it's closed
		...
		If Not cprdb.check() Then Return -1
		If cprdb.sqlcode = 100 Then
			mylog.log(this, "order_service()", "Treatment Workplan record not found (" + string(treatment.treatment_id) + ")", 4)
			Return -1
		End if
	end if
End if
...
if isnull(ps_filepath) then
	log.log(this, "new_attachment()", "Null filepath", 4)
	return -1
end if

if not fileexists(ps_filepath) then
	log.log(this, "new_attachment()", "File does not exist (" + ps_filepath + ")", 4)
	return -1
end if

HA$PBExportHeader$u_ds_treatment_item.sru
forward
...
IF Isnull(parent_patient.open_encounter) THEN
	log.log(This, "refresh_status() in progress ...", "No open encounter", 4)
	RETURN -1
END IF
...
if sqlca.sqlcode = 100 then
	log.log(this, "refresh_service()", "encounter log record not found (" + string(al_encounter_log_id) + ")", 4)
	return -1
end if
...
IF Isnull(al_treatment_id) or al_treatment_id <= 0 THEN
	log.log(This, "refresh_status()", "Invalid treatment_id", 4)
	RETURN -1
END IF
...
ll_row = Find(ls_find, 1, rowcount())
IF ll_row <= 0 THEN
	log.log(This, "refresh_status()", "treatment not found", 4)
	RETURN -1
END IF
...
//IF not tf_check() THEN RETURN -1
//IF sqlca.sqlcode = 100 THEN
//	log.log(This, "refresh_status()", "Unable to refresh treatment_status", 3)
//ELSE
...
if isnull(current_patient.open_encounter) then
	log.log(this, "update_treatment()", "Cannot save treatment without an encounter context", 4)
	return -1
end if
...
If Isnull(puo_treatment) Then
	log.log(This,"treatment()","unable to create treatment component for "+&
				ls_treatment_type,4)
	Return 0
End If
...
if isnull(pl_treatment_id) or pl_treatment_id <= 0 then
	log.log(this, "set_treatment_progress()", "Invalid treatment_id", 4)
	return -1
end if
...
ll_row = find(ls_find, 1, rowcount())
if ll_row <= 0 then
	log.log(this, "set_treatment_progress()", "treatment not found", 4)
	return -1
end if
...
if isnull(ls_parent_treatment_type) then
	log.log(this, "add_followup_treatment_item()", "error getting treatment_type (" + string(pl_parent_treatment_id) + ")", 4)
	return -1
end if
...
If isnull(pstr_treatment.treatment_id) Then
	log.log(this, "new_treatment()", "treatment_id not generated", 4)
	Return -1
End if
...
If not Isvalid(puo_treatment) or Isnull(puo_treatment) Then
	log.log(this, "refresh_treatment_object()", "Invalid treatment object", 4)
	return -1
end if
...
if isnull(pl_treatment_id) then
	...
	if li_sts <= 0 then
		log.log(this, "initialize()", "Reselect treatment data failed", 4)
		return -1
	end if
...
for i = 1 to ll_rowcount
	...
	if li_sts <= 0 then
		log.log(this, "get_encounter_treatments()", "Error getting treatment details (" + current_patient.cpr_id + ", " + string(ll_treatment_id) + ")", 4)
		return -1
	end if
next
...
if isnull(luo_treatment.treatment_description) then
	...
	elseif not isnull(luo_treatment.drug_id) then
		...
		if li_sts <= 0 then
			log.log(this, "order_treatment()", "Unable to get drug definition (" + luo_treatment.drug_id + ")", 4)
	...
	elseif not isnull(luo_treatment.procedure_id) then
		...
		if not tf_check() then return -1
		if sqlca.sqlcode = 100 then
			log.log(this, "order_treatment()", "Unable to get procedure definition (" + luo_treatment.procedure_id + ")", 4)
...
if not tf_check() then return -1
if sqlca.sqlcode = 100 then
	log.log(this, "new_treatment()", "Treatment type not found (" + puo_treatment.treatment_type + ")", 3)
end if
...
If li_sts <= 0 Then
	tf_rollback()
	log.log(this, "new_treatment()", "Error saving new treatment", 4)
	Return -1
End if
...
if isnull(ll_real_treatment_id) then
	log.log(this, "new_treatment()", "Incorrect Treatment_id", 4)
	return -1
end if
...
if isnull(ls_treatment_type) or trim(ls_treatment_type) = "" then
	...
	if not tf_check() then return -1
	if isnull(ls_treatment_type) or trim(ls_treatment_type) = "" then
		log.log(this, "refresh_row()", "Invalid treatment_type (" + string(ll_treatment_id) + ")", 4)
		return -1
	end if
end if
...
if isnull(pl_treatment_id) or pl_treatment_id <= 0 then
	log.log(this, "set_treatment_changed()", "Invalid treatment_id", 4)
	return
end if
...
ll_row = find(ls_find, 1, rowcount())
if ll_row <= 0 then
	log.log(this, "set_treatment_changed()", "treatment not found", 4)
	return
end if

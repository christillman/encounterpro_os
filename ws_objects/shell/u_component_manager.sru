HA$PBExportHeader$u_component_manager.sru
forward
...
public function integer install_libraries ()
end prototypes
...
CHOOSE CASE lower(pstr_component_version.plugin_type)
	CASE "powerbuilder library"
		return common_thread.is_library_installed(pstr_component_definition, pstr_component_version)
	CASE ELSE
		CHOOSE CASE lower(pstr_component_version.independence)
			CASE "single"
				// For single, the desired version needs to be the last version installed
...
CHOOSE CASE lower(lstr_component_version.installer)
	CASE "powerbuilder library"
		li_sts = common_thread.install_library(pstr_component_definition, lstr_component_version)
		if li_sts <= 0 then
			ls_message = "Error installing library (" + pstr_component_definition.component_id + ", " + lstr_component_version.version_name + ")"
			log.log(this, "install_component_version()", ls_message, 4)
			sqlca.jmj_component_log(pstr_component_definition.component_id , &
									lstr_component_version.version,  &
									"Install", & 
	...
	CASE "setup.exe"
		if not common_thread.eprolibnet4.IsUserAdmin() then
...
public function integer install_libraries ();long i
u_ds_data luo_data
long ll_rowcount
...
luo_data.set_dataobject("dw_pb_libraries")

ll_rowcount = luo_data.retrieve()
if ll_rowcount < 0 then return -1

common_thread.remove_all_libraries()

for i = 1 to ll_rowcount
	ls_component_id = luo_data.object.component_id[i]
	lstr_component_definition = f_get_component_definition(ls_component_id)
	lstr_component_version = get_component_version(lstr_component_definition)
	
	common_thread.install_library(lstr_component_definition, lstr_component_version)
next

HA$PBExportHeader$u_patient.sru
forward
...
if ll_assessment_count < 0 then
	log.log(this, "load_assessments()", "Error loading assessments", 4)
	return -1
end if
...
if ll_treatment_count < 0 then
	log.log(this, "load_assessments()", "Error loading treatments", 4)
	return -1
end if
...
if ll_count < 0 then
	log.log(this, "load_encounters()", "Error loading encounters", 4)
	return -1
end if
...
if ll_attachment_count < 0 then
	log.log(this, "load_assessments()", "Error loading attachments", 4)
	return -1
end if
...
if li_sts <= 0 then
	log.log(this, "initialize()", "Unable to load patient", 4)
	return -1
end if
...
if li_sts <= 0 then
	log.log(this, "load_patient()", "Invalid cpr_id (" + cpr_id + ")", 4)
	return -1
end if
...
if isnull(attachment_server) or isnull(attachment_share) then
	log.log(this, "load_patient()", "Invalid Attachment Location (" + string(ll_attachment_location_id) + ")", 4)
	return -1
end if
...
//	li_sts = open_encounter.close()
	...
	elseif li_sts = 0 then
		log.log(this, "exit_office()", "Unable to close current encounter", 4)
		return 0
	end if
end if
...
if isnull(pstr_encounter.encounter_date) then
	log.log(this, "new_encounter()", "Null encounter date", 4)
	return -1
end if

lstr_stamp = f_get_stamp()
if not lstr_stamp.create_encounters then
	log.log(this, "new_encounter()", "This installation is not properly licensed to create encounters", 4)
	return 0
end if
...
if isnull(open_encounter.encounter_id) then
	log.log(this, "new_encounter()", "Error creating new encounter", 4)
...
if pb_order_workplan then
	...
	if li_sts <= 0 then
		log.log(this, "new_encounter()", "Error ordering encounter workplan", 4)
	end if
end if

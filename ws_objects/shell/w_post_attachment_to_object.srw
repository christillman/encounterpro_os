HA$PBExportHeader$w_post_attachment_to_object.srw
forward
...
li_sts = initialize()
if li_sts <= 0 then
	log.log(this,"open()","Initialize failed", 4)
	setnull(attachment_context.object_key)
	closewithreturn(this, attachment_context)
	Return
End If
...
CHOOSE CASE lower(attachment_context.context_object)
	...
	CASE "treatment"
		...
		next
	CASE ELSE
		log.log(this, "initialize()", "Invalid attachment_context.context_object (" + attachment_context.context_object + ")", 4)
		return -1
END CHOOSE
...
if isnull(attachment_context.context_object) or trim(attachment_context.context_object) = "" then
	...
	if isnull(attachment_context.folder) then
		log.log(this,"open()","Null Folder",4)
		setnull(attachment_context.object_key)
		closewithreturn(this, attachment_context)
		Return
	end if
	...
	if not tf_check() then
		log.log(this,"open()","Error getting Folder",4)
		setnull(attachment_context.object_key)
		closewithreturn(this, attachment_context)
		Return
	end if
	if sqlca.sqlcode = 100 then
		log.log(this,"open()","Folder not found (" + attachment_context.folder + ")",4)
		setnull(attachment_context.object_key)
		closewithreturn(this, attachment_context)
		Return
	end if

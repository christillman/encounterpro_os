HA$PBExportHeader$w_encounter_list.srw
forward
global type w_encounter_list from w_window_base
end type
type st_page from statictext within w_encounter_list
end type
type pb_down from u_picture_button within w_encounter_list
end type
type pb_up from u_picture_button within w_encounter_list
end type
type st_pick_encounter_mode from statictext within w_encounter_list
end type
type st_mode_title from statictext within w_encounter_list
end type
type st_type_title from statictext within w_encounter_list
end type
type st_pick_encounter_type from statictext within w_encounter_list
end type
type st_title from statictext within w_encounter_list
end type
...
ll_encounters = dw_encounters.retrieve(service.cpr_id, encounter_type, indirect_flag, '%')
...
ls_encounter_mode = service.get_attribute("encounter_mode")
if isnull(ls_encounter_mode) then
	st_pick_encounter_mode.text = "Direct and Indirect"
else
	st_pick_encounter_mode.text = ls_encounter_mode
end if
...
summary_report_id = service.get_attribute("summary_report_id")
if isnull(summary_report_id) then summary_report_id = "{4B657EFA-AB67-482B-9FAB-1764440DF116}"

summary_display_script_id = long(service.get_attribute("summary_display_script_id"))
...
type pb_epro_help from w_window_base`pb_epro_help within w_encounter_list
boolean visible = true
end type

type st_config_mode_menu from w_window_base`st_config_mode_menu within w_encounter_list
end type
...
service_list.do_service(service.cpr_id, ll_encounter_id, report_service, lstr_attributes)
...
event selected;call super::selected;clear_selected()

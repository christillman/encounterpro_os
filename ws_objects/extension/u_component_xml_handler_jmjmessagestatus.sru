HA$PBExportHeader$u_component_xml_handler_jmjmessagestatus.sru
forward
...
if isnull(ls_root) or lower(ls_root) <> "jmjmessagestatus" then
	log.log(this, "xx_interpret_xml()", "Error - Document root is not 'JMJMessageStatus'", 4)
	return -1
end if
		...
		CASE "ownerid"
			if len(ls_value) > 0 then
				if isnumber(ls_value) then
					...
					log.log(this, "xx_interpret_xml()", "OwnerID not numeric", 3)
				end if
			end if
		CASE "documenttype"
			if len(ls_value) > 0 then
		...
		CASE "severity"
			if len(ls_value) > 0 then
				if isnumber(ls_value) then
					...
					log.log(this, "xx_interpret_xml()", "Severity not numeric", 3)
				end if
			end if
	END CHOOSE
next
...
if isvalid(lo_from_block) and not isnull(lo_from_block) then
	...
	CATCH (pbdom_exception lo_error2)
		log.log(this, "process_xml", "Error - " + lo_error2.text, 4)
		return -1
	END TRY
			...
			CASE "addresseeid"
				if len(ls_value) > 0 then
					if isnumber(ls_value) then
						...
						log.log(this, "xx_interpret_xml()", "From/AddresseeID not numeric", 3)
					end if
				end if
			CASE "description"
				if len(ls_value) > 0 then
...
if isvalid(lo_to_block) and not isnull(lo_to_block) then
	...
	CATCH (pbdom_exception lo_error3)
		log.log(this, "process_xml", "Error - " + lo_error3.text, 4)
		return -1
	END TRY
			...
			CASE "addresseeid"
				if len(ls_value) > 0 then
					if isnumber(ls_value) then
						...
						log.log(this, "xx_interpret_xml()", "to/AddresseeID not numeric", 3)
					end if
				end if
			CASE "description"
				if len(ls_value) > 0 then
...
if isnull(ll_patient_workplan_item_id) then
	...
	log.log(this, "xx_interpret_xml()", ls_message, ll_loglevel)

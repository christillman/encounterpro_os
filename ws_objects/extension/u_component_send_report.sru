HA$PBExportHeader$u_component_send_report.sru
forward
...
elseif li_count = 0 then
	mylog.log(this, "send_to_subscribers()", "No subscribers for message type.  Message will not be sent (" + message_type + ")", 2)
	return -1
end if
...
if ll_message_id <= 0 then
	mylog.log(this, "send_to_subscribers()", "Error logging report for (" + string(subscription_id) + ")", 4)
	Return -1
end if

HA$PBExportHeader$w_route_web_upload.srw
forward
...
if li_sts <= 0 then
	log.log(this, "xx_xx_document_send()", "Error getting document file (" + string(document.patient_workplan_item_id) + ")", 4)
	close(this)
	return
end if
...
if lower(lstr_file.filetype) = "zip" then
	...
	if li_sts < 0 then
		log.log(this, "xx_xx_document_send()", "Error uncompressing document file (" + string(document.patient_workplan_item_id) + ")", 4)
		close(this)
		return
	end if
else
	...
	if li_sts <= 0 then
		log.log(this, "xx_xx_document_send()", "Error creating temp directory (" + filepath + ")", 4)
		close(this)
		return
	end if
	...
	if li_sts < 0 then
		log.log(this, "xx_xx_document_send()", "Error writing document file (" + string(document.patient_workplan_item_id) + ")", 4)
		close(this)
		return
	end if

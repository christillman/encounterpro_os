HA$PBExportHeader$u_component_schedule_encounterpro.sru
forward
...
mylog.log(this, "xx_get_next_checked_in()","Processing Message ID (" + is_message_id + ", " + string(ldt_encounter_date_time,"yyyy/mm/dd hh:mm:ss") + ")",2)
...
if len(trim(ls_cprid)) = 0 OR isnull(ls_cprid) Then
	mylog.log(this, "xx_get_next_checked_in()","Cpr id is NULL",2)
	Return 0	
End If
...
//		ps_encounter_date = string(Today(),"mm/dd/yyyy")
		if ls_appointment_date = ls_this_date then
			...
			USING 	cprdb;
			IF NOT cprdb.check() THEN RETURN -1
			mylog.log(this, "xx_get_next_checked_in()","Encounter Billing ID, Message ID (" + ls_billing_id + ", " + ls_message_id + ") is out of date",2)
			Return 1
		ENd If	
	else
	...
	if not isnull(ll_encounter_id) then
		...
		If (left(ls_open_encounter_date,10) = ls_this_date) and (ls_open_encounter_type = ls_encounter_type) then // encounter created for the same day
			...
			USING 	cprdb;
			IF NOT cprdb.check() THEN RETURN -1
			mylog.log(this, "xx_get_next_checked_in()","There's already an open encounter("+string(ll_encounter_id)+" for this Billing ID, Message ID (" + ls_billing_id + ", " + is_message_id + ")",3)
			Return 1
		End If
	End If
...
mylog.log(this, "xx_get_next_checked_in()","Encounter Message has been processed, Billing ID, Message ID (" + ls_billing_id + ", " + ls_message_id + ")",2)

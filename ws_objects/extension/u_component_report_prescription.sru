HA$PBExportHeader$u_component_report_prescription.sru
forward
...
string ls_prescription_date_format
...
ls_prescription_date_format = get_attribute("prescription_date_format")
if isnull(ls_prescription_date_format) then ls_prescription_date_format = "mmm dd, yyyy"

// Check for valid patient & enocunter
If isnull(current_patient) Then
	mylog.log(this, "xx_printreport()", "No patient context ", 4)
	Return -1
Elseif isnull(ll_encounter_id) Then
	if isnull(current_patient.open_encounter_id) then
		mylog.log(this, "xx_printreport()", "No encounter context ", 4)
		Return -1
	else
...
report_datastore.object.dob.format = ls_prescription_date_format
report_datastore.object.encounter_date.format = ls_prescription_date_format
...
For i = 1 To li_count
	...
	IF Isnull(luo_ordered_by.supervisor) THEN
		if luo_ordered_by.certified = "Y" then
			...
			log.log(this, "xx_printreport()", "the user (" + &
	...
	IF Isnull(luo_provider) THEN
		mylog.log(this, "load_report()", "No provider for prescription (" + &
...
If report_datastore.rowcount() > 0 Then
	print_datastore()
Else
	mylog.log(this, "xx_printreport()", "No prescriptions for this encounter (" + &

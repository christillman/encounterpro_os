HA$PBExportHeader$u_component_xml_handler_jmj.sru
forward
...
if isnull(ls_root) or lower(ls_root) <> "jmjdocument" then
	log.log(this, "xx_interpret_xml()", "Error - Document root is not 'JMJDocument'", 4)
	return -1
end if
		...
		CASE "customerid"
			if len(ls_value) > 0 then
				if isnumber(ls_value) then
					...
					log.log(this, "xx_interpret_xml()", "Customer ID not numeric", 3)
				end if
			end if
		CASE "ownerid"
			if len(ls_value) > 0 then
				if isnumber(ls_value) then
					...
					log.log(this, "xx_interpret_xml()", "Owner ID not numeric", 3)
				end if
			end if
		CASE "actors"
			lo_actors = lo_elem[i]
...
if not isvalid(lo_actors) then
	...
	if li_sts < 0 then
		log.log(this, "xx_interpret_xml()", "Error - An error has occured while reading the actors", 4)
		return -1
	end if
end if

if isnull(owner_id) then
	log.log(this, "xx_interpret_xml()", "Error - Document has no OwnerID element", 4)
	return -1
end if
...
elseif customer_id <> sqlca.customer_id then
	if cpr_mode = "SERVER" then
		log.log(this, "xx_interpret_xml()", "Data file customer_id doesn't match local database", 4)
		return -1
	else
...
if isnull(lstr_encounter.encounter_date) then
	log.log(this, "process_encounter()", "Encounter must have an encounter date", 4)
	return -1
end if
...
if lower(ls_purpose) = "check in" then
	...
	if ld_encounter_date > today() then
		log.log(this, "process_encounter()", "Encounter date may not be in the future", 4)
		return -1
	end if
	if ld_encounter_date < today() then
		if not datalist.get_preference_boolean( "Preferences", "ALLOW_PAST_ENCOUNTERS", false) then
			log.log(this, "process_encounter()", "Encounter date may not be in the past", 3)
	...
	if isnull(lstr_encounter.office_id) then
		...
		if isnull(lstr_encounter.office_id) then
			log.log(this, "process_encounter()", "Encounter Location must be mapped to an Office for check-in events", 4)
			return -1
		end if
	end if
...
if isnull(lstr_treatment.begin_date) then
	log.log(this,"process_treatment()","No begin_date provided in treatment block", 4)
	return -1
end if
...
if not tf_check() then return -1
if li_sts <= 0 then
	log.log(this, "pick_office()", "Error setting office actor", 4)
	return -1
end if

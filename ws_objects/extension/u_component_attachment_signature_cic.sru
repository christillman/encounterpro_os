HA$PBExportHeader$u_component_attachment_signature_cic.sru
forward
...
if not fileexists(ls_tempfile) then
	log.log(this, "xx_render()", "Error creating signature image", 4)
	return -1
end if
...
if li_sts < 0 then
	log.log(this, "display_image()", "Error creating EPImageControl object (" + string(li_sts) + ")", 3)
	ps_file = ls_tempfile
else
	...
	if li_sts <= 0 then
		log.log(this, "display_image()", "Error reducing bitmap", 3)
		ps_file = ls_tempfile
	end if

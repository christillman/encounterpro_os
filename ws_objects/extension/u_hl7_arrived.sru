HA$PBExportHeader$u_hl7_arrived.sru
forward
...
If ll_count > 0 then 
	mylog.log(this, "arrived()", "message referencing this Message ID (" + string(is_message_id) + ") already arrived", 3)
	Return 1
End If	
	...
	****************/
	if upper(bs_system) = 'GENIUS' then
		...
		if upper(ls_arrived) <> 'STARTED' then 
			mylog.log(this, "arrived_genius()", "SIU not arrived (" + ls_arrived  + ")", 2)
			return 1
		end if
	end if	
	...
	if ll_count > 0 then
		...
		mylog.log(this, "arrived()", "PID Segment not provided, Message ID (" + string(is_message_id) + ")", 4)
		Return -1
	end if	
	/*******************
	* PV1 Segment
	...
	****************/
	...
	if upper(bs_system) = 'INTEGRY' then
		...
		if upper(ls_chief_complaint) <> 'CHECK-IN' AND upper(ls_chief_complaint) <> 'ARRIVED' AND upper(ls_chief_complaint) <> 'CI' then 
			mylog.log(this, "arrived_integry()", "SIU not arrived (" + ls_chief_complaint  + ")", 2)
			return 1
		end if
...
Elseif is_msgtype = 'ADT' then
	...
	if isnull(omsg.patientvisit) then
		mylog.log(this, "arrived()", "Pativent Visit Segment not provided, Message ID (" + string(is_message_id) + ")", 4)
		return -1
	end if
	...
	mylog.log(this, "arrived()", "Invalid message type (" + is_msgtype + ")", 4)
	Return 1
end if
	...
	if isnull(ls_external_id) or ls_external_id = "" then
		if isnull(ls_internal_id) or ls_internal_id = "" then
			if isnull(ls_alternate_PID) or ls_alternate_PID = "" then
				mylog.log(this, "arrived()", "Patient External ID not provided, Message ID (" + string(is_message_id) + ")", 4)
				return -1
			else
...
if not isnull(ls_scheduledatetime) then
	...
	if ld_scheduledate <> ld_thisdate then
		if left(upper(ls_allow_past_encounter),1) = "F" or left(upper(ls_allow_past_encounter),1) = 'N' Then
			...
			mylog.log(this, "arrived_mpm()", ls_message , 4)
			GOTO error
		else
			if ld_scheduledate > ld_thisdate then
				ls_message = "Check in date ( "+ls_scheduledate+") is invalid"
			...
			mylog.log(this, "arrived_mpm()", "formatted date(" + ls_scheduledate + ") not today. but allowing the encounter to be created", 3)
...
if upper(bs_system) = 'COMPANION' Then
	if upper(ls_chief_complaint) <> 'CK' then // notification of check in by Companion
		...
		mylog.log(this, "arrived()", "not a check in event", 1)
		GOTO Error
	end if
...
if not isnull(ls_attending_doctor_id) and len(ls_attending_doctor_id) > 0 Then
	...
	If sqlca.sqlcode = 100 then // no match
		...
		if sqlca.sqlcode = 100 then
			...
			mylog.log(this, "arrived_mpm()", ls_provider_message , 3)
			Setnull(ls_attending_doctor)
		end if
	End if
end if
...
If ((isnull(ls_appointment_type_code) or len(ls_appointment_type_code) = 0) AND &
	...
	mylog.log(this, "arrived()",ls_appointment_message, 3)
Else	
	mylog.log(this, "arrived()", "Appointment Type Code(" + ls_appointment_type_code +")" , 1)
	mylog.log(this, "arrived()", "Appointment Type Text(" + ls_appointment_type_text +")" , 1)
...
If not isnull(ls_resource) and len(ls_resource) > 0 Then
	mylog.log(this, "arrived()", "Resource Code(" + ls_resource +")" , 2)
	lb_resource = false
	...
	if not lb_resource then
		mylog.log(this, "arrived()", "Resource not mapped ( " + ls_resource+ ")", 3)
		setnull(ls_attending_resource)
		setnull(ls_resource_encounter_type)
...
if isnull(is_cpr_id) then	
	...
	if not isnull(ls_cprid) then
		...
		IF cprdb.sqlcode = 100 THEN
			...
			mylog.log(this, "arrived()",ls_message, 4)
			GOTO error
		END IF
	end if
else
...
SELECT count(*)  
   ...
   WHERE x_encounterpro_Arrived.billing_id = :ls_epro_billing_id AND
	...
	IF NOT cprdb.check() THEN RETURN -1
If ll_count > 0 then 
	mylog.log(this, "arrived()", "Patient already arrived this day within the hour" + ls_cprid + " ..Aborting Encounter Creation, Message ID (" + ls_billing_id + ", " + is_message_id + ")", 2)
	return 1	
end if

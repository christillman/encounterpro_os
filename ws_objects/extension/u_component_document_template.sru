HA$PBExportHeader$u_component_document_template.sru
forward
...
CATCH (exception le_error)
	log.log(this, "xx_do_source()", "Error setting document clinical objects (" + le_error.text + ")" , 4)
	THROW le_error
END TRY
...
CATCH (exception le_error2)
	log.log(this, "xx_do_source()", "Error resolving templates: " + le_error2.text, 4)
	THROW le_error2
END TRY
...
CHOOSE CASE upper(ls_merge)
	CASE "ZIP"
		...
		if li_sts <= 0 then
			log.log(this, "xx_do_source()", "Error merging documents with compression", 4)
			return -1
		end if
...
if not isnull(ls_compression) and lower(ls_compression) <> "none" then
	...
	li_sts = f_compress_documents(lstr_compressed_document_file, lstr_document_files)
	if li_sts <= 0 then
		log.log(this, "xx_do_source()", "Error compressing document", 4)
		return -1
	end if
...
if isnull(ls_report_id) or len(ls_report_id) < 36 then
	log.log(this, "scan_templates_and_resolve_fields()", "No report_id", 4)
	return -1
end if
...
if isnull(ll_document_patient_workplan_item_id) then
	log.log(this, "scan_templates_and_resolve_fields()", "document_patient_workplan_item_id attribute not found", 4)
	return -1
end if
...
if ll_attribute_sequence > 0 then
	...
	if li_sts < 0 then
		log.log(this, "scan_templates_and_resolve_fields()", "Unable to read document element mappings", 4)
	end if
end if

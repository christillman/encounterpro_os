HA$PBExportHeader$u_component_serverservice.sru
forward
...
if batch_mode then
	if time(ls_time) > now() then
		log.log(this, "xx_initialize()", "billing is running in batch mode and will start sending only @ "+string(batch_processing_time), 3)
	end if
end if
...
if li_sts1 < 0 then
	log.log(this, "timer_ding()", "Error calling do_todo_services()", 4)
end if
...
if isnull(current_user) or not isvalid(current_user) then
	log.log(this,"do_todo_services()","current user object invalid ",3)
	return 1
end if
...
if batch_mode then
	...
	if ldt_now < batch_processing_time then 
		...
		If date(ls_date) >= date(today()) then
			mylog.log(this, "xx_do_service", "Billing batch is already sent "+ls_date, 1)	
			lb_batch_ready = false
		else
			lb_batch_ready = true
		end if
...
if isnull(current_user) or not isvalid(current_user) then
	if server_service_id <= 0 or isnull(server_service_id) then
		log.log(this,"do_scheduled_services()","current user can't be set ",3)
		Return 1
	end if
	SELECT system_user_id
	INTO :ls_logon
...
if isnull(current_user) or not isvalid(current_user) then
	log.log(this,"do_scheduled_services()","current user object invalid ",3)
	return 1
end if

ll_count = luo_services.retrieve(current_user.user_id, office_id)
if ll_count < 0 then
	log.log(this,"do_scheduled_services()","error getting scheduled services",3)
	return -1
end if
...
for i = 1 to ll_count
	...
	if ll_attribute_count < 0 then
		log.log(this,"do_scheduled_services()","error getting service attributes (" + current_user.user_id + ", " + string(ll_service_sequence) + ")",4)
		return -1
	end if

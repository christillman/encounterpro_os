...
{
	/// <summary>
	/// Summary description for AttachmentWrapper.
	/// </summary>
	[Serializable]
	public class AttachmentWrapper : PBCOMWrapper
	{
		public AttachmentWrapper(): base() { }

		public bool Is_Displayable(string Extension)
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Is_Displayable");
			bool bRet = (bool)method.Invoke(connectedClass, new object[]{Extension});
			return bRet;
		}

		public bool Is_Editable(string Extension)
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Is_Editable");
			bool bRet = (bool)method.Invoke(connectedClass, new object[]{Extension});
			return bRet;
		}

		public int Display(byte[] Attachment, string Extension)
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Display");
			return (int)method.Invoke(connectedClass, new object[]{Attachment, Extension});
		}
		
		public byte[] Edit(byte[] Attachment, string Extension)
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Edit");
			byte[] baRet = (byte[])method.Invoke(connectedClass, new object[]{Attachment, Extension});
			return baRet;
		}

		public byte[] Render(byte[] Attachment, string Extension, int Width, int Height, ref string FileType)
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Render");
			object[] parms = new object[]{Attachment, Extension, Width, Height, FileType};
			byte[] baRet = (byte[])method.Invoke(connectedClass, parms);
			FileType = (string)parms[4];
			return baRet;
		}
		...
		public void Dispose()
		{
			if(connectedType == null || connectedClass == null)
				throw new Exception("Not connected to implementation class.");
			System.Reflection.MethodInfo method = connectedType.GetMethod("Dispose");
			method.Invoke(connectedClass, new object[]{});
		}

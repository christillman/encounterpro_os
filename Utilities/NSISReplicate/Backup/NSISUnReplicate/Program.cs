    ...
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length < 3)
            {
                ...
                Console.Error.WriteLine("EX. NSISUnReplicate.exe \"C:\\SourceFiles\\*.*\" \"$INSTDIR\" \"Delete %FILE%\"");
                return;
            }
        ...
        private static void process(string Source, string Dest, string Command)
        {
            ...
            foreach (string file in files)
            {
                ...
                Console.WriteLine(Command.Replace("%FILE%", destFile).Replace("\\r", "\r").Replace("\\n", "\n"));
            }
            Console.WriteLine("RmDir \"" + Dest + "\"");
        }
    }
}

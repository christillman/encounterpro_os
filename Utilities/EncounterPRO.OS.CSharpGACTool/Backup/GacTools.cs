...
namespace EncounterPRO.OS
{
    ...
    {
        static Publish publish = new Publish();
        ...
        public static void RegisterAssembly(string AssemblyPath)
        {
            if (!System.IO.File.Exists(AssemblyPath))
                return;
            publish.RegisterAssembly(AssemblyPath);
        }

        public static void UnRegisterAssembly(string AssemblyPath)
        {
            if (!System.IO.File.Exists(AssemblyPath))
                return;
            publish.UnRegisterAssembly(AssemblyPath);
        }
    }
}
